 ...
 */
AFM.timeline.Model = Base.extend({
    // unique timeline ID
    id: '',
	...
	loadTimeline: function(timelineDTO) {
		...
		for (var i = 0; i < timemarksDTO.length; i++) {
			...
			this.timemarks[i] = new AFM.timeline.Timemark(
			    timemarkDTO.column, 
		...
		for (var i = 0; i < resourcesDTO.length; i++) {
			...
			var resource = new AFM.timeline.Resource(
		...
		for (var i = 0; i < eventsDTO.length; i++) {
			...
			this.addEvent(new AFM.timeline.Event(
			    eventDTO.eventId, 
	...
	*/
    ...
    addResource: function(resource) {
        ...
        for (var column = 0; column < colNum; column++) {
            var timeslotStatus = AFM.timeline.Timeslot.STATUS_AVAILABLE;
            if (column < resource.columnAvailableFrom || column >= resource.columnAvailableTo) {
                timeslotStatus = AFM.timeline.Timeslot.STATUS_NOT_AVAILABLE;
            }
            this.timeslots[row][column] = new AFM.timeline.Timeslot(
	...
	*/	
	getEventForTimeslot: function(timeslot) {
		...
		if (timeslot.status == AFM.timeline.Timeslot.STATUS_EXISTING_RESERVATION ||
		    timeslot.status == AFM.timeline.Timeslot.STATUS_NEW_RESERVATION) {			
 ...
 */
AFM.timeline.Timemark = Base.extend({
 ...
 */
AFM.timeline.Timeslot = Base.extend({
	...
	constructor: function(id, timemark, resource, status) {
	    ...
	    if (valueExists(status)) {
	        ...
	        this.status = AFM.timeline.Timeslot.STATUS_AVAILABLE;
	    }
	},
	...
	isAvailable: function() {
		return (this.status == AFM.timeline.Timeslot.STATUS_AVAILABLE);
	},
    ...
    isUnavailable: function() {
        return (this.status == AFM.timeline.Timeslot.STATUS_NOT_AVAILABLE);
    }
}, {
    // ----------------------- static members ------------------------------------------------------
 ...
 */
AFM.timeline.Resource = Base.extend({
 ...
 */
AFM.timeline.Event = Base.extend({
	...
	constructor: function(eventId, resourceRow, columnStart, columnEnd, isNew, timeline, canEdit, canDelete, preBlockTimeslots, postBlockTimeslots) {
		...
		this.status = isNew ? AFM.timeline.Event.STATUS_NEW : AFM.timeline.Event.STATUS_EXISTING;
    ...
    updateTimeslots: function() {
        var timeslotStatus = this.isNew() ? AFM.timeline.Timeslot.STATUS_NEW_RESERVATION : AFM.timeline.Timeslot.STATUS_EXISTING_RESERVATION;
        ...
        this.timeline.setTimeslotsStatus(preTimeslots, AFM.timeline.Timeslot.STATUS_BLOCK);
        
        var postTimeslots = this.timeline.getTimeslotsForEventPostBlocks(this);
        this.timeline.setTimeslotsStatus(postTimeslots, AFM.timeline.Timeslot.STATUS_BLOCK);
    },
    ...
    clearTimeslots: function() {
        ...
        this.timeline.setTimeslotsStatus(allTimeslots, AFM.timeline.Timeslot.STATUS_AVAILABLE);
    },
	...
	*/
	...
	setStart:function(column){
		if(this.columnStart!=column){
			...
			this.status = AFM.timeline.Event.STATUS_MODIFIED;
	...
	getDescribingStatus: function(){
		var result = "EXISTING";
		if (this.status == AFM.timeline.Event.STATUS_NEW)
			result = "NEW";
		else if(this.status == AFM.timeline.Event.STATUS_DELETED)
			result = "DELETED";
		else if(this.status == AFM.timeline.Event.STATUS_MODIFIED)
			result = "MODIFIED";
		return result;
	},

    isExisting: function() {
	    return this.status == AFM.timeline.Event.STATUS_EXISTING;
	},

    isDeleted: function() {
	    return this.status == AFM.timeline.Event.STATUS_DELETED;
	},

    isNew: function() {
        return (this.status == AFM.timeline.Event.STATUS_NEW || this.status == AFM.timeline.Event.STATUS_MODIFIED);
    }
},
{
    // ----------------------- static constants ----------------------------------------------------
