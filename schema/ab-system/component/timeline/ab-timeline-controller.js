...
/**
 * All timeline classes are contained in the AFM.timeline namespace.
 */
AFM.namespace('timeline');
 ...
 * 
 * @class AFM.timeline.Block
 * @constructor
 */
AFM.timeline.Block = Base.extend({
    ...
    // status (one of the AFM.timeline.Timeslot status codes)
    status: null,
     ...
     * @constructor
     */
    constructor: function(timeline, row, start, end, status, registerEvents) {
    ...
    render: function() {
        if (this.element == null) {
            ...
            var height = this.timeline.timeslotHeight - 1;
            s.height = height + 'px';
    ...
    getCssClassName: function() {
        var className = '';
        if (this.status == AFM.timeline.Timeslot.STATUS_NOT_AVAILABLE) {
            className =  'timeline_unavailable';
        } else if (this.status == AFM.timeline.Timeslot.STATUS_BLOCK) {
            className =  'timeline_block';
        } else if (this.status == AFM.timeline.Timeslot.STATUS_FUTURE_BLOCK) {
            className =  'timeline_future';
        } else if (this.status == AFM.timeline.Timeslot.STATUS_EXISTING_RESERVATION) {
            className =  'timeline_eventReadOnly';
        } else if (this.status == AFM.timeline.Timeslot.STATUS_NEW_RESERVATION) {
     ...
     */
    getZIndex: function() {
        return AFM.timeline.Block.Z_INDEX_BLOCK;
    }
}, 
{
    // z-index constants
 ...
 * 
 * @class AFM.timeline.EventBlock
 * @constructor
 */
AFM.timeline.EventBlock = AFM.timeline.Block.extend({
    
    // reference to AFM.timeline.Event that is displayed by this block
    event: null,
    
    // reference to AFM.timeline.Resource that is this block belongs to
    resource: null,
    ...
    constructor: function(timeline, row, start, end, event, resource) {
        var status = AFM.timeline.Timeslot.STATUS_NEW_RESERVATION;
        
        this.resource = resource;
        if (valueExists(event)) {
            this.event = event;
            if (event.isExisting()) {
                status = AFM.timeline.Timeslot.STATUS_EXISTING_RESERVATION;
            }
        }
        ...
        if (position.start < start) {
	        this.preBlock = new AFM.timeline.Block(timeline, row, position.start, position.end, AFM.timeline.Timeslot.STATUS_BLOCK, false);
        ...
        if (position.end > end) {
	        this.postBlock = new AFM.timeline.Block(timeline, row, position.start, position.end, AFM.timeline.Timeslot.STATUS_BLOCK, false);
    ...
    getCssClassName: function() {
        ...
        if (this.end == this.start) {
            if (this.status == AFM.timeline.Timeslot.STATUS_EXISTING_RESERVATION) {
     ...
     */
    getZIndex: function() {
        return (this.status == AFM.timeline.Timeslot.STATUS_EXISTING_RESERVATION) 
               ? AFM.timeline.Block.Z_INDEX_EXISTIG_EVENT
               : AFM.timeline.Block.Z_INDEX_NEW_EVENT;
    },
    ...
    endDrag: function() {
        if (this.event == null) {
            // create new event
			this.event = new AFM.timeline.Event(null, this.row, this.start, this.end, true, this.timeline.model);
    ...
    handleMouseOver: function(e) {
        ...
        var eventStatus = this.status == AFM.timeline.Timeslot.STATUS_EXISTING_RESERVATION ? getMessage("tooltipExistReserv") : getMessage("tooltipNewReserv");
        
        var defaultTooltip = 
            '<tr><td class="title" colspan="2">' + eventStatus + '</td></tr>'
          + '<tr><td class="label">' + getMessage("tooltipTimeStart") + ':</td><td class="value">' + timeslotStart.timemark.dateTimeLabel + '</td></tr>'
          + '<tr><td class="label">' + getMessage("tooltipTimeEnd") + ':</td><td class="value">' + timeslotEnd.timemark.dateTimeLabel + '</td></tr>';
 ...
 * 
 * @class AFM.timeline.TimelineController
 * @constructor
 */
AFM.timeline.TimelineController = Base.extend({
	...
	constructor: function(id, isEditable, policy) {
		...
		this.grid = new AFM.grid.Grid(id, configObj);
		
		// create the empty model
		this.model = new AFM.timeline.Model(id);
		...
		if (valueExists(policy)) {
		    ...
		    this.policy = new AFM.timeline.DefaultPolicy();
		}
	},
	...
	startDrag: function(e, eventBlock) {
	    ...
	    if (!eventBlock) {
	        ...
	        eventBlock = new AFM.timeline.EventBlock(this, row, column, column, null, this.getResource(row));
	    }
	...
	getTimeslotPosition: function(e) {
	    ...
	    if (this.useLayer) {
	        x = x + this.rowBlocksParent.scrollLeft;
	        y = y + this.rowBlocksParent.scrollTop;
	    }
	    ...
	    var column = this.timeslotPositionsX.length;
	...
	/**
	 * Refreshed the timelinevisible content from the model.
	 */
	refreshTimelineUI: function() {
		...
		for (var c = 0; c < numColumns; c++) {
			...
			if (typeof timemark.type == 'undefined' || timemark.type == 'major') {
			    ...
			    var column = new AFM.grid.Column("timemark-" + c, timemark.dateTimeLabel, 'time', null, null, majorTimemarkSpan);
			    this.grid.addColumn(column);
			} 
			var column = new AFM.grid.Column('', '', 'time', null, this.onCreateTimeslot);
	...
	findRowBlocksParent: function() {
	    ...
	    if (gridHeight != 'undefined' && gridHeight != '') {
	        ...
	        // block pasitions are calculated relative to the original parent element
    		this.timelineX = DOM.getX(this.grid.parentElement);
    		this.timelineY = DOM.getY(this.grid.parentElement);
	        
	    } else {
	        // height is undefined - position blocks in the document
	        this.useLayer = false;
	        this.rowBlocksParent = document.body;
	        
	        // block positions are relative to the page
    		this.timelineX = 0;
    		this.timelineY = 0;
	    }
	},
	...
	cacheTimeslotPositions: function() {
		...
		for (var c = 0; c < this.getTimemarks().length; c++) {
	        ...
	        var left = DOM.getX(timemarkCell) - this.timelineX;
		...
		for (var r = 0; r < this.getRowNumber(); r++) {
	        ...
	        var top = DOM.getY(rowElement) - this.timelineY;
	...
	rePositionHeaderDiv: function() {
		...
		var grid = AFM.view.View.getControl(window, 'timeline');
	 ...
	 * @param {model} Instance of the AFM.timeline.Model class.
	 */
	...
	createRowBlocks: function(row) {
	    ...
	    this.addRowBlock(row, AFM.timeline.Timeslot.STATUS_NOT_AVAILABLE, 0, rowResource.columnAvailableFrom - 1);
	    this.addRowBlock(row, AFM.timeline.Timeslot.STATUS_FUTURE_BLOCK,  rowResource.columnAvailableFrom, rowResource.columnAvailableFrom + rowResource.preBlockTimeslots - 1);
	    this.addRowBlock(row, AFM.timeline.Timeslot.STATUS_FUTURE_BLOCK,  rowResource.columnAvailableTo - rowResource.postBlockTimeslots, rowResource.columnAvailableTo - 1);
	    this.addRowBlock(row, AFM.timeline.Timeslot.STATUS_NOT_AVAILABLE, rowResource.columnAvailableTo, this.getTimemarks().length - 1);
		...
		for(var i = 0; i < events.length; i++){
			...
			if (!event.isDeleted()) {
    	        var eventBlock = new AFM.timeline.EventBlock(this, row, event.columnStart, event.columnEnd, event, rowResource);
     ...
     * @param {row} Row index.
	 */
	addRowBlock: function(row, status, startColumn, endColumn) {
	    if (endColumn >= startColumn) {
	        var block = new AFM.timeline.Block(this, row, startColumn, endColumn, status, true);
    ...
    showTooltip: function(timeslotPosition, content) {
	    ...
	    if (this.useLayer) {
	        tooltipLeft = tooltipLeft - this.rowBlocksParent.scrollLeft;
	        tooltipTop = tooltipTop - this.rowBlocksParent.scrollTop;
	    }
 ...
 */
AFM.timeline.DefaultPolicy = Base.extend({
