...
public class HighlightPatternServiceTest extends DataSourceTestBase {
    ...
    public void testCreateHatchPattern() {
        HighlightPatternService hpatGenerator = new HighlightPatternService();
        
        // only colors, no sortfield
        hpatGenerator.createHatchPatterns("rmcat", "hpattern_acad", true, null, true);
        DataSource ds =
                DataSourceFactory.createDataSourceForFields("rmcat", new String[] { "rm_cat",
                        "hpattern_acad" });
        
        List<String> generatedHPats = new ArrayList<String>();
        for (DataRecord res : ds.getRecords()) {
        ...
        hpatGenerator.createHatchPatterns("gpstd", "hpattern_acad", false, null, false);
        ds =
                DataSourceFactory.createDataSourceForFields("gpstd", new String[] { "gp_std",
                        "hpattern_acad" });
        ...
        hpatGenerator.createHatchPatterns("dp", "hpattern_acad", false, "dv_id", false);
        ds =
                DataSourceFactory.createDataSourceForFields("dp", new String[] { "dv_id", "dp_id",
                        "hpattern_acad" });
        generatedHPats = new ArrayList<String>();
        ...
        for (DataRecord res : ds.getRecords()) {
            ...
            if (dv_id != null && dv_id.equals(res.getString("dp.dv_id"))) {
                String hpat = res.getString("dp.hpattern_acad");
                HighlightPattern pattern = HighlightPatternUtilities.decodePattern(hpat);
        ...
        hpatGenerator.createHatchPatterns("rmtype", "hpattern_acad", true, "rm_cat", false);
        ds =
                DataSourceFactory.createDataSourceForFields("rmtype", new String[] { "rm_type",
                        "rm_cat", "hpattern_acad" });
        generatedHPats = new ArrayList<String>();
