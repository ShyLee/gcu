    ...
    private Date getLeaseEndDate(final String lsId) {
        ...
        final DataSource dataSource =
                DataSourceFactory.createDataSourceForFields(Constants.LS_TABLE, new String[] {
                        Constants.LS_ID, Constants.DATE_END });
        dataSource.addRestriction(Restrictions.eq(Constants.LS_TABLE, Constants.LS_ID, lsId));
        final DataRecord record = dataSource.getRecord();
        if (StringUtil.notNullOrEmpty(record.getDate(Constants.LS_DATE_END))) {
            result = record.getDate(Constants.LS_DATE_END);
        }
        return result;
    }
