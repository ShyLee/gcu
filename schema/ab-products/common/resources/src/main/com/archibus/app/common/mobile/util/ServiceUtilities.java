    ...
    public static String composeFieldRestriction(final String inputRestriction, final String table,
        ...
        if (StringUtil.notNullOrEmpty(value)) {
            if (StringUtil.notNullOrEmpty(inputRestriction)) {
                restriction =
                        restriction + ServiceConstants.SQL_AND + table + ServiceConstants.SQL_DOT
                                + field + ServiceConstants.OPERATOR_EQ
                                + Utility.sqlrMakeLiteral(value);
            } else {
                restriction =
                        table + ServiceConstants.SQL_DOT + field + ServiceConstants.OPERATOR_EQ
                                + Utility.sqlrMakeLiteral(value);
            }
        }
        return restriction;
