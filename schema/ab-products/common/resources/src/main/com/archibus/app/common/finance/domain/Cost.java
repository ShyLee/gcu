...
public class Cost {
     ...
     * @param costType cost type (base, vat, total)
     * @return double
     */
    protected double getIncomeAmount(final boolean isMcVatEnabled, final boolean isBudgetCurrency,
            final String costType) {
        double incomeAmount = this.amountIncome;
        if (isMcVatEnabled && isBudgetCurrency) {
            if (Constants.COST_TYPE_BASE.equals(costType)) {
                incomeAmount = this.amountIncomeBaseBudget;
            } else if (Constants.COST_TYPE_VAT.equals(costType)) {
     ...
     * @return double
     */
    protected double getExpenseAmount(final boolean isMcVatEnabled, final boolean isBudgetCurrency,
            final String costType) {
    ...
    /**
     * Get exchange rate for currency type.
     * 
     * @param isMcAndVatEnabled MC VAT enabled
     * @param currencyParams currency parameters
     * @return double
     */
    protected double getExchangeRate(final boolean isMcAndVatEnabled,
            final CurrencyUtilities currencyParams) {
        return getExchangeRateForDate(isMcAndVatEnabled, currencyParams, new Date());
    }
    
    /**
     * Get exchange rate for currency type and date.
     * 
     * @param isMcAndVatEnabled MC VAT enabled
     * @param currencyParams currency parameters
     ...
     * @return double
     */
    protected double getExchangeRateForDate(final boolean isMcAndVatEnabled,
            final CurrencyUtilities currencyParams, final Date date) {
        double exchangeRate = 1;
        if (isMcAndVatEnabled) {
            final String sourceCurrency =
                    getSourceCurrency(currencyParams.getCurrencyType().toString());
            exchangeRate = currencyParams.getConversionRateForDate(sourceCurrency, date);
        }
        return exchangeRate;
    }
     ...
     * @return asset id
     */
    protected String getAssetId(final String assetKey) {
        String assetId = null;
