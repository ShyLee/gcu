...
public class CostIndexingHelper {
    ...
    public void applyIndexOnDate(final Date dateIndexing, final Date systemDate) {
        ...
        final boolean blnIsLsIndexCost = this.leaseRecord.getInt(Constants.LS_COST_INDEX) == 1;
        final String landlordTenant = this.leaseRecord.getString(Constants.LS_LANDLORD_TENANT);
    ...
    private void applyNewIndex(final Date dateIndexing, final RecurringCost recurringCost,
        ...
        final boolean isVatExcludedForLease =
                this.leaseRecord.getInt(Constants.LS_VAT_EXCLUDE) == 1;
    ...
    private DataRecord getLeaseRecord(final String lsId) {
        final DataSource dsLease =
                DataSourceFactory.createDataSourceForFields(Constants.LS_TABLE, new String[] {
                        Constants.LS_ID, Constants.COST_INDEX, Constants.LANDLORD_TENANT,
                        Constants.VAT_EXCLUDE });
        dsLease.addRestriction(Restrictions.eq(Constants.LS_TABLE, Constants.LS_ID, lsId));
    ...
    private boolean findIndexValue(final String costIndexId, final Date dateIndexing) {
        ...
        final DataSource dsIndexValues =
                DataSourceFactory.createDataSourceForFields(Constants.COST_INDEX_VALUES,
                    new String[] { Constants.COST_INDEX_ID, Constants.DATE_INDEX_VALUE,
                            "index_value" });
        dsIndexValues
            .addRestriction(Restrictions.and(Restrictions.eq(Constants.COST_INDEX_VALUES,
                Constants.COST_INDEX_ID, costIndexId), Restrictions.lte(
                Constants.COST_INDEX_VALUES, Constants.DATE_INDEX_VALUE, dateIndexing)));
        dsIndexValues.addSort(Constants.COST_INDEX_VALUES, Constants.DATE_INDEX_VALUE,
        ...
        if (indexValues.size() > 0) {
            ...
            this.indexValueNew = recIndexValue.getDouble("cost_index_values.index_value");
            this.dateIndexValueNew = recIndexValue.getDate("cost_index_values.date_index_value");
            result = true;
        }
        return result;
    }
    ...
    private List<ScheduledCost> findScheduledCosts(final int recurringCostCode,
        ...
        final Restriction restriction =
                Restrictions.and(Restrictions.eq(Constants.SCHED_COST_TABLE,
                    Constants.COST_TRAN_RECUR_ID, recurringCostCode), Restrictions.gt(
                    Constants.SCHED_COST_TABLE, Constants.DATE_DUE, systemDate));
