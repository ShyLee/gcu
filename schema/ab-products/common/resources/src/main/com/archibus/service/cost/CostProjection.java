...
public class CostProjection {
    ...
    public List<CostPeriod> getPeriodsForAssetAndCostCategory(final String assetId,
        ...
        final Map<String, List<CostPeriod>> costCategoriesToPeriods =
                this.assetsToCostCategoriesToPeriods.get(assetId);
        if (costCategoriesToPeriods != null) {
            return costCategoriesToPeriods.get(costCategoryId);
        } else {
            return new ArrayList<CostPeriod>();
        }
    }
     ...
     */
    public void summarizeCostForAssetAndCostCategory(final String assetId, final Date endDate,
            final String costCategory) {
        final List<CostPeriod> periods =
                this.getPeriodsForAssetAndCostCategory(assetId, costCategory);
        if (periods != null) {
            BigDecimal costHolder = BigDecimal.ZERO;
            for (final CostPeriod period : periods) {
                if (period.getDateStart().before(endDate)) {
    ...
    /**
     * Get multiplication factor based on period.
     * 
     * @return integer
     */
    public int getMultiplicationFactor() {
        int factor = 1;
        if (Period.QUARTER.equals(this.period)) {
            factor = 3;
        } else if (Period.YEAR.equals(this.period)) {
            factor = 12;
        }
        return factor;
    }
    ...
    private List<CostPeriod> createPeriods() {
        ...
        while (c.getTime().before(this.dateEnd)) {
            ...
            } else if (this.period.equals(Period.DATE_RANGE)) {
                values.add(new CostPeriod(Period.DATE_RANGE, this.dateStart, this.dateEnd));
