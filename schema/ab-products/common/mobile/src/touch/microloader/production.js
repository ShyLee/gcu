 ...
 *
 * Modified to load the cached data from the SQLite database instead of local storage.
 * The browser localStorage limited the amount of data that could be saved to 5MB which is
 * not enough for the existing ARCHIBUS applications.
 */
(function(global) {
    ...
    // The Sencha microloader code has been customized to use an in memory data store instead
    // of localStorage. This allows more than 5MB of Sencha app code to be cached in the browser
    // The Sencha app code is stored in the client SQLite database.
    // On start up, the application code is loaded from the database into the in memory storage.
    // Changes to the application code are updated in the in memory storage.
    // When the app is launched the Common.util.AppCacheManager class saves the in memory data to
    // the SQLite database.
    ...
    function getDbConnection() {
        if(dbConnection === null) {
            dbConnection = window.openDatabase('ARCHIBUS', '1.0', 'ARCHIBUS Mobile Applications Database', 50*1024*1024);
        }
        return dbConnection;
    }

    function loadAppData(id, onCompleted, scope) {
        var db = getDbConnection(),
                key = '%' + id + '%';

        db.transaction(function(tx) {
            tx.executeSql('CREATE TABLE IF NOT EXISTS AppCache (key TEXT, value TEXT)', null, function(tx) {
                tx.executeSql('SELECT key, value FROM AppCache WHERE key LIKE ?', [key], function(tx, results) {
                    for(var i=0; i < results.rows.length; i++) {
                        var item = results.rows.item(i);
                        Abm.appCacheData.push({key: item.key,
                            value: item.value });
                    }
