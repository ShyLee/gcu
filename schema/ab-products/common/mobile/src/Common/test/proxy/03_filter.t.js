StartTest(function (t) {
    t.requireOk('Common.store.proxy.ProxyUtil','Common.util.Filter', function () {
        ...
        var filterProperties = Common.store.proxy.ProxyUtil.getFilterProperties(filter);
        ...
        t.is(filterProperties.isEqualNull, true, 'Filter isEqualNull property matches');

        // Create filter statement

        var filterStatement = ProxyUtil.getFilterRestriction([filter]);
        ...
        var filter2 = Ext.create('Common.util.Filter',
                { property: 'eq_id',
                    value: '1000',
                    conjunction: 'AND'
                });
