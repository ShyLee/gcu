 ...
 * @since 21.1
 * 
 * 01.08.2013 Added string parse fomat of YYYY/mm/dd HH:mm:ss.nnn to allow values to be parsed in mobile Safari.
 * 01.16.2013. Simplified the processTimeString function logic. JM
 */
...
Ext.define('Common.type.Time', {
	...
	/**
	 * Matches time strings in HH:mm or HH:mm:ss formats
	 */
	...
	applyValue : function(newValue, oldValue) {
		...
		if (timeValue === null) {
			...
			throw new Error('The input value cannot be converted to a Common.type.Time type.');
		}
	 ...
	 * @private
	 * @param {String}
	 *            timeString A string in the formats of HH:mm, HH:mm:ss or YYYY-MM-dd HH:mm:ss.nnn
	 * @return {Date}
	 * @throws {Error}
	 *             Throws an exception if the input value cannot be converted to a Common.type.Time type.
	 */
	processTimeString : function(timeString) {
		var dateParseResult, parsedDate, formatStrings = [ 'Y-m-d h:i:s.u', 'h:i', 'h:i:s' ], i;
		...
		if (this.dashesRegEx.test(timeString)) {
