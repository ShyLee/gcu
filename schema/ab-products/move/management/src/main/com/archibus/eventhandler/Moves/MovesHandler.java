...
public class MovesHandler extends EventHandlerBase {
    ...
    public void requestIndividualMove(final String mo_id) {
        ...
        // @translatable
        String text =
                "Thank you for submitting a move request. We will notify you via email upon the Approval, Issuing and Closing of this move request.";
        text = localizeString(context, text);
        
        text = this.newLine + text;
        
        // @translatable
        String textContact = "A move request has been submitted with you as the contact.";
        textContact = localizeString(context, textContact);
        
        textContact = this.newLine + textContact;
        
        String link = "\n\n" + getWebCentralPath(context) + "/ab-mo-examine-form.axvw";
        link += "?mo_id=" + mo_id;
        
        text = text + link;
        textContact = textContact + link;
        
        final String requestor = getEmailAddress(context, getRequestor(context, mo_id, ""));
        final String deptContact =
                getEmailAddress(context, getDepartmentContact(context, mo_id, ""));
        
        // @translatable
        String subject = "Requested move";
        ...
        if (notNull(deptContact).length() != 0) {
            sendEmail(context, textContact, this.prefix + subject, deptContact);
        }
    }
    ...
    public void requestGroupMove(final String project_id) {
        ...
        String link = "\n\n" + getWebCentralPath(context) + "/ab-mo-gp-examine-form.axvw";
        try {
            link += "?project_id=" + java.net.URLEncoder.encode(project_id, "UTF-8");
        ...
        // @translatable
        String subject = "Requested group move";
        subject = localizeString(context, subject);
        this.prefix = localizeString(context, this.prefix);
    ...
    public void routeIndividualMoveForApproval(final String mo_id, String apprv_mgr1,
        ...
        if (!routed) {
            ...
            message = localizeString(context, message);
            
            message = this.newLine + message + this.newLine;
            
            // @translatable
            String text2 = "Move [{0}]  routed for approval.";
            final Object[] args = new Object[] { mo_id.toString() };
            text2 = prepareMessage(context, text2, args);
            
            text2 = this.newLine + text2 + this.newLine;
            
            String link = getWebCentralPath(context) + "/ab-mo-approve-form.axvw";
            link += "?mo_id=" + mo_id;
            
            final String moveCoordinator =
            ...
            // @translatable
            String subject = "Approve request";
            subject = localizeString(context, subject);
            
            // @translatable
            String subject2 = "Move Routed For Approval";
            subject2 = localizeString(context, subject2);
            
            if (notNull(appr_mgr1_email).length() != 0) {
                sendEmail(context, message + link, this.prefix + subject, appr_mgr1_email);
            }
            
            if (notNull(appr_mgr2_email).length() != 0) {
                sendEmail(context, message + link, this.prefix + subject, appr_mgr2_email);
            }
            
            if (notNull(appr_mgr3_email).length() != 0) {
                sendEmail(context, message + link, this.prefix + subject, appr_mgr3_email);
            }
            
            if (notNull(moveCoordinator).length() != 0) {
                sendEmail(context, text2, this.prefix + subject2, moveCoordinator);
            }
            
            if (notNull(requestor).length() != 0) {
                sendEmail(context, text2, this.prefix + subject2, requestor);
            }
            
            if (notNull(deptContact).length() != 0) {
                sendEmail(context, text2, this.prefix + subject2, deptContact);
            }
        }
    }
    ...
    public void routeGroupMoveForApproval(final String project_id, String apprv_mgr1,
        ...
        // @translatable
        String text2 = "Move project [{0}] routed for approval.";
        final Object[] args = new Object[] { project_id };
        text2 = prepareMessage(context, text2, args);
        
        text2 = this.newLine + text2 + this.newLine;
        
        String link = getWebCentralPath(context) + "/ab-mo-gp-approve-form.axvw";
        try {
            link += "?project_id=" + java.net.URLEncoder.encode(project_id, "UTF-8");
        } catch (final UnsupportedEncodingException uee) {
            // write to log file
        ...
        // @translatable
        String subject = "Move Routed For Approval";
        subject = localizeString(context, subject);
        this.prefix = localizeString(context, this.prefix);
        
        if (notNull(projectManager).length() != 0) {
            sendEmail(context, text2, this.prefix + subject, projectManager);
        }
        
        if (notNull(requestor).length() != 0) {
            sendEmail(context, text2, this.prefix + subject, requestor);
        }
        
        if (notNull(deptContact).length() != 0) {
            sendEmail(context, text2, this.prefix + subject, deptContact);
        }
        
        // @translatable
        String subject2 = "Move approval required";
        subject2 = localizeString(context, subject2);
        
        if (notNull(appr_mgr1_email).length() != 0) {
            sendEmail(context, text + link, this.prefix + subject2, appr_mgr1_email);
        }
        
        if (notNull(appr_mgr2_email).length() != 0) {
            sendEmail(context, text + link, this.prefix + subject2, appr_mgr2_email);
        }
        
        if (notNull(appr_mgr3_email).length() != 0) {
            sendEmail(context, text + link, this.prefix + subject2, appr_mgr3_email);
        }
    }
    ...
    public void approveIndividualMove(final String mo_id, final String apprv_mgr1,
        ...
        // @translatable
        String text = "Move [{0}] has been approved.";
        final Object[] args = new Object[] { mo_id.toString() };
        ...
        final String deptContact =
                getEmailAddress(context, getDepartmentContact(context, mo_id, ""));
        final String moveCoordinator = getEmailAddress(context, getMoveCoordinator(context, mo_id));
        
        // @translatable
        String subject = "Approved move";
        subject = localizeString(context, subject);
        this.prefix = localizeString(context, this.prefix);
        ...
        if (notNull(moveCoordinator).length() != 0) {
            sendEmail(context, text, this.prefix + subject, moveCoordinator);
        }
    }
    ...
    public void approveGroupMove(final String project_id, final String apprv_mgr1,
        ...
        // @translatable
        String text = "Move has been approved for project: [{0}]";
        final Object[] args = new Object[] { project_id };
        text = prepareMessage(context, text, args);
        ...
        // @translatable
        String subject = "Approved group move";
        subject = localizeString(context, subject);
        this.prefix = localizeString(context, this.prefix);
    ...
    public void rejectIndividualMove(final String mo_id, final String apprv_mgr1,
        ...
        // @translatable
        String text = "This move has been rejected. Click the link below to view the move details";
        text = localizeString(context, text);
    ...
    public void rejectGroupMove(final String project_id, final String apprv_mgr1,
        ...
        // @translatable
        String text = "Move for project [{0}] has been rejected.";
        final Object[] args = new Object[] { project_id };
        text = prepareMessage(context, text, args);
    ...
    public void withdrawIndividualMove(final EventHandlerContext context) {
        ...
        // @translatable
        String text = "Move Request " + mo_id.toString() + " has been withdrawn.";
        final Object[] args = new Object[] { mo_id.toString() };
        text = prepareMessage(context, text, args);
        ...
        // @translatable
        String subject = "Requested move withdrawn";
        subject = localizeString(context, subject);
        this.prefix = localizeString(context, this.prefix);
    ...
    public void withdrawGroupMove(final EventHandlerContext context) {
        ...
        // @translatable
        String text = "Move Request [{0}] has been withdrawn.";
        final Object[] args = new Object[] { project_id };
        text = prepareMessage(context, text, args);
    ...
    public void issueIndividualMove(final String mo_id, final String requestor) {
        ...
        // @translatable
        String text = "Move [{0}] has been issued.";
        final Object[] args = new Object[] { mo_id.toString() };
        text = prepareMessage(context, text, args);
        ...
        if (notNull(requestor).length() != 0) {
            sendEmail(context, text, this.prefix + subject, requestorEmail);
        }
        ...
        for (final Iterator it = records2.iterator(); it.hasNext();) {
            ...
            if (notNull(emailAddress).length() != 0) {
                sendEmail(context, text, this.prefix + subject, emailAddress);
            }
        }
    ...
    public void issueGroupMove(final String project_id, final String requestor) {
        ...
        // @translatable
        String text = "Move for project [{0}] has been issued.";
        final Object[] args = new Object[] { project_id };
        text = prepareMessage(context, text, args);
        ...
        // @translatable
        String subject = "Issued move";
        subject = localizeString(context, subject);
        
        // @translatable
        final String subject2 = "Issue move";
        subject = localizeString(context, subject);
        this.prefix = localizeString(context, this.prefix);
        ...
        for (final Iterator it = records2.iterator(); it.hasNext();) {
            ...
            if (notNull(emailAddress).length() != 0) {
                sendEmail(context, text, this.prefix + subject2, emailAddress);
            }
        }
    ...
    /**
     * getMoveTypeProject helper method
     * 
     * @param context Description of the Parameter
     * @param project_id Description of the Parameter
     * @return The moveType value
    ...
    private String getMoveTypeProject(final EventHandlerContext context, final String project_id) {
        return (String) selectDbValue(context, "mo", "mo_type",
            "project_id=" + literal(context, project_id));
    }
    ...
    public void closeIndividualMove(final String mo_id) {
        ...
        // @translatable
        String text = "Move request closed for move: [{0}]";
        final Object[] args = new Object[] { mo_id.toString() };
        text = prepareMessage(context, text, args);
    ...
    public void closeGroupMove(final String project_id) {
        ...
        // @translatable
        String text = "Move closed for project: [{0}]";
        final Object[] args = new Object[] { project_id };
        text = prepareMessage(context, text, args);
    ...
    public void calculateHistoricalEmployeeHeadcount() {
        ...
        ds.addRestriction(Restrictions.and(
            Restrictions.eq("hist_em_count", "year", new String().valueOf(year)),
            Restrictions.eq("hist_em_count", "month", new String().valueOf(month))));
    ...
    public void updateAssociatedServiceRequestStatus(final String tableName, final String pkeyValue) {
        ...
        if (activityLogId != 0 && isValidActivityLogId(activityLogId)) {
            ...
            // if isStatusSame is true, call SeviceDesk WFR to update the status,else do nothing
            if (isStatusSame) {
                ...
                if (activityLogStatus != null) {
                    ...
                    if (activityLogStatus.equals("CANCELLED")
                            || activityLogStatus.equals("REJECTED")) {
                        final String useWorkspaceTransactions =
                                ContextStore
                                    .get()
                                    .getProject()
                                    .getActivityParameterManager()
                                    .getParameterValue(
                        ...
                        final DataRecord activityLogRecord =
                                activityLogDS.getRecord("activity_log.activity_log_id="
                                        + activityLogId);
    ...
    private void updateAssociatedRmpct(final String tableName, final String pkeyValue,
        ...
        for (final DataRecord moRecord : moRecords) {
            ...
            if (rmpctRecords.size() > 0) {
                ...
                final JSONObject assignment =
                        getInsertAssignmentObject(activityLogId, moRecord, null, isApproved);
                insertList.put(assignment);
            }
        }
    ...
    private JSONObject getInsertAssignmentObject(final int activityLogId,
            final DataRecord moRecord, final DataRecord rmpctRecord, final boolean isApproved) {
        ...
        moRecord.getDate("mo.date_to_perform");
        final JSONObject assignment = new JSONObject();
...
//        should be the same as the primary_em of the "from location" record that had the employee previously.     
        assignment.put("primary_em",assignment.get("primary_rm"));
