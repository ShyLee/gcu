...
public class MovePaginatedReportGenerator extends JobBase {
    ...
    private final Map<String, Map<String, String>> moData = new HashMap<String, Map<String, String>>() {
        {
    ...
    public MovePaginatedReportGenerator(String rptType, String projectId, String selectedId) {
    ...
    public void run() {
        ...
        if ((this.rptType.equals("group") && StringUtil.notNullOrEmpty(this.projectId))
            ...
            counter = 1;
            for (String crtKey : this.moKeys) {
                Map<String, String> crtData = this.moData.get(crtKey);
                String moType = crtData.get("type");
                boolean generate = false;
                String strRestriction = null;
                if (this.rptType.equals("group")) {
                    fileName = "move-management-group-moves-";
                    jobTitle = EventHandlerBase.localizeString(context.getEventHandlerContext(),
                        this.JOB_TITLE_GP, this.getClass().getName());
                    generate = isData(crtKey, this.projectId, this.moveId)
                            && !moType.equals("single");
                    if (moType.equals("group")) {
                        // information shown from project table
                        strRestriction = " project.project_id = '"
                                + SqlUtils.makeLiteralOrBlank(this.projectId) + "' "
                                + "AND project.project_type = 'Move' ";
                    } else {
                        // information shown from mo table
                        strRestriction = " mo.project_id = '"
                                + SqlUtils.makeLiteralOrBlank(this.projectId) + "' "
                                + "AND mo.mo_type = '" + SqlUtils.makeLiteralOrBlank(moType) + "'";
                    }
                } else if (this.rptType.equals("single")) {
                    String moveType = getIndividualMoveType(this.moveId);
                    jobTitle = EventHandlerBase.localizeString(context.getEventHandlerContext(),
                    ...
                    if (moType.equals("single")) {
                        ...
                        strRestriction = " activity_log.mo_id = '"
                                + SqlUtils.makeLiteralOrBlank(this.moveId) + "' "
                                + "AND activity_log.project_id IS NULL ";
                    } else {
                        // information shown from mo table
                        strRestriction = " mo.mo_id = '" + SqlUtils.makeLiteralOrBlank(this.moveId)
                                + "' " + "AND mo.mo_type = '" + SqlUtils.makeLiteralOrBlank(moType)
                                + "'";
                    }
                }
                if (generate) {
                    ...
                    Map<String, Object> moRestriction = new HashMap<String, Object>();
            ...
            String strRestriction = " AND mo_scenario_em.project_id = '"
                    + SqlUtils.makeLiteralOrBlank(this.projectId) + "' "
            ...
            com.archibus.ext.report.docx.Report report = new com.archibus.ext.report.docx.Report();
            report.setTitle(this.DOC_TITLE_SCENARIO + " : " + this.scenarioId + " ; "
                    + this.projectId);
        ...
        if (files.size() > 0) {
            ...
            String mainFileName = fileName + dateString + ".docx";
            ...
            for (String fullPath : files) {
                File crtFile = new File(fullPath);
                if (crtFile.exists()) {
                    crtFile.delete();
                }
            }
     ...
     */
    private boolean isData(String moKey, String projectId, String moId) {
     ...
     */
    private String getIndividualMoveType(String moveId) {
        DataSource ds = DataSourceFactory.createDataSource();
        ds.addTable("mo");
        ds.addField("mo", "mo_type");
        ds.addField("mo", "mo_id");
        ds.addRestriction(Restrictions.eq("mo", "mo_id", moveId));
        DataRecord rec = ds.getRecord();
