...
function getVpaParameterForTable(userName, tableName){
	...
	if(valueExists(vpaObject) && valueExists(vpaObject.restriction)){
		var vpaRestriction = vpaObject.restriction;
		var vpaBlId = "";
		var vpaSiteId = "";
		var relOp = "";
		if(valueExists(vpaRestriction["bl.bl_id"])){
			var blIdList = vpaRestriction["bl.bl_id"].clauses[0].value;
			vpaBlId =  "(" + tableName + "bl_id IN ('" + blIdList.join('\',\'') + "'))";
		}
		if(valueExists(vpaRestriction["site.site_id"])){
			var siteIdList = vpaRestriction["site.site_id"].clauses[0].value;
			vpaSiteId = "(" + tableName + "bl_id IN (select bl_id from bl where site_id IN ('" + siteIdList.join('\',\'') + "')))";
		}
		if(vpaBlId.length > 0){
			result += vpaBlId;
			relOp = " OR ";
