var localizationReadWriteFilesController = View.createController('localizationReadWriteFiles', {
    ...
    var subfolderPrompt = String(getMessage('subfolderPrompt'));
		...
		if ( !this.bAxvwXslJsp && !this.bJs && !this.bJava && !this.bResx && !this.bMnu){
	...
	abLocalizationReadWriteFilesConsolePanel_onReadFiles: function(){
		...
		if (this.errorMsg != ''){
			...
			try {			
				var jobId = Workflow.startJob('AbSystemAdministration-LocalizationRead-readTranslatableFiles', this.language, this.activityDir, this.bAxvwXslJsp, this.bJava, this.bJs, this.bResx, this.bMnu, this.bCore);
				var controller = this;
	...
	abLocalizationReadWriteFilesConsolePanel_onWriteFiles: function(){
		...
		if (this.errorMsg !=''){
			...
			try {
				var jobId = Workflow.startJob('AbSystemAdministration-LocalizationWrite-writeTranslatableFiles', this.language, this.activityDir, this.bAxvwXslJsp, this.bJs, this.bJava, this.bResx, this.bMnu, this.bCore, this.whenWriting, this.activityId, this.dbExtension, this.locale);
				var controller = this;
...
/*	
	abLocalizationReadWriteFilesConsolePanel_onShow: function(){		
		this.setCommonSelectedOptions();
		
		var restriction = new Ab.view.Restriction();
		var console = View.panels.get('abLocalizationReadWriteFilesConsolePanel');
		var language = console.getFieldValue('lang_files.language');
		...
		var activityDir = '';
		if (valueExistsNotEmpty(this.activityDir) ){
			  activityDir = this.activityDir;
		   	restriction.addClause('lang_files.filename', this.activityDir + '%', 'LIKE');
		}
			
		if (this.bAxvwXslJsp == true ){
			 restriction.addClause('lang_files.filename', activityDir + '%.axvw', 'LIKE', 'OR');
			 restriction.addClause('lang_files.filename', activityDir + '%.xsl', 'LIKE', 'OR');
		}

		if (this.bJs == true ){
			 restriction.addClause('lang_files.filename', activityDir + '%.js', 'LIKE', 'OR' );
			 restriction.addClause('lang_files.constant', '%||%', 'LIKE', 'AND');
		}		

		if (this.bJava == true ){
			 restriction.addClause('lang_files.filename', activityDir + 'com.%', 'LIKE', 'OR');
		}		
			
		var report = View.panels.get('abLocalizationReadWriteFiles_detailsPanel');
		report.refresh(restriction);
		report.show(true);		
	},	

	abLocalizationReadWriteFilesConsolePanel_onClear: function(){
		this.language= null;
		this.activityDir= null;
		this.bAxvwXslJsp= false;
		this.bJs= false;
		this.bJava= false;
		this.bCore= false;
		this.bAppend= null;
		...
		this.abLocalizationReadWriteFilesConsolePanel.setFieldValue('lang_files.language', '');
		$('abLocalizationReadWriteFiles_lang_files.activitiesToInclude.single').checked = false;
		$('abLocalizationReadWriteFiles_lang_files.activitiesToInclude.all').checked = false;
		...
		$('abLocalizationReadWriteFiles_lang_files.fileWriting.append').checked = false;
		$('abLocalizationReadWriteFiles_lang_files.fileWriting.create').checked = false;
		
		this.abLocalizationReadWriteFiles_detailsPanel.clear();
	}	
*/	
});
