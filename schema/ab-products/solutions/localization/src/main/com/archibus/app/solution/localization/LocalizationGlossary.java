...
public class LocalizationGlossary extends LocalizationBase {
     ...
     */
    public void readStringsIntoGlossary(String language, String stringsToInclude) {
        ...
        String sqlStatement = "UPDATE lang_glossary SET transfer_status = '"
                + UPDATE_STATUS_NO_CHANGE + "'";

        if (SqlUtils.isOracle()) {
            this.isOracle = true;
        }

        DataSource ds = DataSourceFactory.createDataSource().addTable("lang_glossary").addQuery(
            sqlStatement);
        ds.executeUpdate();
        ...
        if (this.stopRequested) {
            ...
            this.status.addProperty("affectedLangFiles", Integer.toString(getCountInserted(
                "lang_glossary", "language")));
    ...
    private void readLangFilesIntoGlossary() {
        ...
        String sqlStatement = "INSERT INTO lang_glossary ( string_english, language, string_trans, transfer_status )"
                + " SELECT DISTINCT RTRIM(LTRIM( string_english )), language, MAX(RTRIM(LTRIM( string_trans ))), "
                + " '"
                + UPDATE_STATUS_INSERTED
                + "' "
                + " FROM lang_files "
                + " WHERE language = '" + this.language + "' " + " AND string_english IS NOT NULL ";
        ...
        if (this.stringsToInclude.compareToIgnoreCase(LocalizationGlossary.NEED_TRANS) == 0) {
            if (this.isOracle) {
                sqlStatement += " AND (string_trans IS NULL OR LENGTH(RTRIM(LTRIM(string_trans))) > 0 ) ";
            } else {
                sqlStatement += " AND (string_trans IS NULL OR RTRIM(LTRIM(string_trans)) = '' ) ";
            }
        }

        sqlStatement += " AND (filename LIKE '%') " + " AND NOT EXISTS "
                + " (SELECT 1 FROM lang_glossary " + " WHERE language = '" + this.language + "' "
    ...
    private void readLangStringsIntoGlossary() {
        ...
        String sqlStatement = "INSERT INTO lang_glossary ( string_english, language, string_trans, string_type, transfer_status )"
                + " SELECT DISTINCT RTRIM(LTRIM( string_english )), language, MAX(RTRIM(LTRIM( string_trans ))), "
                + "'1', "
                + " '"
                + UPDATE_STATUS_INSERTED
    ...
    private void readLangEnumsIntoGlossary() {
        ...
        String sqlStatement = "INSERT INTO lang_glossary ( string_english, language, string_trans, string_type, transfer_status )"
                ...
                + "' "
                + " FROM lang_enum "
                + " WHERE language = '" + this.language + "' " + " AND enum_english IS NOT NULL ";
        ...
        if (this.stringsToInclude.compareToIgnoreCase(LocalizationGlossary.NEED_TRANS) == 0) {
            if (this.isOracle) {
                sqlStatement += " AND (enum_trans IS NULL OR LENGTH(RTRIM(LTRIM(enum_trans))) > 0 ) ";
            } else {
                sqlStatement += " AND (enum_trans IS NULL OR RTRIM(LTRIM(enum_trans)) = '' ) ";
            }
        }

        sqlStatement += " AND NOT EXISTS " + " (SELECT 1 FROM lang_glossary "
     ...
     * @param stringsToInclude String ("all", or "needTrans")
     */
    public void writeStringsFromGlossary(String language, String stringsToInclude) {
        this.status.setTotalNumber(100);
        ...
        if (this.stopRequested) {
            ...
            this.status.addProperty("affectedFiles", Integer.toString(getCountInsertedUpdated(
                "lang_files", "language")));
            this.status.addProperty("affectedStrings", Integer.toString(getCountInsertedUpdated(
                "lang_strings", "language")));
            this.status.addProperty("affectedEnums", Integer.toString(getCountInsertedUpdated(
                "lang_enum", "language")));
            this.status.setCode(JobStatus.JOB_COMPLETE);
        }
    ...
    private void writeToLangFiles() {
        ...
        String sqlStatement = "UPDATE lang_files "
                + " SET transfer_status = "
                + " '"
        ...
        DataSource ds = DataSourceFactory.createDataSource().addTable("lang_files").addQuery(
            sqlStatement);
        ds.executeUpdate();
    }
    ...
    private void writeToLangStrings() {
        ...
        String sqlStatement = "UPDATE lang_strings "
                + " SET transfer_status = "
                + " '"
                + UPDATE_STATUS_UPDATED
                + "',"
        ...
        // execute the the update
        DataSource ds = DataSourceFactory.createDataSource().addTable("lang_strings").addQuery(
            sqlStatement);
        ds.executeUpdate();
    }
    ...
    private void writeToLangEnums() {
        ...
        String sqlStatement = "UPDATE lang_enum "
                + " SET transfer_status = "
                + " '"
                + UPDATE_STATUS_UPDATED
                + "',"
        ...
        // execute the the update
        DataSource ds = DataSourceFactory.createDataSource().addTable("lang_enum").addQuery(
            sqlStatement);
        ds.executeUpdate();
    }
}