...
public class LocalizationWrite extends LocalizationBase {
    ...
    public void writeTranslatableFile(final String language, final String activityToInclude,
        ...
        if (fileType == LocalizationBase.FILE_TYPE_WRITE_VIEW) {
            restriction +=
                    " AND (filename LIKE '%.axvw' OR filename LIKE '%.xsl' OR filename LIKE '%.jsp' OR filename LIKE '%.acts') ";
        ...
        if (fileType == LocalizationBase.FILE_TYPE_WRITE_JS) {
            restriction += " AND (filename LIKE '%.js' AND constant LIKE '%||%')";
        }
        ...
        for (int j = 0; j < records.size(); j++) {
            ...
            if (fileType == LocalizationBase.FILE_TYPE_WRITE_JS) {
            ...
            else if ((fileType == LocalizationBase.FILE_TYPE_WRITE_VIEW || fileType == LocalizationBase.FILE_TYPE_WRITE_JAVA)) {
                ...
                if (whenWriting == LocalizationBase.APPEND_LANG_FILE) {
                    ...
                    // iterate through child elements of root
                    for (final Iterator<Element> i = langElementRoot.elementIterator("string"); i
                        .hasNext();) {
                        final Element localElement = i.next();
                        
                        final String key1 = localElement.attributeValue("key1");
                        final String key2 = localElement.attributeValue("key2");
                        final String key3 = localElement.attributeValue("key3");
                        final String value = localElement.attributeValue("value");
                        
                        // if already exist, delete the element
                        if (key1.compareToIgnoreCase(fileName) == 0
                                && key2.compareToIgnoreCase(constant) == 0
                                && key3.compareToIgnoreCase(stringEnglish) == 0
                                && value.compareToIgnoreCase(stringTrans) != 0) {
                            langElementRoot.remove(localElement);
                            // add = true;
                            break;
                        }
                    }
    ...
    public void writeTranslatableFiles(final String language, final String activityToInclude,
            ...
            final Boolean bMnu, final Boolean bCore, final int whenWriting,
        ...
        while (iterator.hasNext()) {
            ...
            if (type.equals(FILE_TYPE_WRITE_VIEW)) {
                sqlStatement +=
                        " ((filename LIKE '%.axvw' OR filename LIKE '%.xsl' OR filename LIKE '%.jsp' OR filename LIKE '%.acts') ";
    ...
    public void writeTranslatableStrings(final String language, final String tableName,
            final String fieldName, final int stringType) {
            ...
            if (stringType == LocalizationBase.TYPE_ML_HEADING) {
                ...
                sqlStatement =
                        "UPDATE afm_flds_lang SET ml_heading_"
                                ...
                                + "WHERE RTRIM(LTRIM(afm_flds.ml_heading)) = RTRIM(LTRIM(lang_strings.string_english)) AND "
                                + "afm_flds_lang.table_name = afm_flds.table_name AND "
                                + "afm_flds_lang.field_name = afm_flds.field_name AND "
    ...
    public void writeTranslatableTables(final String language, final Boolean bSchema,
        ...
        this.translatableFieldsByName =
                ContextStore.get().getProject().getTranslatableFieldsByName();
        ...
        while (it.hasNext()) {
            ...
            if (isExcludeTable) {
                ...
                if (bSchema && isSchemaTable || bPnav && isPNavTable || bOther && !isSchemaTable
                    ...
                    // lang_strings
                    this.writeTranslatableStrings(language, tableName, fieldName, fieldType);
                }
            }
        } // end while
