...
public class LocalizationBase extends JobBase {
    ...
    public Map<String, String> translatableFieldsByName;
     ...
     * @return boolean
     */
    public boolean checkSchemaTable(String tableName) {
        
        for (String element : this.schemaTable) {
     ...
     * @return boolean
     */
    public boolean checkPNavTable(String tableName) {
        
        for (String element : this.pNavTable) {
            if (tableName.compareToIgnoreCase(element) == 0) {
                return true;
            }
        }
     ...
     * @return boolean
     */
    public boolean checkExcludeTable(String tableName) {
        
        for (String element : this.excludeTable) {
            if (tableName.compareToIgnoreCase(element) == 0) {
                return true;
            }
        }
     ...
     */
    public int getCountInserted(String tableName, String pkFieldName) {
     ...
     * @return affectedNum Int Count of inserted or updated records in provided table
     */
    public int getCountInsertedUpdated(String tableName, String pkFieldName) {
     ...
     */
    public void setStatusToNoChange(String table) {
        String sql =
                "UPDATE " + table + " SET transfer_status = '" + UPDATE_STATUS_NO_CHANGE
                        + "' WHERE language='" + this.language + "'";
        DataSource ds = DataSourceFactory.createDataSource().addTable(table).addQuery(sql);
        ds.executeUpdate();
    }
