...
function toggleSolidFillColors()
{
	if(customSolidColor){
		...
		View.panels.get('chartStackedArea_ds').setSolidFillColors([]);
	} else {
		alert("set custom custom color.");
		View.panels.get('chartStackedArea_ds').setSolidFillColors(customFillColors);
	}
	View.panels.get('chartStackedArea_ds').refresh();
...
function toggleGradientFillColors()
{
	if(customGradientColor==0){
		...
		View.panels.get('chartStackedArea_ds').setGradientFillColors("linearGradient", customFillColors, customDesturatedFillColors, 0.5, 1.0);
		customGradientColor++;
	} else if(customGradientColor==1) {
		alert("set default linear gradient color.");
		View.panels.get('chartStackedArea_ds').setGradientFillColors("linearGradient", [], [], 1.0, 1.0);
		customGradientColor++;
	}else if(customGradientColor==2) {
		alert("set custom radical gradient color.");
		View.panels.get('chartStackedArea_ds').setGradientFillColors("radialGradient", customFillColors, customDesturatedFillColors, 1.0, 0.5);
		customGradientColor++;
	}else {
		alert("set default radical gradient color.");
		View.panels.get('chartStackedArea_ds').setGradientFillColors("radialGradient", [], [], 1.0, 1.0);
...
function toggleTooltips()
{
	...
	View.panels.get('chartStackedArea_ds').setDataTipFunction(showAllTooltips);
