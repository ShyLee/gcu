...
public class HotelingNotifyText extends EventHandlerBase {
    ...
    public Map<String, String> getEmailText(final List<DataRecord> bookingsEmployeesList,
            final List<DataRecord> bookingsVisitorsList,
        ...
        getEmailContentFromBookingList(bookingAction, bookingsEmployeesList, recurringRule,
            parameters, selectRecordDs, HotelingConstants.NOTIFY_LIST_TYPE_EMPLOYEE);
        getEmailContentFromBookingList(bookingAction, bookingsVisitorsList, recurringRule,
            parameters, selectRecordDs, HotelingConstants.NOTIFY_LIST_TYPE_VISITOR);
        getEmailContentFromBookingList(bookingAction, bookingsApprovesList, recurringRule,
    ...
    private void getEmailContentFromBookingList(final String bookingAction,
            final List<DataRecord> bookingList, final String recurringRule,
        ...
        if (bookingList != null) {
            if (HotelingConstants.BOOKING_ACTION_CREATE.equals(bookingAction)) {
                ...
                getEmailContentForCreate(bookingAction, bookingList, recurringRule, parameters,
                    selectRecordDs, listType, context);
                
            } else if (!HotelingConstants.NOTIFY_LIST_TYPE_APPROVER.equals(listType)) {
                getEmailContentForApprove(bookingAction, bookingList, parameters, listType);
            }
        }
    ...
    private void getEmailContentForApprove(final String bookingAction,
            final List<DataRecord> bookingList, final Map<String, String> parameters,
            final String listType) {
            ...
            for (int bookIndex = 0; bookIndex < bookingList.size(); bookIndex++) {
                ...
                if (parentPctId2 == parentPctId1) {
                    if (StringUtil.notNullOrEmpty(recurringRule1)) {
                        ...
                        // get recurring text
                        text = this.getRecurringText(index, booking, text, listType, bookingAction);
                        index++;
                        isRecurring = true;
                    } else {
                        pctIdSubject =
                        ...
                        text = this.getRegularText(booking, text, listType, bookingAction);
                        isRegular = true;
                    }
                }
            }
        ...
        for (int bookingListindex = 0; bookingListindex < bookingList.size(); bookingListindex++) {
            ...
            for (int bookIndex = 0; bookIndex < bookings.size(); bookIndex++) {
                ...
                if (StringUtil.notNullOrEmpty(recurringRule)) {
                    ...
                    // get recurring text
                    text = this.getRecurringText(bookIndex, booking, text, listType, bookingAction);
                } else {
                    pctIdSubject =
                            pctIdSubject
                                    + HotelingConstants.EMAIL_TEXT_COMMA
    ...
    private String getRecurringText(final int index, final DataRecord booking, final String text,
            final String type, final String bookingAction) {
        final EventHandlerContext context = ContextStore.get().getEventHandlerContext();
        ...
        if (index == 0) {
            ...
            newText = addParentBookingText(newText, context, parentPctId);
            newText = addEmployeeOrVisitorText(newText, type, context, emId, visitorId);
            ...
            newText = addLocationText(newText, context, blId, flId, rmId);
            
        }
        newText = addBookingText(booking, newText, type, bookingAction, true, context);
        return newText;
    }
    ...
    private String getRegularText(final DataRecord booking, final String text, final String type,
        ...
        newText = addEmployeeOrVisitorText(newText, type, context, emId, visitorId);
        newText = addLocationText(newText, context, blId, flId, rmId);
        newText = addBookingText(booking, newText, type, bookingAction, false, context);
    ...
    private String addBookingText(final DataRecord booking, final String text, final String type,
            final String bookingAction, final boolean isRecurring, final EventHandlerContext context) {
        final String rmpctTableName = HotelingConstants.RMPCT + HotelingConstants.DOT;
        final long pctId = booking.getInt(rmpctTableName + HotelingConstants.PCT_ID);
        ...
        newText =
                newText
                        ...
                        + EventHandlerBase.localizeString(context,
                            ...
                            HotelingConstants.class.getName()) + HotelingConstants.EMAIL_TEXT_COLON
                        + dayPart + HotelingConstants.EMAIL_TEXT_NEW_LINE;
        if (HotelingConstants.BOOKING_ACTION_APPROVED.equals(bookingAction)
                || HotelingConstants.BOOKING_ACTION_CREATE.equals(bookingAction)) {
            // @translatable
            newText =
                    newText
                            + EventHandlerBase.localizeString(context, HotelingConstants.STATUS,
                                HotelingConstants.class.getName())
        ...
        final String newText =
                text
                        + EventHandlerBase.localizeString(context,
                            ...
                            HotelingConstants.class.getName()) + HotelingConstants.EMAIL_TEXT_COLON
                        + blId + HotelingConstants.EMAIL_TEXT_CONCAT + flId
    ...
    private String addEmployeeOrVisitorText(final String text, final String type,
            final EventHandlerContext context, final String emId, final int visitorId) {
        String newText = "";
        ...
        final String newText =
                text
                        ...
                        + EventHandlerBase.localizeString(context,
                            ...
                            HotelingConstants.class.getName()) + HotelingConstants.EMAIL_TEXT_COLON
                        + parentPctId + HotelingConstants.EMAIL_TEXT_NEW_LINE;
        return newText;
    }
