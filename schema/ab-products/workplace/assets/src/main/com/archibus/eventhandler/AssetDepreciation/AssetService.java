...
public class AssetService extends JobBase {
    ...
    private void calculateDepreciation(String assetTable) {
        ...
        if (this.assetTable.equals("ta")) {
            ...
            this.assetFields = "ta_id, " + this.dateReceivedFld + "," + this.costPurchaseFld
            ...
            this.assetFields = "eq_id," + this.dateReceivedFld + "," + this.costPurchaseFld
                    + ",value_salvage , deprec_method , deprec_period";
            this.sqlLeaseRest = " AND ta_lease_id IS NULL";
        }

        this.assetFromWhereStmt = " FROM " + this.assetTable + ", property_type" + " WHERE "
                + this.assetTable + ".property_type = " + " property_type.property_type" + " AND "
                + this.dateReceivedFld + " IS NOT NULL" + " AND " + this.costPurchaseFld + " > 0.0"
                + this.sqlLeaseRest;
        ...
        DataSource dsRep = DataSourceFactory.createDataSource();
        ...
        List<DataRecord> recsRep = dsRep.getAllRecords();

        DataSource dsAsset = DataSourceFactory.createDataSource();
        ...
        dsAsset.addQuery("SELECT " + this.assetFields + this.assetFromWhereStmt,
            SqlExpressions.DIALECT_GENERIC);
        List<DataRecord> recsAsset = dsAsset.getAllRecords();

        for (DataRecord recRep : recsRep) {

            this.reportId = recRep.getString("dep_reports.report_id");
            this.lastDate = recRep.getDate("dep_reports.last_date");

            for (DataRecord recAsset : recsAsset) {
                ...
                this.monthsBetweenAssetReceiptAndReport = this.getDatesDiff(this.lastDate,
                    this.dateReceived);
                ...
                if (this.monthsBetweenAssetReceiptAndReport > 0
                    ...
                    this.costOriginal = recAsset.getDouble(this.assetTable + "."
                            + this.costPurchaseFld);
    ...
    private void createAssetDepreciationRecord() {
         ...
         */
            ...
            for (int i = 1; i <= this.monthsBetweenAssetReceiptAndReport; i++) {
                currentDep = (this.costOriginal - this.valSalvage) * (this.depLife - i + 1)
                        / sumMonthDigits;
                crtCostOriginal = crtCostOriginal - currentDep;
                accumDep += currentDep;
            }
    ...
    private int getDatesDiff(Date lastDate, Date receivedDate) {
        int monthDiff = 0;
        Calendar cLastDate = Calendar.getInstance();
        cLastDate.setTime(lastDate);

        Calendar cDateReceived = Calendar.getInstance();
