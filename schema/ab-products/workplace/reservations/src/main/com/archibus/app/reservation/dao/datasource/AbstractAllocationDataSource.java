...
public abstract class AbstractAllocationDataSource<T extends AbstractAllocation> extends
    ...
    /**
     * Remove all allocations by parent.
     * 
     ...
     *            <p>
     *            Justification: Case #4: Statements with DELETE FROM ... pattern.
     */
    @SuppressWarnings("PMD.AvoidUsingSql")
    public final void removeByParentId(final Integer parentId) {
        if (parentId == null) {
            return;
        }
        SqlUtils.executeUpdate(this.tableName, "DELETE FROM " + this.tableName
                + " WHERE exists (select res_id from reserve where reserve.res_id = "
                + this.tableName + ".res_id and reserve.res_parent = " + parentId + " ) ");
    }
