...
public class ReservationsCommonHandler extends ReservationsEventHandlerBase {
            ...
            if (!resId.equals("")) {
                ...
                if (((String) valuesToMail.get("locale")).equals("")) {
                    valuesToMail.put("locale",
                        getParentContextAttributeXPath(context, "/*/preferences/locale"));
                }
    ...
    // ---------------------------------------------------------------------------------------------
    // BEGIN approveReservation wfr
    // ---------------------------------------------------------------------------------------------
    /**
     * This rule gets the reservation type (room or resource) and the selected reservation
     * identifier, and approves it. Inputs: res_type Define the type of reservation that must be
     * evaluated. Outputs:
     * 
     * @param context Event handler context.
     */
    public void approveReservation(final String res_type, final String res_pk_list) {
        
        final String RULE_ID = "approveReservation";
        // this.log.info("Executing '"+ACTIVITY_ID+"-"+RULE_ID+"' .... ");
        final EventHandlerContext context = ContextStore.get().getEventHandlerContext();
        ...
        // this.log.info(ACTIVITY_ID + "-" + RULE_ID + "' [res_type input]: " + reservation_type);
        
        boolean allOk = false;
        String sql = "";
        
        // approveReservation rule error message
        final String errMessage =
                localizeMessage(context, ACTIVITY_ID, "APPROVERESERVATION_WFR",
                    "APPROVERESERVATIONERROR", null);
            ...
            if (!pkElements.isEmpty()) {
                ...
                for (final Iterator it = pkElements.iterator(); it.hasNext();) {
                    ...
                    if (reservation_type.equals("room")) {
                        ...
                        // For the room reservations, the rule must get the primary key value
                        // [rmres_id]
                        // from the parameter, and check for the identifier if the reservation can
                        // or not be approved
                        sql =
                                " SELECT rmres_id " + " FROM reserve_rm " + " WHERE rmres_id="
                                        + rmres_id + " AND status='Awaiting App.'";
                        // this.log.info(ACTIVITY_ID+"-"+RULE_ID+"[Select-1]: "+sql);
                        
                        final List listRecordsSelect1 = retrieveDbRecords(context, sql);
                        
                        // If record returned, then we can approve the room reservation,
                        // by executing the following query
                        if (!listRecordsSelect1.isEmpty()) {
                            
                            sql =
                                    " UPDATE reserve_rm " + " SET status='Confirmed' "
                                            + " WHERE rmres_id=" + rmres_id;
                            // this.log.info(ACTIVITY_ID+"-"+RULE_ID+"[Update-1]: " + sql);
                            
                            try {
                                
                                executeDbSql(context, sql, false);
                                
                                // check if the room reservation is a room and resource reservation,
                                // or if it's an only room reservation
                                sql =
                                        " SELECT res_id as selectedid, "
                                                + " (SELECT COUNT(*) "
                                                + " FROM reserve_rs "
                                                + " WHERE reserve_rs.res_id=reserve_rm.res_id) AS totalrs , "
                                                + " (SELECT COUNT(*) FROM reserve_rs"
                                                + " WHERE reserve_rs.res_id=reserve_rm.res_id AND "
                                                + " status='Awaiting App.') AS totalrs_forapproval "
                                                + " FROM reserve_rm WHERE rmres_id=" + rmres_id;
                                // this.log.info(ACTIVITY_ID+"-"+RULE_ID+"[Select-2]: "+sql);
                                
                                final List listRecordsSelect2 = retrieveDbRecords(context, sql);
                                
                                if (!listRecordsSelect2.isEmpty()) {
                                    
                                    final Map recordOfSql1 = (Map) listRecordsSelect2.get(0);
                                    final int selectedid =
                                            getIntegerValue(context, recordOfSql1.get("selectedid"))
                                                .intValue();
                                    final int totalrs =
                                            getIntegerValue(context, recordOfSql1.get("totalrs"))
                                                .intValue();
                                    final int totalrs_forapproval =
                                            getIntegerValue(context,
                                                recordOfSql1.get("totalrs_forapproval")).intValue();
                                    
                                    if ((totalrs == 0)
                                            || (totalrs != 0 && totalrs_forapproval == 0)) {
                                        
                                        sql =
                                                " UPDATE reserve " + " SET status='Confirmed' "
                                                        + " WHERE res_id=" + selectedid;
                                        // this.log.info(ACTIVITY_ID+"-"+RULE_ID+"[Update-2]: " +
                                        // sql);
                                ...
                                // kb# 3019197 Added by Keven 2008-8-20
                                sql =
                                        " UPDATE wr SET status='A', date_stat_chg="
                                                + formatSqlIsoToNativeDate(context,
                                                    "CurrentDateTime")
                                                + ","
                                                + " time_stat_chg="
                                                + formatSqlIsoToNativeTime(context,
                                                    "CurrentDateTime") + " WHERE rmres_id="
                                                + rmres_id + " AND status = 'R'";
                                // this.log.info(ACTIVITY_ID+"-"+RULE_ID+"[Update-3]: " +
                                // sql);
                    ...
                    if (reservation_type.equals("resource")) {
                        ...
                        // For the resource reservations, the rule must get the primary key value
                        // rsres_id
                        // from the parameter, and check for the identifier if the reservation
                        // can or not be approved
                        sql =
                                " SELECT rsres_id " + " FROM reserve_rs " + " WHERE rsres_id="
                                        + rsres_id + " AND status='Awaiting App.'";
                        
                        // this.log.info(ACTIVITY_ID+"-"+RULE_ID+"[Select-1]: "+sql);
                        
                        final List listRecordsSelect1 = retrieveDbRecords(context, sql);
                        
                        // If record returned, then we can approve the resource reservation,
                        // by executing the following query
                        if (!listRecordsSelect1.isEmpty()) {
                            
                            sql =
                                    " UPDATE reserve_rs " + " SET status='Confirmed' "
                            ...
                            try {
                                ...
                                // check if the resource reservation is a room and resource
                                // reservation,
                                // or if it's an only resource reservation
                                sql =
                                        " SELECT res_id as selectedid, "
                                                + " (SELECT COUNT(*) "
                                                + " FROM reserve_rm "
                                                + " WHERE reserve_rm.res_id=reserve_rs.res_id) AS totalrm, "
                                                + " (SELECT COUNT(*) FROM reserve_rm"
                                                + " WHERE reserve_rm.res_id=reserve_rs.res_id AND "
                                                + " status='Awaiting App.') AS totalrm_forapproval, "
                                                + " (SELECT COUNT(*) FROM reserve_rs res"
                                                + " WHERE res.res_id=reserve_rs.res_id AND "
                                                + " status='Awaiting App.') AS totalrs_forapproval "
                                                + " FROM reserve_rs WHERE rsres_id=" + rsres_id;
                                // this.log.info(ACTIVITY_ID+"-"+RULE_ID+"[Select-2]: "+sql);
                                ...
                                if (!listRecordsSelect2.isEmpty()) {
                                    ...
                                    final int totalrm =
                                            getIntegerValue(context, recordOfSql1.get("totalrm"))
                                                .intValue();
                                    final int totalrm_forapproval =
                                            getIntegerValue(context,
                                                recordOfSql1.get("totalrm_forapproval")).intValue();
                                    final int totalrs_forapproval =
                                            getIntegerValue(context,
                                                recordOfSql1.get("totalrs_forapproval")).intValue();
                                    
                                    // Update only if is ok this:
                                    if ((totalrm == 0 && totalrs_forapproval == 0)
                                            || (totalrm != 0 && totalrm_forapproval == 0 && totalrs_forapproval == 0)) {
    ...
    // ---------------------------------------------------------------------------------------------
    // END approveReservation wfr
    // ---------------------------------------------------------------------------------------------
    
    // ---------------------------------------------------------------------------------------------
    // BEGIN rejectReservation wfr
    // ---------------------------------------------------------------------------------------------
    /**
     * This rule gets the reservation type (room or resource) and the selected reservation
     * identifier, and rejects it. Inputs: res_type Define the type of reservation that must be
     * evaluated. Outputs:
     * 
     * @param context Event handler context.
     */
    public void rejectReservation(final String res_type, final String res_pk_list,
            final String comments) {
        
        final String RULE_ID = "rejectReservation";
        // this.log.info("Executing '"+ACTIVITY_ID+"-"+RULE_ID+"' .... ");
        ...
        // approveReservation rule error message
        final String errMessage =
                localizeMessage(context, ACTIVITY_ID, "REJECTRESERVATION_WFR",
                    "REJECTRESERVATIONERROR", null);
                ...
                for (final Iterator it = pkElements.iterator(); it.hasNext();) {
                    ...
                    if (reservation_type.equals("room")) {
                        ...
                        if (!listRecordsSelect1.isEmpty()) {
                            ...
                            sql =
                                    " UPDATE reserve_rm " + " SET " + " status='Rejected', "
                                            + " date_rejected= "
                                            + formatSqlIsoToNativeDate(context, "CurrentDateTime")
                                            // PC KB 3037852
                                            + ", " + " comments=" + literal(context, comments)
                                            + " WHERE rmres_id=" + rmres_id;
                            //this.log.info(ACTIVITY_ID + "-" + RULE_ID + "[Update-1]: " + sql);
                            ...
                            try {
                                ...
                                // Once we have rejected the room reservations, the system must
                                // reject too the general reservation, and all the resources
                                // reservation
                                sql =
                                        " SELECT res_id as selectedid " + " FROM reserve_rm "
                                                + " WHERE rmres_id=" + rmres_id;
                                // this.log.info(ACTIVITY_ID+"-"+RULE_ID+"[Select-2]: "+sql);
                                ...
                                if (!listRecordsSelect2.isEmpty()) {
                                    ...
                                    sql =
                                            " UPDATE reserve " + " SET status='Rejected' "
                                                    + " WHERE res_id=" + selectedid;
                                    // this.log.info(ACTIVITY_ID+"-"+RULE_ID+"[Update-2]: " + sql);
                                    ...
                                    try {
                                        ...
                                        try {
                                            ...
                                            // After rejecting the room, resources and general
                                            // reservation,
                                            // the system must cancel all the work requests
                                            // associated to these reservations
                                            sql =
                                                    " UPDATE wr "
                                                            + " SET status='Can', "
                                                            + " time_stat_chg = "
                                            ...
                                            // this.log.info(ACTIVITY_ID+"-"+RULE_ID+"[Update-4]: "
                                            // + sql);
                                            ...
                                            try {
                                                ...
                                                // this.log.info(ACTIVITY_ID+"-"+RULE_ID+"[Update-5]:
                                                // " + sql);
                                                ...
                                                try {
                                                    ...
                                                    handleError(context, ACTIVITY_ID + "-"
                        ...
                        // After rejecting the resource reservation, the system must notify the
                        // users that
                        // these reservations have been rejected
                        if (allOk) {
                            // Guo changed 2008-09-12 to remove all executeDbCommit(context)
                            // executeDbCommit(context);
                            // this.log.info("'"+ACTIVITY_ID+"-"+RULE_ID+"' do commit");
                            allOk = false;
                            ...
                            if (sendEmailNotifications != null
                                ...
                                // Check if it must send email cancel invitations, and the dates to
                                // cancel
                                sql =
                                        "SELECT attendees,date_start,res_parent,time_start FROM reserve "
                                                + " WHERE reserve.res_id = " + res_id;
                                // this.log.info("'"+ACTIVITY_ID+"-"+RULE_ID+"' [sql reject
                                // invitations]: "+sql);
                                final List recordsSqlreject = retrieveDbRecords(context, sql);
                                
                                if (!recordsSqlreject.isEmpty()) {
                                    final Map recordOfSqlreject = (Map) recordsSqlreject.get(0);
                                    final String sql_attendees =
                                            getString(recordOfSqlreject, "attendees");
                                    final String sql_date_start =
                                            getDateValue(context,
                                                recordOfSqlreject.get("date_start")).toString();
                                    final String sql_res_parent =
                                            getString(recordOfSqlreject, "res_parent");
                                    // this.log.info("'"+ACTIVITY_ID+"-"+RULE_ID+"' [attendees]:
                                    // "+sql_attendees);
                                    // this.log.info("'"+ACTIVITY_ID+"-"+RULE_ID+"' [date_start]:
                                    // "+sql_date_start);
                                    // this.log.info("'"+ACTIVITY_ID+"-"+RULE_ID+"' [time_start]:
                                    // "+sql_time_start);
                                    // this.log.info("'"+ACTIVITY_ID+"-"+RULE_ID+"' [res_parent]:
                                    // "+sql_res_parent);
                                    // If it must send cancel invitations
                                    if (!sql_attendees.equals("")) {
                                        // If it's a regular reservation
                                        if (sql_res_parent.equals("")) {
                                            context.addResponseParameter("res_id",
                                                Integer.toString(res_id));
                                        ...
                                        // If it's a recurring reservation
                                            ...
                                            context.addResponseParameter("res_parent",
                                                sql_res_parent);
                                            // User wants to reject only one date of the recurrence
                                        ...
                                        // Clear the error message parameter before calling the WFR
                                        context.addResponseParameter("message", "");
                                        this.sendEmailInvitations(context);
                                        if (context.parameterExists("message")) {
                                            if (!(errorMessage.length() == 0)) {
                                                errorMessage += "\n";
                                            }
                            ...
                            // String to store the error message from the notify and sendInvitations
                            // WFRs
                            if (!(errorMessage.length() == 0)) {
                    ...
                    if (reservation_type.equals("resource")) {
                        ...
                        if (!listRecordsSelect1.isEmpty()) {
                            ...
                            sql =
                                    " UPDATE reserve_rs " + " SET status='Rejected', "
                                            + " date_rejected= "
                                            + formatSqlIsoToNativeDate(context, "CurrentDateTime")
                                            // PC KB 3037852
                                            + ", " + " comments=" + literal(context, comments)
                            ...
                            try {
                                executeDbSql(context, sql, false);
                                // check what to do with the general reservation status
                                // Guo changed 2008-07-31 to solve KB3018884
                                sql =
                                        " SELECT res_id as selectedid, "
                                                + " (SELECT COUNT(*) FROM reserve_rm "
                                                + " WHERE res_id=a.res_id "
                                                + " AND status<>'Rejected' AND status<>'Cancelled') AS totalrm_notrejectedcancel, "
                                                + " (SELECT COUNT(*) FROM reserve_rm "
                                                + " WHERE res_id=a.res_id "
                                                + " AND status='Awaiting App.') AS totalrm_forapproval, "
                                                + " (SELECT COUNT(*) FROM reserve_rm "
                                                + " WHERE res_id=a.res_id "
                                                + " AND status='Confirmed') AS totalrm_confirmed, "
                                                + " (SELECT COUNT(*) FROM reserve_rs "
                                                + " WHERE res_id=a.res_id "
                                                + " AND status<>'Rejected' AND status<>'Cancelled') AS  totalrs_notrejectedcancel, "
                                                + " (SELECT COUNT(*) FROM reserve_rs "
                                                + " WHERE res_id=a.res_id "
                                                + " AND status='Awaiting App.') AS totalrs_forapproval, "
                                                + " (SELECT COUNT(*) FROM reserve_rs "
                                                + " WHERE res_id=a.res_id "
                                                + " AND status='Confirmed') AS totalrs_confirmed "
                                                + " FROM reserve_rs a WHERE rsres_id=" + rsres_id;
                                // this.log.info(ACTIVITY_ID+"-"+RULE_ID+"[Select-2]: "+sql);
                                ...
                                if (!listRecordsSelect2.isEmpty()) {
                                    ...
                                    final int selectedid =
                                            getIntegerValue(context, recordOfSql1.get("selectedid"))
                                                .intValue();
                                    res_id = selectedid;
                                    final int totalrm_notrejectedcancel =
                                            getIntegerValue(context,
                                                recordOfSql1.get("totalrm_notrejectedcancel"))
                                                .intValue();
                                    final int totalrm_forapproval =
                                            getIntegerValue(context,
                                                recordOfSql1.get("totalrm_forapproval")).intValue();
                                    final int totalrm_confirmed =
                                            getIntegerValue(context,
                                                recordOfSql1.get("totalrm_confirmed")).intValue();
                                    final int totalrs_notrejectedcancel =
                                            getIntegerValue(context,
                                                recordOfSql1.get("totalrs_notrejectedcancel"))
                                                .intValue();
                                    final int totalrs_forapproval =
                                            getIntegerValue(context,
                                                recordOfSql1.get("totalrs_forapproval")).intValue();
                                    final int totalrs_confirmed =
                                            getIntegerValue(context,
                                                recordOfSql1.get("totalrs_confirmed")).intValue();
                                    // this.log.info(ACTIVITY_ID + "-" + RULE_ID + "Values: " +
                                    // Integer.toString(totalrm_notrejected) + " " +
                                    // Integer.toString(totalrm_forapproval) + " " +
                                    // Integer.toString(totalrm_confirmed) + " " +
                                    // Integer.toString(totalrs_notrejected) + " " +
                                    // Integer.toString(totalrs_forapproval));
                                    
                                    if ((totalrm_forapproval > 0) || (totalrs_forapproval > 0)) {
                                    } else if ((totalrs_notrejectedcancel == 0)
                                            && (totalrm_notrejectedcancel == 0)) {
                                        sql =
                                                " UPDATE reserve " + " SET status='Rejected' "
                                                        + " WHERE res_id=" + selectedid;
                                        // this.log.info(ACTIVITY_ID+"-"+RULE_ID+"[Update-2]: " +
                                    ...
                                    } else if ((totalrs_forapproval == 0 && totalrs_confirmed > 0)
                                            || (totalrs_forapproval == 0 && totalrm_confirmed > 0)) {
                                        sql =
                                                " UPDATE reserve " + " SET status='Confirmed' "
                                                        + " WHERE res_id=" + selectedid;
                                        // this.log.info(ACTIVITY_ID + "-" + RULE_ID + "[Update-3]:
                                ...
                                // After rejecting the room, resources and general reservation,
                                // the system must cancel all the work requests associated to these
                                // reservations
                                sql =
                                        " UPDATE wr "
                                                + " SET status='Can', "
                                                ...
                                                + formatSqlIsoToNativeDate(context,
                                                    "CurrentDateTime") + " WHERE rsres_id="
                                                + rsres_id + " AND status IN ('R','Rev','A','AA')";
                                // this.log.info(ACTIVITY_ID+"-"+RULE_ID+"[Update-3]: " + sql);
                                ...
                                try {
                                    ...
                                    sql =
                                            " UPDATE wr "
                                                    ...
                                                    + rsres_id
                                                    + " AND status IN ('I','HP','HA','HL')";
                                    // this.log.info(ACTIVITY_ID+"-"+RULE_ID+"[Update-4]: " + sql);
            ...
            if (!listParamValue.isEmpty()) {
                ...
                sql =
                        "INSERT INTO hreserve_rm (res_id, rmres_id, date_start, time_start, time_end, "
                                ...
                                + " recurring_order, comments, 'Closed' as status, guests_internal, guests_external "
                                + " FROM reserve_rm "
                                + " WHERE "
                                + currentDate
                                + " >= "
                ...
                sql =
                        " INSERT INTO hreserve_rs (res_id, rsres_id, date_start, time_start, time_end,"
                                ...
                                + " recurring_order, comments, 'Closed' as status "
                                + " FROM reserve_rs "
                                + " WHERE "
                                + currentDate
                                + " >= "
            ...
            if (!resId.equals("")) {
                ...
                if (((invitation_type.equals("new")) && (!parentId.equals("0")))
                    ...
                    for (final Iterator it = listRoom.iterator(); it.hasNext();) {
                        final TreeMap rooms = (TreeMap) it.next();
                        body +=
                                messages.get("BODY2") + " " + rooms.get("reserve_rm.date_start")
                                        + " ";
                    ...
                    if (invitation_type.equals("new") || invitation_type.equals("cancel")) {
                        ...
                        if (invitation_type.equals("new") && !parentId.equals("0")) {
                            ...
                            if (jsReservation.getString("recur_type").equals("day")) {
                                ...
                                parametersValues.put("rruleInternal", (res_recur_val1.get(0)));
                                parametersValues.put("WKST", "SU");
                            } else if (jsReservation.getString("recur_type").equals("week")) {
                                String selected_days = "";
                                if (res_recur_val1.get(0).toString() != "null") {
                                    selected_days += "SU,";
                                }
                                if (res_recur_val1.get(1).toString() != "null") {
                                    selected_days += "MO,";
                                }
                                if (res_recur_val1.get(2).toString() != "null") {
                                    selected_days += "TU,";
                                }
                                if (res_recur_val1.get(3).toString() != "null") {
                                    selected_days += "WE,";
                                }
                                if (res_recur_val1.get(4).toString() != "null") {
                                    selected_days += "TH,";
                                }
                                if (res_recur_val1.get(5).toString() != "null") {
                                    selected_days += "FR,";
                                }
                                if (res_recur_val1.get(6).toString() != "null") {
                                    selected_days += "SA,";
                                }
    ...
    // ---------------------------------------------------------------------------------------------
    // BEGIN checkRoomAndResourcesApproval wfr
    // ---------------------------------------------------------------------------------------------
    /**
     ...
     * Inputs: context context (EventHandlerContext); Outputs:
     * 
     * @param context Event handler context.
     */
    public void checkRoomAndResourcesApproval(final EventHandlerContext context) {
        
        final String RULE_ID = "checkRoomAndResourcesApproval";
        // this.log.info("Executing '" + ACTIVITY_ID + "-" + RULE_ID + "' .... ");
        
        // checkRoomAndResourcesApproval rule error message
        final String errMessage =
                localizeMessage(context, ACTIVITY_ID, "CHECKROOMANDRESOURCESAPPROVAL_WFR",
                    "CHECKROOMANDRESOURCESAPPROVALERROR", null);
        
        // checkRoomAndResourcesApproval rule message to add as comment while rejecting a
        // reservation
        ...
        try {
            ...
            int actionApprovalExpired;
            String userApprovalExpired = "";
            String reservationRes_id = "";
            String reservationRmRes_id = "";
            String reservationRsRes_id = "";
            String reservationRecord = "";
            
            // Check the room reservations in status 'Awaiting App.' where the time to approve
            // expired (approve_days)
            sql =
                    " SELECT action_approval_expired, user_approval_expired, rmres_id, res_id "
                            + " FROM reserve_rm r, rm_arrange a "
                            + " WHERE r.status='Awaiting App.' "
                            + " AND r.bl_id=a.bl_id AND r.fl_id=a.fl_id AND r.rm_id=a.rm_id AND r.config_id=a.config_id "
                            + " AND r.rm_arrange_type_id=a.rm_arrange_type_id AND "
                            + formatSqlDaysBetween(context, currentDate, "r.date_start")
                            + " < a.approve_days";
            
            final List recordsRmReservation = retrieveDbRecords(context, sql);
            
            if (!recordsRmReservation.isEmpty()) {
                
                for (final Iterator it = recordsRmReservation.iterator(); it.hasNext();) {
                    final Map record = (Map) it.next();
                    
                    actionApprovalExpired =
                            getIntegerValue(context, record.get("action_approval_expired"))
                                .intValue();
                    userApprovalExpired = getString(record, "user_approval_expired");
                    reservationRes_id = getIntegerValue(context, record.get("res_id")).toString();
                    reservationRmRes_id =
                            getIntegerValue(context, record.get("rmres_id")).toString();
                    
                    switch (actionApprovalExpired) {
                    // If the system must automatically approve the room reservation
                        case 0:
                            // sets in context the res_type and res_pk_list params to next wfr to
                            // call
                            reservationRecord =
                                    "<records><record reserve_rm.rmres_id='" + reservationRmRes_id
                                            + "'><keys reserve_rm.rmres_id='" + reservationRmRes_id
                                            + "'/></record></records>";
                            context.addResponseParameter("res_type", "room");
                            context.addResponseParameter("res_pk_list", reservationRecord);
                            // BEGIN: approve the room reservation
                            this.approveReservation("room", reservationRecord);
                            break;
                        // If the system must automatically reject the room reservation
                        case 1:
                            // sets in context the res_type and res_pk_list params to next wfr to
                            // call
                            reservationRecord =
                                    "<record reserve_rm.rmres_id='" + reservationRmRes_id
                                            + "'><keys reserve_rm.rmres_id='" + reservationRmRes_id
                                            + "'/></record>";
                            context.addResponseParameter("res_type", "room");
                            context.addResponseParameter("comments", rejectedMessage);
                            context.addResponseParameter("res_pk_list", reservationRecord);
                            // BEGIN: reject the room reservation
                            this.rejectReservation("room", reservationRecord, rejectedMessage);
                            break;
                        // If the system must automatically notify by email to the defined user
                        case 2:
                            // sets in context the res_type and res_pk_list params to next wfr to
                            // call
                            context.addResponseParameter("res_id", reservationRes_id);
                            context.addResponseParameter("rmrsres_id", reservationRmRes_id);
                            context.addResponseParameter("user_to_notify", userApprovalExpired);
                            // BEGIN: notify the approver defined
                            this.notifyApprover(context, "room");
                            break;
                    }
                }
            }
            
            // Check the resource reservations in status 'Awaiting App.' where the time to approve
            // expired (approve_days)
            sql =
                    " SELECT action_approval_expired, user_approval_expired, rsres_id, res_id "
                            + " FROM reserve_rs r, resources e "
                            + " WHERE r.status='Awaiting App.' "
                            + " AND r.resource_id=e.resource_id " + " AND "
                            + formatSqlDaysBetween(context, currentDate, "r.date_start")
                            + " < e.approve_days";
            
            final List recordsRsReservation = retrieveDbRecords(context, sql);
            
            if (!recordsRsReservation.isEmpty()) {
                
                for (final Iterator it = recordsRsReservation.iterator(); it.hasNext();) {
                    final Map record = (Map) it.next();
                    ...
                    reservationRes_id = getIntegerValue(context, record.get("res_id")).toString();
                    reservationRsRes_id =
                            getIntegerValue(context, record.get("rsres_id")).toString();
                    
                    switch (actionApprovalExpired) {
                    // If the system must automatically approve the resource reservation
                        case 0:
                            // sets in context the res_type and res_pk_list params to next wfr to
                            // call
                            reservationRecord =
                                    "<records><record reserve_rs.rsres_id='" + reservationRsRes_id
                                            + "'><keys reserve_rs.rsres_id='" + reservationRsRes_id
                                            + "'/></record></records>";
                            context.addResponseParameter("res_type", "resource");
                            context.addResponseParameter("res_pk_list", reservationRecord);
                            // BEGIN: approve the resource reservation
                            this.approveReservation("resource", reservationRecord);
                            break;
                        // If the system must automatically reject the room reservation
                        case 1:
                            // sets in context the res_type and res_pk_list params to next wfr to
                            // call
                            reservationRecord =
                                    "<record reserve_rs.rsres_id='" + reservationRsRes_id
                                            + "'><keys reserve_rs.rsres_id='" + reservationRsRes_id
                                            + "'/></record>";
                            context.addResponseParameter("res_type", "resource");
                            context.addResponseParameter("comments", rejectedMessage);
                            context.addResponseParameter("res_pk_list", reservationRecord);
                            // BEGIN: reject the resource reservation
                            this.rejectReservation("resource", reservationRecord, rejectedMessage);
                            break;
                        // If the system must automatically notify by email to the defined user
                        case 2:
                            // sets in context the res_type and res_pk_list params to next wfr to
                            // call
                            context.addResponseParameter("res_id", reservationRes_id);
                            context.addResponseParameter("rmrsres_id", reservationRsRes_id);
                            context.addResponseParameter("user_to_notify", userApprovalExpired);
                            // BEGIN: notify the approver defined
                            this.notifyApprover(context, "resource");
                            break;
                    }
                }
            }
    ...
    // ---------------------------------------------------------------------------------------------
    // END checkRoomAndResourcesApproval wfr
    // ---------------------------------------------------------------------------------------------
