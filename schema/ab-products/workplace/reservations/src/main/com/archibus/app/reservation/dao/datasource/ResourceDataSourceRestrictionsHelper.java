...
public final class ResourceDataSourceRestrictionsHelper {
                ...
                for (final RoomAllocation roomAllocation : roomAllocations) {
                    ...
                    dataSource
                        .addRestriction(Restrictions
                            .sql(" resources.bl_id = ${parameters['blId']} "
     ...
     * 
     */
    public static void addNatureRestriction(final DataSource dataSource, final ResourceNature nature) {
        if (nature != null) {
            dataSource.addParameter("nature", nature.toString(), DataSource.DATA_TYPE_TEXT);
            dataSource
                .addRestriction(Restrictions
                    .sql(" EXISTS (select resource_std.resource_std from resource_std where resources.resource_std "
        ...
        if (reservation instanceof RoomReservation) {
            ...
            for (final RoomAllocation roomAllocation : roomAllocations) {
                dataSource.addParameter(Constants.BL_ID_PARAMETER, roomAllocation.getBlId(),
                    DataSource.DATA_TYPE_TEXT);
                dataSource
                    .addParameter("flId", roomAllocation.getFlId(), DataSource.DATA_TYPE_TEXT);
                dataSource
                    .addParameter("rmId", roomAllocation.getRmId(), DataSource.DATA_TYPE_TEXT);
                dataSource.addParameter("configId", roomAllocation.getConfigId(),
                    DataSource.DATA_TYPE_TEXT);
                dataSource.addParameter("arrangeTypeId", roomAllocation.getArrangeTypeId(),
                    DataSource.DATA_TYPE_TEXT);
                
                dataSource
                    .addRestriction(Restrictions
                        .sql(" resources.resource_std NOT IN (select rm_arrange.res_stds_not_allowed from rm_arrange "
                                + "where bl_id = ${parameters['blId']} and fl_id = ${parameters['flId']} and rm_id = ${parameters['rmId']} "
                                + " and config_id = ${parameters['configId']} and arrange_type_id = ${parameters['arrangeTypeId']} ) "));
            }
        }
    }
