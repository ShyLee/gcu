...
View.createController('showMap', {
	...
	switchMapLayer: function(item) {
		View.controllers.get('showMap').map.map.switchMapLayer(item.text);
	},
	...
	mapPanel_onShowMap: function(str, level) {
		...
		if( blMarkerProperty == null ){
			blMarkerProperty = new Ab.flash.ArcGISMarkerProperty('bl_ds', ['bl.lat', 'bl.lon'],'bl.bl_id',['bl.address1', 'bl.city_id', 'bl.state_id', 'bl.ctry_id', 'bl.use1']);		
			this.map.updateDataSourceMarkerPropertyPair('bl_ds', blMarkerProperty);
		}
