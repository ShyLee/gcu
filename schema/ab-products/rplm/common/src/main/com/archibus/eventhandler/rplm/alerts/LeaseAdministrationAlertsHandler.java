...
public class LeaseAdministrationAlertsHandler extends EventHandlerBase {
	...
	public void generateAlerts(EventHandlerContext context) throws ExceptionBase {
		...
		DataSource ds0 = DataSourceFactory.createDataSource();
		...
		//DataSource ds0 = DataSourceFactory.loadDataSourceFromFile("ab-rplm-lsadmin-alerts-service.axvw", "alertsDs");
		List<DataRecord> alerts = ds0.getRecords();

		PortfolioAlert alert = null;
		for (Iterator<DataRecord> it = alerts.iterator(); it.hasNext(); ) {
            DataRecord record = (DataRecord) it.next();
	...
	class PortfolioAlert {
		...
		public PortfolioAlert(DataRecord alertRecord) {
            ...
            DataSource dsOp = DataSourceFactory.createDataSource();
		...
		public void setNotifyList() {
			...
			String sqlQuery0 = "SELECT DISTINCT email,locale FROM afm_users "
		...
		public void sendAlert(EventHandlerContext context) {
			if (!this.notifyList.isEmpty()) {
				...
				values.put("ls_landlord_tenant", notNull(this.alertRecord.getValue("ls.landlord_tenant")));
				values.put("ls_lease_sublease", notNull(this.alertRecord.getValue("ls.lease_sublease")));
				...
				values.put("ls_alert_definition_alert_type", notNull(this.alertRecord.getValue("ls_alert_definition.alert_type")));
				values.put("ls_alert_definition_color", notNull(this.alertRecord.getValue("ls_alert_definition.color")));

				String alert_type = (String) values.get("ls_alert_definition_alert_type");
				
				Iterator<DataRecord> it = this.notifyList.iterator();
		        while (it.hasNext()) {
		            DataRecord record = (DataRecord) it.next();
		            rcpt_to = (String) record.getValue("afm_users.email");
		            String afmUsersLocale = (String) record.getValue("afm_users.locale");

	                String[] args = getArgs(context, values, alert_type, afmUsersLocale);
		            
		            if (alert_type.equals("Lease Due Date")) {
	                    subject = prepareMessage(context, RPLM_ACTIVITY_ID, "LS_ALERTS_WFR", "LS_ALERT_TITLE", afmUsersLocale, args);
	                    body = prepareMessage(context, RPLM_ACTIVITY_ID, "LS_ALERTS_WFR", "LS_ALERT_TEXT", afmUsersLocale, args);
		            } 
	                else if (alert_type.equals("Option Due Date")) {
	                    subject = prepareMessage(context, RPLM_ACTIVITY_ID, "LS_ALERTS_WFR", "OP_ALERT_TITLE", afmUsersLocale, args);
	                    body = prepareMessage(context, RPLM_ACTIVITY_ID, "LS_ALERTS_WFR", "OP_ALERT_TEXT", afmUsersLocale, args);
	                }
		            if (StringUtil.notNullOrEmpty(rcpt_to)) {
						sendEmail(body, LeaseAdministrationAlertsHandler.this.mail_from, LeaseAdministrationAlertsHandler.this.smtp_host, LeaseAdministrationAlertsHandler.this.smtp_port, subject, rcpt_to, null,
						        null, null, null, null, CONTENT_TYPE, AB_RPLM_LEASE_ADMINITSRATION);
		            }
		        }
			}
		}

        private String[] getArgs(EventHandlerContext context, Map<String, Object> values, String alert_type, String afmUsersLocale) {
            String[] args = null;
            
            // uses un-documented core API
            Locale locale = XmlImpl.stringToLocale(afmUsersLocale);

            if (alert_type.equals("Lease Due Date")) {
            	args = new String[] { 
            			(String) values.get("ls_ls_id"), 
            			(String) values.get("ls_date_end"), 
            			"\n\n" + this.leaseRecord.findField("ls.ls_id").getFieldDef().multiLineHeadingsToString(locale) +": " + values.get("ls_ls_id") +"\n" + 
            			this.leaseRecord.findField("ls.description").getFieldDef().multiLineHeadingsToString(locale) +": " + values.get("ls_description") +"\n" + 
            			this.leaseRecord.findField("ls.landlord_tenant").getFieldDef().multiLineHeadingsToString(locale).toString().replaceAll("\\s", "") +": " + values.get("ls_landlord_tenant") +"\n" + 
            			this.leaseRecord.findField("ls.lease_sublease").getFieldDef().multiLineHeadingsToString(locale).toString().replaceAll("\\s", "") +": " + values.get("ls_lease_sublease") +"\n" + 
            			this.leaseRecord.findField("ls.date_start").getFieldDef().multiLineHeadingsToString(locale) +": " + values.get("ls_date_start") +"\n" + 
            			this.leaseRecord.findField("ls.bl_id").getFieldDef().multiLineHeadingsToString(locale) +": " + values.get("ls_bl_id") +"\n" + 
            			this.leaseRecord.findField("ls.pr_id").getFieldDef().multiLineHeadingsToString(locale) +": " + values.get("ls_pr_id") +"\n" + 
            			this.leaseRecord.findField("ls.comments").getFieldDef().multiLineHeadingsToString(locale) +": " + values.get("ls_comments") +"\n\n", 
            			"\n" +LeaseAdministrationAlertsHandler.this.webcentral_path +"/" + 
            			notNull(getActivityParameterString(context, RPLM_ACTIVITY_ID, "LS_ALERTS_VIEW")) + 
            			"?ls.ls_id=" +values.get("ls_ls_id"), 
            			(String) values.get("ls_alert_definition_color")};
            } 
            else if (alert_type.equals("Option Due Date")) {
            	args = new String[] { 
            			(String) values.get("ls_ls_id") + "-" +values.get("ls_op_id"), 
            			(String) values.get("ls_date_end"), 
            			"\n\n" +this.optionRecord.findField("op.ls_id").getFieldDef().multiLineHeadingsToString(locale) +": " + values.get("ls_ls_id") +"\n" + 
            			this.optionRecord.findField("op.op_id").getFieldDef().multiLineHeadingsToString(locale) +": " + values.get("ls_op_id") +"\n" + 
            			this.optionRecord.findField("op.op_type").getFieldDef().multiLineHeadingsToString(locale) +": " + values.get("ls_op_type") +"\n" + 
            			this.optionRecord.findField("op.description").getFieldDef().multiLineHeadingsToString(locale) +": " + values.get("ls_description") +"\n" + 
            			this.optionRecord.findField("op.comments").getFieldDef().multiLineHeadingsToString(locale) +": " + values.get("ls_comments") +"\n\n", 
            			"\n" +LeaseAdministrationAlertsHandler.this.webcentral_path +"/" + 
            			notNull(getActivityParameterString(context, RPLM_ACTIVITY_ID, "OP_ALERTS_VIEW")) + 
            			"?ls.ls_id=" +values.get("ls_ls_id") +"&ls.op_id=" +values.get("ls_op_id"), 
            			(String) values.get("ls_alert_definition_color")};
            }
            return args;
        }
