var prepaymentController = View.createController('prepaymentCtrl',{
	...
	formInvoice_onSave: function(){
		var paymentId = parseInt(this.formPayment.getFieldValue('invoice_payment.payment_id'));
		if(!valueExistsNotEmpty(paymentId)){
		...
		// apply payment
		try{
			var result = Workflow.callMethod('AbRPLMChargebackInvoice-InvoiceService-applyPaymentFromPrepayment', this.invoice_id, paymentId, prepaymentAmount);
			var restriction = new Ab.view.Restriction();
			restriction.addClause('invoice.invoice_id', this.invoice_id, '=');
			this.formInvoice.refresh(restriction);
			restriction = new Ab.view.Restriction();
