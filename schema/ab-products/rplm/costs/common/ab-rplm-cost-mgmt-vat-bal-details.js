	...
	setVatBalParameters:function(result, projectionType, isRecurring, isSchedule, isActual, event, restriction, calculationPeriod, isMcAndVatEnabled, displayVAT, displayCurrency){
		...
		for (var i = 0; i < event.restriction.clauses.length; i++)
			...
			if(event.restriction.clauses[i].name == 'cost_tran_recur.ls_id') {
				...
				if(index != -1){
					restrictionValue_costCategoryId = restrictionValue_projectionType.substring(index+1);
		...
		if (restrictionValue_projectionType != ''){
			this.restriction = this.projectionType+'_id'+'='+"'"+restrictionValue_projectionType+"' ";
		}else{ 
		this.restriction = " 1=1 ";
		}
		if (restrictionValue_costCategoryId != ''){
			this.restriction += " AND cost_cat_id = '" + restrictionValue_costCategoryId + "' ";
		}
