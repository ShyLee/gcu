 ...
 * 		1 - building, 2 - structure, 3 - land, 4 - lease, 5 - sublease
 * 	
 */
...
function showDetails(event){
	...
	switch(parseInt(level)){
		case 2:
			{
				type = node.data['property.field_type'];
				nodeId = node.data['property.pr_id'];
				break;
			}
		case 3:
			{
				type = node.data['bl.field_type'];
				nodeId = node.data['bl.bl_id'];
				break;
			}
		case 4:
			{
				type = node.data['ls.field_type'];
				nodeId = node.data['ls.ls_id'];
				break;
			}
		case 5:
			{
				type = node.data['su.field_type'];
				nodeId = node.data['su.su_id'];
				break;
			}
	}
	mngCostLsBlPrController.loadDetails(type, nodeId);
}
