...
function onAutoCompleteSelect(form, fieldName, selectedValue){
	if (fieldName == "cost_tran_sched.cost_cat_id") {
		setCamFieldValue(fieldName, selectedValue);
	}
}
...
function selectCostCat(cmdObj){
	var controller = addEditScheduledCostController;
	var form = cmdObj.getParentPanel();
	var restriction = null;
	...
	if(controller.runtimeParameters.cam){
		if(valueExistsNotEmpty(controller.view.activityParameters["AbRPLMCosts-CAM_Reconciliation"])){
        	...
        	restriction = new Ab.view.Restriction();
        	restriction.addClause("cost_cat.cost_cat_id", camCostCat, "IN");
        }
	}
	...
	View.selectValue(
			form.id,
			getMessage('cost_cat'),
			['cost_tran_sched.cost_cat_id'],
			'cost_cat',
			['cost_cat.cost_cat_id'],
			['cost_cat.cost_cat_id','cost_cat.cost_class_id'],
			restriction,
			addEditScheduledCostController.afterSelectListenerCostCateg,
			false);
}
