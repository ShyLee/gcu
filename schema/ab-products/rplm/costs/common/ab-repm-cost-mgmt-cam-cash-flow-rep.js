var controller = View.createController('calculateCashFlow', {
	...
	console_onFilter : function() {
		...
		this.calculateCashFlow(this, panel, dateStart, dateEnd, recurrCostRestriction, schedCostRestriction, actualCostRestriction); 
		
		var panelTitle = getMessage("reportCashFlow_title") + getCostTypeMessage(this.displayVAT.type, this.isMcAndVatEnabled);
		var camCostLabel = getMessage("camCost_" + this.cam_cost.replace("-", "_"));
	...
	calculateCashFlow: function(controller, panel, dateStart, dateEnd, recurringCostRestriction, scheduledCostRestriction, actualCostRestriction){
		...
		try {
		    ...
		    View.openJobProgressBar(getMessage('searchMessage'), jobId, '', function(status) {
				...
				var missingExchangeRate = status.jobProperties["missingExchangeRate"];
				
				if(missingExchangeRate && displayError && missingExchangeRate == "true"){
					controller.showExchangeRateError = false;
					View.showMessage(getMessage("errorMissingExchangeRate"));
				}
	...
	getConsoleRestriction: function() { 
        ...
        for(var i = 0; i < projectionTypeRadio.length; i++) {
        	if (projectionTypeRadio[i].checked){
        		this.projectionTypeRadioValue = projectionTypeRadio[i].value;
        		costAsocWith = analyzeBy[this.projectionTypeRadioValue];
        		restriction = " "+ costAsocWith+" IS NOT NULL ";
        		if(this.projectionTypeRadioValue == 'lsBl' || this.projectionTypeRadioValue == 'lsProp' || this.projectionTypeRadioValue == 'lsAll'){
        			this.projectionType = 'ls';
        		}else{
        			this.projectionType = this.projectionTypeRadioValue;
        		}
        		this.printableRestriction.push({'title': this.console.fields.get('show_cost_associated_with').fieldDef.title, 'value': getMessage("projectionType_" + projectionTypeRadio[i].value)});
        	}
        }
        ...
        var cam_cost = null;
        if (camCostSelectObj) {
        	cam_cost = camCostSelectObj.value;
        }
        ...
        for(var i = 0; i < calculationTypeRadio.length; i++) {
        	if (calculationTypeRadio[i].checked){
        		this.calculationType = calculationTypeRadio[i].value;
        		this.printableRestriction.push({'title': this.console.fields.get('show_cost_types_of').fieldDef.title, 'value': getMessage("calculationType_" + calculationTypeRadio[i].value)});
        	}
        }
        
        var calculationPeriodRadio = document.getElementsByName("calculationPeriod");
        for(var i = 0; i < calculationPeriodRadio.length; i++) {
        	if (calculationPeriodRadio[i].checked){
        		this.calculationPeriod = calculationPeriodRadio[i].value;
        		this.printableRestriction.push({'title': this.console.fields.get('group_results_by').fieldDef.title, 'value': getMessage("calculationPeriod_" + calculationPeriodRadio[i].value)});
        	}
        }
        
        if(valueExistsNotEmpty(ctry_id)){
        	this.printableRestriction.push({'title': this.console.fields.get('bl.ctry_id').fieldDef.title, 'value': ctry_id});
        }
        if(valueExistsNotEmpty(regn_id)){
        	this.printableRestriction.push({'title': this.console.fields.get('bl.regn_id').fieldDef.title, 'value': regn_id});
        }
        if(valueExistsNotEmpty(state_id)){
        	this.printableRestriction.push({'title': this.console.fields.get('bl.state_id').fieldDef.title, 'value': state_id});
        }
        if(valueExistsNotEmpty(city_id)){
        	this.printableRestriction.push({'title': this.console.fields.get('bl.city_id').fieldDef.title, 'value': city_id});
        }
        if(valueExistsNotEmpty(site_id)){
        	this.printableRestriction.push({'title': this.console.fields.get('bl.site_id').fieldDef.title, 'value': site_id});
        }
        if(valueExistsNotEmpty(pr_id)){
        	this.printableRestriction.push({'title': this.console.fields.get('property.pr_id').fieldDef.title, 'value': pr_id});
        }
        if(valueExistsNotEmpty(bl_id)){
        	this.printableRestriction.push({'title': this.console.fields.get('bl.bl_id').fieldDef.title, 'value': bl_id});
        }

        if(document.getElementById("cost_cat_id_ex_check").checked){
        	this.printableRestriction.push({'title': getMessage('exclude_cost_cat_of').replace('{0}', ' '), 'value': document.getElementById("cost_cat_id_storage_ex").value});
        }
        
        if(document.getElementById("cost_cat_id_sh_check").checked){
        	this.printableRestriction.push({'title': getMessage('show_cost_cat_of').replace('{0}', ' '), 'value': document.getElementById("cost_cat_id_storage_sh").value});
        }
			...
			switch (costAsocWith)
				{
					case ' cost_tran_recur.ls_id': 
    						...
    						if(this.projectionTypeRadioValue == 'lsBl'){
    							restriction += bl_ls_JOIN + blGeoRestriction;	
    						}else if(this.projectionTypeRadioValue == 'lsProp'){
        ...
        for(var i = 0; i < objAnalyzeCostFor.length; i++) {
        	if (objAnalyzeCostFor[i].checked){
        		this.analyzeCostFor = objAnalyzeCostFor[i].value;
        		this.printableRestriction.push({'title': this.console.fields.get('analyze_costs_for').fieldDef.title.replace('{0}', ' '), 'value': getMessage("analyzeCostFor_" + objAnalyzeCostFor[i].value)});
        	}
        }
		...
		if (valueExistsNotEmpty(cam_cost)){
			this.cam_cost = cam_cost;
			if (this.cam_cost != "ALL") {
				restriction += " AND cost_tran_recur.cam_cost = '" + this.cam_cost + "' ";
			}
		}
...
//				toFiscalYear.setMonth(parseInt(fiscalYearMonth)-1, parseInt(fiscalYearDay));
	...
	reportCashFlow_onChart: function(){
		var chartTitle = getMessage("chartTitle").replace("{0}", getCostTypeMessage(this.displayVAT.type, this.isMcAndVatEnabled)) + " " + getMessage("projectionType_"+this.projectionTypeRadioValue);
...
function enableGeo(){
    ...
    var radioLeaseProperty = document.getElementById("projectionTypeLsProp");
...
function onClickCashFlow(event){
	...
	if(controller.rowRestriction){
		...
		panel.show(false);
		controller.calculateCashFlow(controller, panel, dateStart, dateEnd, recurrCostRestriction, schedCostRestriction, actualCostRestriction); 	
		// select first tab and disable the other two tabs
		controller.tabsCashFlow.enableTab('tabCashFlowDetails', true);
		controller.tabsCashFlow.selectTab('tabCashFlowDetails');

		var costTypeMessage = getCostTypeMessage(controller.displayVAT.type, controller.isMcAndVatEnabled);
		var camCostLabel = getMessage("camCost_" + controller.cam_cost.replace("-", "_"));
		if (valueExistsNotEmpty(camCostLabel)) {
			costTypeMessage += " - " +camCostLabel;
		}
		
		
		panel.setTitle(getMessage("reportCashFlowDetails_title").replace("{0}", costTypeMessage)+" "+controller.getAssetKeyLabel(controller.projectionType)+": "+ controller.rowRestriction.value);
	}
}
	...
	View.openDialog('ab-rplm-cost-mgmt-cam-cash-flow-rep-details.axvw', null, true, {
            ...
            afterInitialDataFetch: function(dialogView){
                ...
                dialogController.setCashFlowParameters( View.controllers.get('calculateCashFlow').WF_result,
                										...
                										View.controllers.get('calculateCashFlow').detailsRestriction,
