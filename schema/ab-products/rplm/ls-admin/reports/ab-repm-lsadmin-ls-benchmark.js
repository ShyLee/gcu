...
var abRepmLsadminLsBenchmarkController = View.createController('abRepmLsadminLsBenchmarkCtrl', {
	...
	abRepmLsadminLsBenchmark_gridPanel_onDetails: function(row){
		...
		this.abRepmLsadminLsBenchmarkDetails_lsContactsPanel.refresh(restriction);
	...
	abRepmLsadminLsBenchmarkDetails_bldgPanel_afterRefresh: function(){
		if (valueExistsNotEmpty(this.abRepmLsadminLsBenchmarkDetails_bldgPanel.getFieldValue('bl.bldg_photo'))) {
			this.abRepmLsadminLsBenchmarkDetails_bldgPanel.showImageDoc('image_field', 'bl.bl_id', 'bl.bldg_photo');
		}else{
			this.abRepmLsadminLsBenchmarkDetails_bldgPanel.fields.get(this.abRepmLsadminLsBenchmarkDetails_bldgPanel.fields.indexOfKey('image_field')).dom.src = null;
			this.abRepmLsadminLsBenchmarkDetails_bldgPanel.fields.get(this.abRepmLsadminLsBenchmarkDetails_bldgPanel.fields.indexOfKey('image_field')).dom.alt = getMessage('text_no_image');
		}
	},
	...
	abRepmLsadminLsBenchmarkDetails_propPanel_afterRefresh: function(){
		if (valueExistsNotEmpty(this.abRepmLsadminLsBenchmarkDetails_propPanel.getFieldValue('property.prop_photo'))) {
			this.abRepmLsadminLsBenchmarkDetails_propPanel.showImageDoc('image_field', 'property.pr_id', 'property.prop_photo');
		}else{
			this.abRepmLsadminLsBenchmarkDetails_propPanel.fields.get(this.abRepmLsadminLsBenchmarkDetails_propPanel.fields.indexOfKey('image_field')).dom.src = null;
			this.abRepmLsadminLsBenchmarkDetails_propPanel.fields.get(this.abRepmLsadminLsBenchmarkDetails_propPanel.fields.indexOfKey('image_field')).dom.alt = getMessage('text_no_image');
		}
	},
...
function onApplyFilter(crtFilter, printableRestriction, displayVAT, displayCurrency){
	...
	if(controller.isMcAndVatEnabled){
		...
		if(costs.ids.length > 0){
			try{
			    ...
			    View.openJobProgressBar(getMessage('msg_update_costs'), jobId, '', function(status) {
			    	...
			    	summarizeCosts(controller, overviewPanel, dateFrom, dateTo, period, isFromCosts, isFromScheduledCosts, isFromRecurringCosts, isActiveRecurringCosts, isSqlStringRestriction, displayVAT, displayCurrency, displayMessage);
			    });
			}catch(e){
		...
		summarizeCosts(controller, overviewPanel, dateFrom, dateTo, period, isFromCosts, isFromScheduledCosts, isFromRecurringCosts, isActiveRecurringCosts, isSqlStringRestriction, displayVAT, displayCurrency, null);
	}
	
}

function summarizeCosts(controller, overviewPanel, dateFrom, dateTo, period, isFromCosts, isFromScheduledCosts, isFromRecurringCosts, isActiveRecurringCosts, isSqlStringRestriction, displayVAT, displayCurrency, displayMessage){
	...
	var currencyVatParams = {
			"vat": displayVAT.type,
			"type": (displayCurrency.exchangeRateType == 'Budget'?"budget":"custom"),
	...
	try {
	    ...
	    View.openJobProgressBar(getMessage('msg_summarize_costs'), jobId, '', function(status) {
			...
			overviewPanel.addParameter("currencyCode", displayCurrency.code);
