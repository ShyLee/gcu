...
public abstract class AbstractRoomTransactionRecorder {
    ...
    protected RoomTransaction getAvailableEmptyRoomPart(final IRoomCategoryDao roomCategoryDao,
            final List<RoomTransaction> roomTransactionsList) {
        ...
        for (final RoomTransaction roomTransaction : roomTransactionsList) {
            ...
            if (!StringUtil.notNullOrEmpty(roomTransaction.getEmployeeId())) {
        ...
        // KB3037886 - If there is no occupiable empty room part, use the last empty room part.
        if (availableEmptyRoomPart == null) {
            availableEmptyRoomPart = emptyRoomPart;
        }
        
        return availableEmptyRoomPart;
    }
}