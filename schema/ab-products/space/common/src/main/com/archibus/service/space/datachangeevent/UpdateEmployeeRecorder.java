...
public class UpdateEmployeeRecorder extends AbstractRoomTransactionRecorder implements
    ...
    public void recordUpdateTransaction(final User user, final Date dateTime,
        ...
        this.locationChanged = this.compareLocation(employee);
    ...
    private Room getFromLocation(final Employee employee, final Date dateTime) {
        final List<RoomTransaction> roomTransactions =
                this.getRoomTransactionDao().findPrimaryForEmployee(employee, dateTime);
    ...
    private void processLocationChange(final User user, final Date dateTime, final Employee employee) {
        ...
        final boolean isNeedRevertFutureTransactionForLocationChange =
                (this.fromRoom != null)
                        && (SpaceFutureTransactionCommon.needRevertFutureTransactions(
                            employee.getBuildingId(), employee.getFloorId(), employee.getRoomId(),
                            dateTime, false, false) || SpaceFutureTransactionCommon
    ...
    private boolean compareDepartment(final Employee employee) {
        ...
        if (this.getRoomTransactionDao().findEmDepartmentChange(employee, new Date()).size() > 0) {
            changed = true;
        }
     ...
     */
    private boolean compareLocation(final Employee employee) {
        
        boolean changed = false;
        if (this.getRoomTransactionDao().findForEmployee(employee, new Date()).isEmpty()
                || this.getRoomTransactionDao().findEmLocationChange(employee, new Date()).size() > 0) {
            changed = true;
        }
