var controller = View.createController('abSpAsgnRmCatRmTypeToRm_Control', {
    ...
    afterViewLoad: function(){
        ...
        ddRecords = View.dataSources.get('ds_ab-sp-asgn-dv-dp-to-rm_dp').getRecords();
        ctRecords = View.dataSources.get('ds_ab-sp-asgn-rmcat-rmtype-to-rm_rmtype').getRecords();
...
function showHideByType(){
	...
	switch (controller.type){
	   case "ct":
		   ...
		   controller.abSpAsgnDvDpToRm_dvTree.show(false);
		   controller.abSpAsgnRmstdToRm_rmstdAssignGrid.show(false);
		   controller.abSpAsgnRmstdToRm_rmstdGrid.show(false);
		   controller.abSpAsgnRmcatRmTypeToRm_rmtypeAssignGrid.show(false);
		   controller.abSpAsgnEmToRm_emAssigned.show(false);
		   controller.abSpAsgnEmToRm_emSelect.show(false);
	     break;
	   case "rs":
		   controller.abSpAsgnDvDpToRm_dpAssignGrid.show(false);
		   controller.abSpAsgnDvDpToRm_dvTree.show(false);
		   controller.abSpAsgnRmcatRmTypeToRm_rmtypeAssignGrid.show(false);
		   controller.abSpAsgnRmcatRmTypeToRm_rmcatTree.show(false);
		   controller.abSpAsgnRmstdToRm_rmstdAssignGrid.show(false);
		   controller.abSpAsgnEmToRm_emAssigned.show(false);
		   controller.abSpAsgnEmToRm_emSelect.show(false);
	     break;
...
function onRmTypeTreeClick(ob){
  ...
  //reset the assign color.
    resetAssgnColor('rmtype.hpattern_acad', ctRecords, 'rmtype.rm_cat', rmCatId, 'rmtype.rm_type', rmTypeId)
  ...
  //load drawing panel by attribute type
	switch (controller.type)
	   ...
	   case "ct":
		    ...
		    controller.abSpAsgnRmcatRmTypeToRm_drawingPanel.setTitle("<font color='#F04000'>"+getMessage('selectType')+"</font>");
		    break;
	   case "rs":
		    var grid = View.panels.get('abSpAsgnRmstdToRm_rmstdAssignGrid');
		    flTreeClickHandler(currentNode, drawingPanel, grid);
		    drawingPanel.isLoadedDrawing = true; 
		    controller.abSpAsgnRmcatRmTypeToRm_drawingPanel.setTitle("<font color='#F04000'>"+getMessage('selectStd')+"</font>");
		    break;
	   case "dd":
		    ...
		    drawingPanel.isLoadedDrawing = true; 
		    controller.abSpAsgnRmcatRmTypeToRm_drawingPanel.setTitle("<font color='#F04000'>"+getMessage('selectDp')+"</font>");
		    break;
	   case "em":
		    ...
		    controller.abSpAsgnRmcatRmTypeToRm_drawingPanel.setTitle("<font color='#F04000'>"+getMessage('selectEm')+"</font>");
		    break;
	   default:
		   View.showMessage(getMessage('noAbbrTypeSelected'));
...
function resizeLegend(){
	var legend=controller.abSpAsgnEmToRm_legendGrid;
 ...
 */
function changeType(){
	var draw=controller.abSpAsgnRmcatRmTypeToRm_drawingPanel;
	...
	switch (controller.type)
	   ...
	   case "ct":
		   ...
		   controller.abSpAsgnRmstdToRm_rmstdGrid.show(false); 
		   controller.abSpAsgnDvDpToRm_dvTree.show(false); 
		   controller.abSpAsgnRmcatRmTypeToRm_rmcatTree.show(true); 
	   ...
	   case "rs":
		   ...
		   controller.abSpAsgnRmcatRmTypeToRm_rmcatTree.show(false); 
		   controller.abSpAsgnDvDpToRm_dvTree.show(false); 
		   controller.abSpAsgnRmstdToRm_rmstdGrid.show(true); 
	   ...
	   case "dd":
		   ...
		   controller.abSpAsgnRmcatRmTypeToRm_rmcatTree.show(false);
		   controller.abSpAsgnRmstdToRm_rmstdGrid.show(false); 
		   controller.abSpAsgnDvDpToRm_dvTree.show(true); 
		   ...
		   //set drawing select ability
		   setSelectability(controller.onclickedFlObj.restriction,'Assign_Department');
	   ...
	   case "em":
		   ...
		   controller.abSpAsgnRmstdToRm_rmstdGrid.show(false); 
		   controller.abSpAsgnDvDpToRm_dvTree.show(false); 
		   controller.abSpAsgnEmToRm_emSelect.show(true);
		...
		// Specify instructions for the Drawing Control
		   ...
		   draw.addEventListener('onclick', onDwgPanelClicked);
	        ...
	        //set drawing select ability
	        setSelectability(controller.onclickedFlObj.restriction,'Assign_Employee');
	        
	        if(controller.flag){
	        controller.abSpAsgnEmToRm_legendGrid.afterCreateCellContent = setLegendLabel;
	        }
	        ...
	        controller.abSpAsgnEmToRm_emSelect.addEventListener('onMultipleSelectionChange', onEmSelectionChange);
		   break;
	   default:
		   View.showMessage(getMessage('noAbbrTypeSelected'));
	}
...
function onDpTreeClick(ob){
    ...
    //reset the assign color
    resetAssgnColor('dp.hpattern_acad', ddRecords, 'dp.dv_id', dvId, 'dp.dp_id', dpId)
    if (drawingPanel.isLoadedDrawing) {
    	//sets the current assignment values
...
function detectIfExistsFutureInDefineEm(){
    ...
    var em_id = controller.currentEmAssign.getValue('em.em_id');
    
    var bl_id = controller.currentEmAssign.getValue('em.bl_id');
    var fl_id = controller.currentEmAssign.getValue('em.fl_id');
    var rm_id = controller.currentEmAssign.getValue('em.rm_id');
...
function removeEmpSelectionFromDrawing(blId, flId, rmId){
    ...
    var dcl = new Ab.drawing.DwgCtrlLoc(blId, flId, rmId, controller.dwgName);
...
function addAssignmentRows(pk){
    ...
    controller.currentEmAssign=emAssign;
...
function normalFloor(){
	...
	layout=View.getLayoutManager('nested_west');
	layout.expandRegion('north');
	controller.abSpAsgnRmcatRmTypeToRm_drawingPanel.actions.get('max').show(true);
	controller.abSpAsgnRmcatRmTypeToRm_drawingPanel.actions.get('normal').show(false);
