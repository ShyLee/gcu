var ascBjUsmsProcAsgnApproveReqApproveTabController = View.createController("ascBjUsmsProcAsgnApproveReqApproveTabController", {
    ...
    ascBjUsmsProcAsgnApproveReqApproveTabApproveForm_onApprove: function(){
        ...
        if (result.code == 'executed') {
			...
			{	
				//修改eq_check的状态为通过
				var restriction=new Ab.view.Restriction();
    			var checkId = this.ascBjUsmsProcAsgnApproveReqApproveTabForm1.getFieldValue("activity_log.check_id");
    			restriction.addClause("eq_check.check_id",checkId,"=");
    ...
    ascBjUsmsProcAsgnApproveReqApproveTabApproveForm_onReject: function(){
        ...
        if (result.code == 'executed'){
    	   //驳回eq_check
			var restriction=new Ab.view.Restriction();
    		var checkId = this.ascBjUsmsProcAsgnApproveReqApproveTabAddInfoForm.getFieldValue("eq_check.check_id");
    		restriction.addClause("eq_check.check_id",checkId,"=");
    			
    		var eqCheckRecord = this.ascBjUsmsProcAsgnApproveReqApproveTabAddFormDS.getRecord(restriction);
    		eqCheckRecord.setValue("eq_check.approved",'2');
    		this.ascBjUsmsProcAsgnApproveReqApproveTabAddFormDS.saveRecord(eqCheckRecord);
    		
    		//驳回eq_checkReport
    		var ascBjUsmsEqCheckReportDs=View.dataSources.get('ascBjUsmsEqCheckReportDs');
    		var checkMainId = this.ascBjUsmsProcAsgnApproveReqApproveTabAddInfoForm.getFieldValue("eq_check.check_main_id"); 
    		...
    		var reportReocord=ascBjUsmsEqCheckReportDs.getRecord(reportRes);
    		reportReocord.setValue('eq_check_report.audit_status','3');
    		ascBjUsmsEqCheckReportDs.saveRecord(reportReocord);
       } 
