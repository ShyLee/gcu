...
var controller = View.createController('aschlRmByDpController', {
    ...
    afterViewLoad: function(){
        ...
        this.abSpHlRmByDp_flGrid.addEventListener('onMultipleSelectionChange', function(row){
    		View.dataSources.get('ds_ab-sp-hl-rm-by-dp_drawing_rmHighlight').addParameter('rmDv', " AND rm.dv_id IS NOT NULL");
        ...
        document.getElementById("viewToolbar_title").innerHTML = this.dvName + "实验室公房情况";
        
        var res3= new Ab.view.Restriction();
    	res3.addClause("rm.dv_id","51","=");
    	res3.addClause("rm.dp_id","21","=");
    	this.abScSearchRmLayoutByDvGrid.refresh(res3);
    },
    showLabInfo:function(){
    	var panel = this.abScSearchRmLayoutByDvCountPeopleGrid;
    	var selectedIndex = panel.selectedRowIndex;
    	var dv_id = panel.rows[selectedIndex]["dp.dv_id"];
    	var dp_id = panel.rows[selectedIndex]["dp.dp_id"];
    	this.dvId=dv_id;
    	this.dpId=dp_id;
    	   	
    	this.abSpHlRmByDp_flGrid.addEventListener('onMultipleSelectionChange', function(row){
    		View.dataSources.get('ds_ab-sp-hl-rm-by-dp_drawing_rmHighlight').addParameter('rmDv', " AND rm.dv_id = '" + dv_id + "' AND rm.dp_id = '" + dp_id + "'");
			View.panels.get('abSpHlRmByDp_DrawingPanel').clear();
            View.panels.get('abSpHlRmByDp_DrawingPanel').addDrawing(row, null);
            
        });
    	
    	this.abSpHlRmByDp_flGrid.addParameter('dvRes', "rm.dv_id= '" + this.dvId + "' and rm.dp_id='" + this.dpId + "'");
    	this.abSpHlRmByDp_flGrid.refresh();
    	
    	var res1= new Ab.view.Restriction();
    	res1.addClause("rm.dv_id",dv_id,"=");
    	res1.addClause("rm.dp_id",dp_id,"=");
    	this.abSpHlRmByDp_flGrid.refresh(res1);
    	
    	var res2= new Ab.view.Restriction();
    	res2.addClause("rm.dv_id",dv_id,"=");
    	res2.addClause("rm.dp_id",dp_id,"=");
    	this.abScSearchRmLayoutByBuGridReport.refresh(res2);
    	
    	var res3= new Ab.view.Restriction();
    	res3.addClause("rm.dv_id",dv_id,"=");
    	res3.addClause("rm.dp_id",dp_id,"=");
    	this.abScSearchRmLayoutByDvGrid.refresh(res3);
    },
    ...
    abScSearchRmLayoutByDvCountPeopleGrid_onShow:function(){
    	var res1= new Ab.view.Restriction();
    	this.abSpHlRmByDp_flGrid.refresh(res1);
...
function setDrawingTitle(){
    if (dvId) {
        var title = String.format(getMessage('drawingPanelTitle1'), controller.dvId);
