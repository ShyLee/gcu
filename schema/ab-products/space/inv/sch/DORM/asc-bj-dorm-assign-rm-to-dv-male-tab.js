var controller = View.createController('abSpAsgnRmDpToRm_Control', {
    ...
    onStart:function(){
		...
		if(this.dvId!=undefined && this.dvId!=""){
			...
			this.abSpAsgnDvDpToRm_drawingPanel.setTitle("分配男生宿舍给-"+this.dvName);
			this.abSpAsgnDvDpToRm_drawingPanel.appendInstruction("default", "", "分配男生宿舍给-"+this.dvName);
			
//			if(this.abSpAsgnDvDpToRm_drawingPanel.isLoadedDrawing){
			            ...
			            if (button == 'yes') {
			            	try {
			            		var parameters = {
			            				...
			            				restriction: "rm.bl_id ='" + blId + "' and rm.fl_id='"+flId+"' and rm.rm_cat='301'"
			            		};
			            		var cap_em="";
			            		var result = Workflow.call('AbCommonResources-getDataRecords', parameters);
			            		var records=result.data.records;
...
//    abSpAsgnDvDpToRm_dpAssignGrid_onRemove:function(){
//    	var grid = View.panels.get("abSpAsgnDvDpToRm_dpAssignGrid");
//        var row = grid.rows[grid.selectedRowIndex];
//        this.abSpAsgnDvDpToRm_drawingPanel.unassign("dv.dv_id", controller.dvId);
