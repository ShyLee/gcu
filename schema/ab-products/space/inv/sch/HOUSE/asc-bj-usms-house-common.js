...
/**
 * 把日期2016-1-1 改为 2016-01-01
 * @param dateString
 * @returns {String}
 */
function ABZF_getYearMonthDateByString(dateString){
    var yearMonthDay = "";
    if (valueExists(dateString)) {
        var year = dateString.split("-")[0];//获取年
        var temp_month = parseInt(dateString.split("-")[1]);//获取月
        var month = temp_month < 10 ? "0" + temp_month : temp_month;
        var temp_day = parseInt(dateString.split("-")[2]);//获取日
        var day = temp_day < 10 ? "0" + temp_day : temp_day;
