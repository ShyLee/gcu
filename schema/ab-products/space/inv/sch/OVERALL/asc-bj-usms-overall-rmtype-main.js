 ...
 * @author Keven.xi
 */
View.createController('ascBjUsmsOverallRmTypeMainController', {
    ...
    afterViewLoad: function(){
        ...
        this.ascBjUsmsOverallRmtypeMain_siteBasicGrid.addParameter('siteIdRes', this.siteId);
        this.ascBjUsmsOverallRmtypeMain_rmtypeGrid.addParameter('siteIdRes', this.siteId);
        this.ascBjUsmsOverallRmtypeMain_rmtypeGrid.buildPostFooterRows = addTotalRowForBl;
    },
    
    ascBjUsmsOverallRmtypeMain_rmtypeGrid_afterRefresh: function(){
        var selectedRows = new Array();
        var grid = View.panels.get('ascBjUsmsOverallRmtypeMain_rmtypeGrid');
...
function calPercentAreaJianzhu(panel,totleArea){
	...
	for (var i = 0; i < panel.rows.length; i++) {
		...
		headerRow.cells[5].innerHTML='占比';
...
function addTotalRowForBl(parentElement){
    ...
    for (var i = 0; i < this.rows.length; i++) {
    	...
    	//room count
		var rmTypeRoomCount = row['rmtype.total_count_rm'];
		if(row['rmtype.total_count_rm.raw']){
			rmTypeRoomCount = row['rmtype.total_count_rm.raw'];
		}
