        ...
        if (result.code == "executed") {
            if (result.message == "OK") {
				...
				if (!valueExists(this.arrTimeline[this.globalTimeline])) {
					...
					this.arrTimeline[this.globalTimeline].addColumn('info', "", 'image', this.viewResourcesDetails.createDelegate(this), getMessage('info'), 'ab-sch-rr-info.gif');
					this.arrTimeline[this.globalTimeline].addColumn('resources_approval', "", 'image', null, getMessage('approval'), 'ab-sch-rr-approval.gif');
...
function buildNonUniqueResourcesList(ID){
    ...
    for (var i = 0; i < NonUniqueResources.length; i++) {
        if (NonUniqueResources[i].resource_nature == addResourceReservContentController.ResourceNatures[ID]) {
            ...
            info_icon_path = "ab-sch-rr-info.gif";
            appr_icon_path = "ab-sch-rr-approval.gif";
