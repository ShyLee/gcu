...
function initRestriction() {
	...
	// If any of the groups in the User.groups[] array is 'RESERVATION SERVICE DESK'
	if (ABRV_isMemberOfGroup(View.user,'RESERVATION SERVICE DESK')) {
		// He can see all the reservation that needs an approval, and the arrangement or 
		// resource has not an approver assigned
		res_rmRestriction.addClause("rm_arrange.group_name", "", "IS NULL");
		res_rsRestriction.addClause("resources.group_name", "", "IS NULL");
	}
	
	// If any of the groups in the User.groups[] array is 'RESERVATION APPROVER'
	if (ABRV_isMemberOfGroup(View.user,'RESERVATION APPROVER')) {
