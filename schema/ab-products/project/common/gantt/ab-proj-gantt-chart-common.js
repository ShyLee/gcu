...
function cascadeAllTaskDependencies() {
	try {
	    var result = Workflow.callMethod("AbProjectManagement-ProjectGanttService-cascadeAllTaskDependencies");
	    if (result.message) {
	    	return true;
	    }
	    else return false;
...
function cascadeTaskDependencies(record) {
	...
	try {
	    var result = Workflow.callMethod("AbProjectManagement-ProjectGanttService-cascadeTaskDependencies", activity_log_id);
	    if (result.message) {
