...
function afterCreateDataRows_removeWorkRequest(parentElement, columns, isValidUserProcess, panel){
	// search the "createWorkReq" column
	var serviceReqColumnIndex = -1;
	for (var c = 0; c < columns.length; c++) {
		if(columns[c].id == "createWorkReq") {
			serviceReqColumnIndex = c;
			break;
		}
	}
	
	// no "createWorkReq" column, nothing left to do
	if (serviceReqColumnIndex == -1) {
		return;
	}
	if(!isValidUserProcess){
		panel.removeColumn(serviceReqColumnIndex);
		/*
		 * 03/29/2010 IOAN kb 3026726
		 * remove header cell
		 */
		var trNode = panel.headerRows[0];
		var thNode = trNode.cells[serviceReqColumnIndex];
		trNode.removeChild(thNode);
		serviceReqColumnIndex = -1;
	}

	// if "createWorkReq" column deleted, return
	if (serviceReqColumnIndex == -1) {
		return;
	}
}
