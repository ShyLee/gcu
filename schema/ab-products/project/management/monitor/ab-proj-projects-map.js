var projProjectsMapPage1Controller = View.createController('projProjectsMapPage1', {	
	...
	afterViewLoad: function(){;
		this.map = new Ab.flash.Map(
			'mapPanel', 
			'map',
			'projProjectsMapPage1_buildingsDS',
			true					
		);
	...
	projProjectsMapPage1_buildingsGrid_onShowBuildings: function() {
		...
		if (isValidGisLicense) {
			...
			var markerProperty = this.map.getMarkerPropertyByDataSource('projProjectsMapPage1_buildingsDS');
			if (markerProperty == null) {
				markerProperty = new Ab.flash.ArcGISMarkerProperty('projProjectsMapPage1_buildingsDS', ['bl.lat', 'bl.lon'], ['bl.bl_id'], ['bl.address1', 'bl.city_id', 'bl.state_id', 'bl.ctry_id']);
