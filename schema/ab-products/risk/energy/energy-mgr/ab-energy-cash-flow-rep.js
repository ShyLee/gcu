	...
	console_onFilter : function() {
		...
		try {
			var jobId = Workflow.startJob('AbCommonResources-CostService-getCashFlowProjection', 
						...
						this.isFromActualCosts, this.isFromScheduled, this.isFromRecurring, restriction, restriction_tab1, restriction_tab2, {});
			var controller = this;
...
*/
	...
	appendSubtotals: function(panel, calculationPeriod) {
		...
		for (var r = 0; r < panel.dataSet.rowValues.length; r++) {
			...
			if ((row != 0) && (this.groupByCostCategory = true)) {
            	...
            	var separatorIndexCurrent = panel.dataSet.rowValues[row].n.indexOf('|');
            	var separatorIndexPrevious = panel.dataSet.rowValues[row-1].n.indexOf('|');
	...
	updateDimensionHeadings: function(projectionType, calculationPeriod, calculationType, dataSet) {
		...
		switch (projectionType) {
			case 'ls': 
				this.reportCashFlow.parentElement.childNodes[0].firstChild.childNodes[1].childNodes[0].innerHTML = getMessage('leaseCode');
				break;
			case 'bl': 
				this.reportCashFlow.parentElement.childNodes[0].firstChild.childNodes[1].childNodes[0].innerHTML = getMessage('buildingCode');
				break;
			case 'ac': 
				this.reportCashFlow.parentElement.childNodes[0].firstChild.childNodes[1].childNodes[0].innerHTML = getMessage('accountCode');
				break;
			case 'pr': 
				this.reportCashFlow.parentElement.childNodes[0].firstChild.childNodes[1].childNodes[0].innerHTML = getMessage('propertyCode');
				break;
			}
