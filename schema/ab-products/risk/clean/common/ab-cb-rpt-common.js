 ...
 */
function existsSelectedItemInSameRoom(grid, rmRow){
	var selectedRows = grid.getSelectedRows();
	
	for (var i = 0; i < selectedRows.length; i++) {
		var selRow = selectedRows[i].row;
		if(selRow.getFieldValue('activity_log.bl_id') == rmRow.getFieldValue('activity_log.bl_id')
				&& selRow.getFieldValue('activity_log.fl_id') == rmRow.getFieldValue('activity_log.fl_id')
				&& selRow.getFieldValue('activity_log.rm_id') == rmRow.getFieldValue('activity_log.rm_id'))
			return true;
	}
