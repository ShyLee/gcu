...
var abCbAssessItemsListCtrl = View.createController('abCbAssessItemsListCtrl', {
	...
	validateFilter: function(){
		...
		if(!compareDates(console, 'date_assessed_from', 'date_assessed_to', 'msg_field_smaller_than', "<=")){
			return false;
		}
		return true;
	},
	...
	getFilterRestriction: function(){
		...
		// material code [hcm_id]
		this.addRestrictionClause(console, 'activity_log.hcm_id', 'activity_log.hcm_id', restriction, '=', ')AND(');
		//hazard status code [hcm_haz_status_id]
		this.addRestrictionClause(console, 'activity_log.hcm_haz_status_id', 'activity_log.hcm_haz_status_id', restriction, '=', ')AND(');
		//friable [hcm_friable]
		this.addRestrictionClause(console, 'activity_log.hcm_friable', 'activity_log.hcm_friable', restriction, '=', ')AND(');
		//location of material [hcm_loc_typ_id]
		this.addRestrictionClause(console, 'activity_log.hcm_loc_typ_id', 'activity_log.hcm_loc_typ_id', restriction, '=', ')AND(');
		//contains hazard [hcm_is_hazard]
		if(console.getFieldValue('hcm_is_hazard_or1') && console.getFieldValue('hcm_is_hazard_or2')){
			restriction.addClause('activity_log.hcm_is_hazard', console.getFieldValue('hcm_is_hazard_or1'), '=', ')AND(');
			restriction.addClause('activity_log.hcm_is_hazard', console.getFieldValue('hcm_is_hazard_or2'), '=', 'OR');
		}else if(console.getFieldValue('hcm_is_hazard_or1')){
			this.addRestrictionClause(console, 'hcm_is_hazard_or1', 'activity_log.hcm_is_hazard', restriction, '=', ')AND(');
		}else if(console.getFieldValue('hcm_is_hazard_or2')){
			this.addRestrictionClause(console, 'hcm_is_hazard_or2', 'activity_log.hcm_is_hazard', restriction, '=', ')AND(');
		}
		this.addRestrictionClause(console, 'activity_log.hcm_is_hazard', 'activity_log.hcm_is_hazard', restriction, '=', ')AND(');
		//Material Condition Code [hcm_cond_id]
		this.addRestrictionClause(console, 'activity_log.hcm_cond_id', 'activity_log.hcm_cond_id', restriction, '=', ')AND(');
		//Pending Action [hcm_pending_act]
		this.addRestrictionClause(console, 'activity_log.hcm_pending_act', 'activity_log.hcm_pending_act', restriction, '=', ')AND(');
		//Hazard Rank Code [hcm_haz_rank_id]
		this.addRestrictionClause(console, 'activity_log.hcm_haz_rank_id', 'activity_log.hcm_haz_rank_id', restriction, '=', ')AND(');
		//Hazard Rating Code [hcm_haz_rating_id]
		this.addRestrictionClause(console, 'activity_log.hcm_haz_rating_id', 'activity_log.hcm_haz_rating_id', restriction, '=', ')AND(');
		//Hazard Response [repair_type]
		this.addRestrictionClause(console, 'activity_log.repair_type', 'activity_log.repair_type', restriction, '=', ')AND(');
		//Date Assessed (From) [date_assessed_from] !! This is alias 
		this.addRestrictionClause(console, 'date_assessed_from', 'activity_log.date_assessed', restriction, '>=', ')AND(', false);
		//Date Assessed (To) [date_assessed_to] !! This is alias
		this.addRestrictionClause(console, 'date_assessed_to', 'activity_log.date_assessed', restriction, '<=', ')AND(', false);
	...
	addRestrictionClause: function(console, source , target, restriction, op, relOp, replace){
		...
		var value = console.getFieldValue(source);
...
*/
		try{
			var result  = Workflow.callMethod('AbRiskCleanBuilding-CleanBuildingService-copyAssessmentItems', pKeys, projectId, assessedBy, assignedTo);
