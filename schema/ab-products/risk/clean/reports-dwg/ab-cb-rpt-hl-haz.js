...
var abCbRptHlHazController = View.createController('abCbRptHlHazCtrl',{
    ...
    createMap:function(){
    	...
    	if(hasValidArcGisMapLicense()){
			this.map = new Ab.flash.Map(
				...
				'abCbRptHlHaz_dsBuilding', //Data Source
				true
			);
		}
    },
	 ...
	 * geocode a building
	 */
    abCbRptHlHaz_htmlMap_onGeocode: function(){
        var geoCodeTool = new Ab.flash.Geocoder(this.map);
 		var restriction = new Ab.view.Restriction();
		restriction.addClause('bl.bl_id', this.blId, '=');
		
        geoCodeTool.geoCode('abCbRptHlHaz_dsGeoBuilding', restriction, 'bl', 'bl.bl_id', ['bl.lat', 'bl.lon'], ['bl.address1', 'bl.city_id', 'bl.state_id', 'bl.zip', 'bl.ctry_id'], true);
        // KB 3033802  show markers after geocode action.
        if (this.map) {
        	this.map.refresh(restriction);
        }
		// refresh the tree after geocoding
		var tree = this.abCbRptHlHaz_ctryTree;
...
function highlightAssetsOnDwgLoad(){
	...
	case "rooms":
		var rooms = View.panels.get('abCbRptHlHaz_gridRepRooms').gridRows.items;
		recsToHighlight = controller.getRoomIdsForDrawing(rooms);
		recsType = "rm";
		break;

	case "hazards":
		var hazards = View.panels.get('abCbRptHlHaz_gridRepHazards').gridRows.items;
		recsToHighlight = controller.getHazardIdsForDrawing(hazards);
		recsType = "haz";
		break;

	case "samples":
		var samples = View.panels.get('abCbRptHlHaz_gridRepSamples').gridRows.items;
		recsToHighlight = controller.getSampleIdsForDrawing(samples);
		recsType = "sam";
		break;
...
function onGridsMultipleSelectionChange(row) {
	...
	if(row.row.isSelected()){
		...
		if(assetToDisplay == "rooms"){
			items = controller.getRoomIdsForDrawing(controller.abCbRptHlHaz_gridRepRooms.gridRows.items);
		} else if(assetToDisplay == "hazards"){
			items = controller.getHazardIdsForDrawing(controller.abCbRptHlHaz_gridRepHazards.gridRows.items);
		} else if(assetToDisplay == "samples"){
			items = controller.getSampleIdsForDrawing(controller.abCbRptHlHaz_gridRepSamples.gridRows.items);
		}
