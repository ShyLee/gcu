			...
			if(existsSimilar()){
				View.confirm(getMessage("similarIncidentExists"), function(button){
					if (button == 'no') {
			        	...
			        	abEhsTrackIncidentsCtrl.saveIncident();
			        }
			    })
			}else{
				abEhsTrackIncidentsCtrl.saveIncident();
...
function isDateIncidentValid(){
	...
	if(valueExistsNotEmpty(dateIncident)){
		var objDateIncident = abEhsTrackIncidentsCtrl.abEhsTrackIncidents_incidents_ds.parseValue("ehs_incidents.date_incident", dateIncident, false);
