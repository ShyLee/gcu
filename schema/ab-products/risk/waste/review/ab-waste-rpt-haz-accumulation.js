...
function setInstruction(grid){
	 var instructions = "<span style='font-weight:bold'>"+getMessage("instructionMess1")+"</span>";
	    ...
	    instructions +="<span style='background-color:#FC6'>"+" "+View.activityParameters['AbRiskWasteMgmt-satellite_days_deadline']+" "+getMessage("instructionMess7")+"</span>";
...
function refreshRecordBg(gridObj){
	...
	for (var i = 0; i < rows.length; i++) {
		...
		if(dateStartDiff>=(accumDays-deadlineDays)){
			Ext.get(rows[i].row.dom).setStyle('background-color', '#FC6');
		}
