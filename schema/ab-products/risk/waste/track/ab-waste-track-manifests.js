		...
		if(valueExistsNotEmpty(dateShipped)){
			if(row['waste_manifests.date_ship_offset']!=undefined){
				...
				if ( ( wasteGeneratorType=='L' && ( (dateShipOffset>=callLarge)&&!dateFacContact||(dateShipOffset>=excLarge)  )  )
					...
					Ext.get(row.row.dom).setStyle('background-color', '#993333');
					
					Ext.get(row.row.dom).setStyle('fontWeight', 'bold');
				}
					...
					Ext.get(row.row.dom).setStyle('background-color', '#FF9900');
				}
					...
					Ext.get(row.row.dom).setStyle('background-color', '#FFFF00');
				}
			}
		}		
}
