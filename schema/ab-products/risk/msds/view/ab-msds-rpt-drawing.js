...
var abRiskMsdsRptDrawingController = View.createController('abRiskMsdsRptDrawingController', {
	...
	abRiskMsdsRptDrawingTreeSite_onShowSeletedFloorPlan : function() {
		if (this.checkSelection()) {
			...
			for ( var i = 0; i < flNodes.length; i++) {
				...
				restriction += "OR (bl_id='" + flNodes[i].parent.data['bl.bl_id'] + "' and fl_id='" + flNodes[i].data["fl.fl_id"] + "') "
	...
	abRiskMsdsRptDrawingFormLocation_onClearSelectedRoom : function() {
		...
		opts.zoomToId = false;
		FABridge.abDrawing.root().highlightRooms(opts);
	}
...
function selectRoom(row) {
	...
	dcl.setFromCtrlOb(row, row.row.isSelected());
	var s = dcl.getFullRoomString();
	opts.appendRec(s);
	opts.mode = 'selected';
	opts.zoomToId = false;
