...
//			if (clause.value==''||clause.value==0) {
				...
				if(clause.op == "IN"){
					...
					return " AND " + clause.name +" "+clause.op + " '" + clause.value + "'";
				}
			}
	},
    /**
    ...
    filterFirstChar: function(){
		...
		while(ghs_id.substring(0,1)==Ab.form.Form.MULTIPLE_VALUES_SEPARATOR){
			ghs_id=ghs_id.substring(1);
		}
		while(chemical_name.substring(0,1)==Ab.form.Form.MULTIPLE_VALUES_SEPARATOR){
			chemical_name=chemical_name.substring(1);
		}
