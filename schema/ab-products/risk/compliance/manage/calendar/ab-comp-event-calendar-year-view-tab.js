...
View.createController('abCompEventCalendarYearViewTab', {
	...
	/**
	* Event handler for action 'Filter', show/hide console regions.
	*/
	panelHtml_onFilter1: function() {
		var layoutManager = View.getOpenerView().getLayoutManager('main');
...
function onclickEvent(id) {
	...
	if (openerView.viewName.toLowerCase().indexOf('rpt') != -1) {
		...
		openerView.openDialog(popUp,restriction);	
	}else{
		openerView.popUpTitle = popUpTitle;
		openerView.openDialog(popUp);
	}
}
