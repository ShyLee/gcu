var abGbRptFpSrcDetailCtrl = View.createController('abGbRptFpSrcDetailCtrl', {
	...
	abGbRptFpSrcDetail_scope_afterRefresh: function(){
		this.abGbRptFpSrcDetail_scope.removeSorting();
		// last row is total we must chnage style
		var lastRow = this.abGbRptFpSrcDetail_scope.gridRows.get(this.abGbRptFpSrcDetail_scope.gridRows.length-1);
		lastRow.cells.each(function(cell){
			cell.dom.className =  'totals';
			cell.dom.style.textAlign = 'right';
		});
		
		setColumnTitle(this.abGbRptFpSrcDetail_scope, "gb_fp_totals.vf_total_emiss");
	},
	
	abGbRptFpSrcDetail_source_afterRefresh: function(){
		this.abGbRptFpSrcDetail_source.removeSorting();
		// last row is total we must chnage style
		var lastRow = this.abGbRptFpSrcDetail_source.gridRows.get(this.abGbRptFpSrcDetail_source.gridRows.length-1);
		lastRow.cells.each(function(cell){
			cell.dom.className =  'totals';
			cell.dom.style.textAlign = 'right';
		});
		
		setColumnTitle(this.abGbRptFpSrcDetail_source, "gb_fp_totals.vf_total_emiss");
	},
