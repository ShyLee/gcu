var abGbFpDataS3WastesSol_ctrl = View.createController('abGbFpDataS3WastesSol_ctrl', {
	...
	setFieldsStyle: function(){
		this.abGbFpDataS3WastesSol_methodology.getFieldLabelElement("field_CO2Emissions").style.fontWeight = "bold";
	},
...
function onClickMethodology(row){
    ...
    try {
        ...
        var bl_id = row["gb_fp_s3_waste_sol.bl_id"];
        var calc_year = parseInt(row["gb_fp_s3_waste_sol.calc_year"]);
        var scenario_id = row["gb_fp_s3_waste_sol.scenario_id"];
        var source_id = parseInt(row["gb_fp_s3_waste_sol.source_id"]);
        ...
        var methPanel = abGbFpDataS3WastesSol_ctrl.abGbFpDataS3WastesSol_methodology;
        
        methPanel.showInWindow(dialogConfig);
        
		var restriction = new Ab.view.Restriction();
		restriction.addClause('gb_fp_s3_waste_sol.source_id', source_id);
		
		methPanel.refresh(restriction);
        methPanel.setFieldValue("amount_recycle_units", methPanel.getFieldValue("gb_fp_s3_waste_sol.units"));
        
        //set virtual fields
		if(result.data['message']){
			showInformationInForm(methPanel,result.data['message']);
		}
		setVirtualFieldValue(result.data, methPanel, 'gb_fp_s3_waste_sol.amount_disp_tons', 'amount_disp_tons');
		setVirtualFieldValue(result.data, methPanel, 'gb_fp_s3_waste_sol.disp_emiss_fact', 'disp_emiss_fact');
		setVirtualFieldValue(result.data, methPanel, 'gb_fp_s3_waste_sol.amount_recy_tons', 'amount_recy_tons');
		setVirtualFieldValue(result.data, methPanel, 'gb_fp_s3_waste_sol.recy_emiss_fact', 'recy_emiss_fact');
		setVirtualFieldValue(result.data, methPanel, 'gb_fp_s3_waste_sol.emiss_mtce', 'emiss_mtce');
		setVirtualFieldValue(result.data, methPanel, 'gb_fp_s3_waste_sol.c_CO2', 'c_CO2');
    } 
    catch (e) {
        Workflow.handleError(e);
    }
