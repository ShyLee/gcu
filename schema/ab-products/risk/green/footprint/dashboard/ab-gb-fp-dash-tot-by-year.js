...
Ab.chart.ChartControl.prototype.getParameters = function(restriction){
	...
	var dataAxis;
	 if(this.chartType == "lineChart"){
		 dataAxis = this.configObj.getConfigParameter('dataAxis');
	 }else if (this.chartType == "stackedBarChart"){
		dataAxis = [];
		for(var i = 0; i < this.configObj.getConfigParameter('dataAxis').length; i++){
			var dAxis = this.configObj.getConfigParameter('dataAxis')[i];
