...
public class PopulateServicePeriodsService {
    ...
    public static void run() {
        ...
        final String[] svcPeriodFlds =
                { "date_start", "date_end", "cost", "demand", "consumption", "num_days",
                        "time_period", "bill_type_id", "bl_id" };
        ...
        for (final DataRecord billRecord : billRecords) {
            ...
            amount_expense =
                    Double.parseDouble(billRecord.getValue("bill_archive.amount_expense")
                        .toString());
            try {
                date_service_end =
    ...
    /**
     * Fetches all the records from the energy_bl_svc_period table
     */
    private static List<DataRecord> loadBillPeriods() {
        final String billTable = "bill_archive";
        final String[] billFields =
                { "date_service_end", "date_service_start", "time_period", "amount_expense",
                        "amount_income", "qty_energy", "qty_power", "qty_volume", "bill_type_id",
                        "vn_id", "bill_id", "bl_id", "bill_type_id" };
        ...
        final List<DataRecord> billRecords = bill_ds.getRecords();
        return billRecords;
    }
    ...
    private static void populatePeriodOats() {
        final String SQL =
                ""
                        ...
                        + "       WHERE   p.time_period            = energy_bl_svc_period.time_period "
