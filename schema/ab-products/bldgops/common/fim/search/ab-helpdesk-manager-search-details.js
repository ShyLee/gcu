...
var helpdeskSearchDetailsController = View.createController('searchDetailsGrid', { 
	...
	hidePanel: function(panelId) {
		// View.panels.get(panelId);
		var panel = document.getElementById(panelId+"_layoutWrapper");
		panel.style.display = "none";
	},
	
	hideWorkRequestPanels: function() {
	 	
		if (this.detailsWrDS.getRecords().length == 0) {  
			this.hidePanel("wrOverviewPanel");
			this.hidePanel("wrDetailsPanel");
		} else if (this.detailsWrDS.getRecords().length == 1) {
			this.hidePanel("wrOverviewPanel")
		} else { 
		}
	},
