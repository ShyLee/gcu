...
public class Notification extends StepImpl {
    ...
    public void sendRequest(final Message message) {
        ...
        if (StringUtil.notNullOrEmpty(this.attachments)) {
            ...
            try {
                MessageHelper.sendMessage(this.context, message, attachmentBodyParts);
            } catch (final Exception e) {
                this.log.debug("Sending mail with attachments failed");
                e.printStackTrace();
                sendEmail(this.context, message.getText(), message.getSubject(),
                    message.getMailTo(), this.activity_id);
            }
        } else {
            sendEmail(this.context, message.getText(), message.getSubject(), message.getMailTo(),
                this.activity_id);
        }
    }
