...
public class ServiceLevelAgreement extends HelpdeskEventHandlerBase {
     ...
     * 
     * <p>
     * The response parameters are stored in a <code>Map</code>. This makes it easy to extend. <br />
     ...
     * @return
     */
    public static ServiceLevelAgreement getInstance(final EventHandlerContext context,
            final String tableName, final String fieldName, final int pkValue) {
        
        final String key = "sla." + tableName + "." + fieldName + "." + pkValue;
     ...
     */
    public static ServiceLevelAgreement getInstance(final EventHandlerContext context,
            final int priority, final int ordering_seq, final String activity_type) {
        
        final String key = "sla." + activity_type + "." + ordering_seq + "." + priority;
     ...
     * <li>{@link com.archibus.eventhandler.sla.ServiceLevelAgreement#lookupResponseParameters()
     * Look up response parameters}</li>
     * </ol>
     * 
     * @param context Workflow rule execution context
     * @param values problem parameters
     */
    public ServiceLevelAgreement(final EventHandlerContext context, final Map values) {
        super();
        ...
        final String key =
                "sla." + this.activity_type + "." + this.ordering_seq + "." + this.priority;
     ...
     */
    public ServiceLevelAgreement(final EventHandlerContext context, final String tableName,
        ...
        if (pkValue > 0) {
            ...
            final Object[] record =
                    selectDbValues(context, tableName, Constants.REQUEST_PARAMETER_NAMES, fieldName
                            + "=" + pkValue);
            ...
            if (record == null) {
                ...
                final String errorMessage =
                        prepareMessage(context, "No record found for table [{0}] with id = [{1}]",
                            args);
     ...
     */
    public ServiceLevelAgreement(final EventHandlerContext context, final JSONObject json) {
        this.context = context;
     ...
     * <li>{@link com.archibus.eventhandler.sla.ServiceLevelAgreement#lookupSLAforPKeys() Look up
     * other parameters in the database}</li>
     * </ol>
     * 
     * @param context Workflow rule execution context
     ...
     */
    public ServiceLevelAgreement(final EventHandlerContext context, final int priority,
            final int ordering_seq, final String activity_type) {
        this.context = context;
        // set the primary key
    ...
    private void lookupDefaultPriority() {
        final Object tmp =
                selectDbValue(this.context, Constants.SLA_REQUEST_TABLE, "default_priority",
                    "activity_type = " + literal(this.context, this.activity_type)
     ...
     * <li>{@link #lookupStandardParameters() equipment standard and employee standard}</li>
     * <li>{@link #getResponseParametersFromPrimaryKey() Lookup response parameters for the primary
     * key}</li>
     * </ol>
     * 
     */
    private void lookupSLAforPKeys() {
        // request parameters
        final String where =
                " ordering_seq =" + this.ordering_seq + " AND activity_type = "
                        + literal(this.context, this.activity_type);
        final Object[] record =
                selectDbValues(this.context, Constants.SLA_REQUEST_TABLE,
                    Constants.SLA_REQUEST_FIELDS, where);
        
        if (record == null) {
            // @translatable
            final String errorMessage =
                    "No SLA Request parameters found for type [{0}] AND ordering seq [{1}]";
    ...
    public void lookupResponseParameters() {
        ...
        if (this.requestParameters.containsKey("prob_type")
            ...
            final StringBuffer sql =
                    new StringBuffer("SELECT " + Constants.SLA_REQUEST_PKEYS + " FROM "
            ...
            final List<Object[]> records = selectDbRecords(this.context, sql.toString());
            ...
            // get the last record
            final Object[] record = records.get(records.size() - 1);
    ...
    private int retrieveSlaOfPM(final Map<String, Object> requestParameters2) {
        ...
        final boolean isEqNull = !StringUtil.notNullOrEmpty(this.requestParameters.get("eq_id"));
        final String[] currentFieldNameArray =
                isEqNull ? Constants.PM_REQUEST_PARAMETER_NAMES_HK
                        : Constants.PM_REQUEST_PARAMETER_NAMES_EQ;
        
        // Construct match sql string
        final String matchSql = getMatchSql(fieldsValue, isEqNull);
        ...
        final DataSource slaRequestSelectDS =
                DataSourceFactory.createDataSourceForFields(
                    "helpdesk_sla_request",
                    new String[] { "activity_type", "prob_type", "pmp_id", "eq_std", "eq_id",
                            "site_id", "bl_id", "fl_id", "rm_id", "ordering_seq" }).addSort(
                    "ordering_seq");
            ...
            for (final String element : currentFieldNameArray) {
                ...
                for (final DataRecord hsr : slaRequestList) {
                    final String hsrFieldValue =
                            hsr.getString("helpdesk_sla_request" + "." + fieldName);
        ...
        if (returnedSlaRequestList.size() == 0) {
            returnedSlaRequestList =
                    slaRequestSelectDS.getRecords(" activity_type='" + Constants.PM_ACTIVITY_TYPE
                            + "' AND ordering_seq=1 ");
        }
     ...
     */
    private String getMatchSql(final JSONObject fieldValues, final boolean isEqNull) {
        String matchSql =
                " activity_type='" + Constants.PM_ACTIVITY_TYPE + "'" + " AND prob_type='"
                        + Constants.PM_PROB_TYPE + "'" + " AND ( 1=0 ";
        ...
        if (fieldValues.has("fl_id") && !"".equals(fieldValues.get("fl_id"))) {
            matchSql +=
                    " OR ( bl_id='" + fieldValues.get("bl_id") + "' " + " AND fl_id='"
                            + fieldValues.get("fl_id") + "' " + ")";
        }
        if (fieldValues.has("rm_id") && !"".equals(fieldValues.get("rm_id"))) {
            matchSql +=
                    " OR ( bl_id='" + fieldValues.get("bl_id") + "' " + " AND fl_id='"
                            + fieldValues.get("fl_id") + "' " + " AND rm_id='"
                            + fieldValues.get("rm_id") + "' " + ")";
        }
        if (!isEqNull) {
     ...
     * 
     * <p>
     * When the <code>activity_id</code> and <code>ordering_seq</code> and <code>priority</code> is
     * known the response parameters can be retrieved.
     * </p>
     * 
     * <p>
     * <b>Pseudo-code:</b>
     * <ol>
     * <li>Get response parameters for the given primary key from <code>helpdesk_sla_response</code>
     * </li>
     * <li>Put response parameters in the {@link #responseParameters map} or set automation booleans
     * and create the {@link ServiceWindow service window}</li>
    ...
    private void getResponseParametersFromPrimaryKey() {
        final String where =
                "activity_type = " + literal(this.context, this.activity_type)
                        + " AND ordering_seq =" + this.ordering_seq + " AND priority = "
                        + this.priority;
        
        final Object[] response =
                selectDbValues(this.context, Constants.SLA_RESPONSE_TABLE,
                    Constants.SLA_PARAMETER_NAMES, where);
        
        if (response == null) {
            // @translatable
            final String errorMessage = "No matching SLA found for [{0}]";
        ...
        final boolean allow_work_on_holidays = json.getInt("allow_work_on_holidays") > 0;
        ...
        this.serviceWindow =
                new ServiceWindow(this.context, getTimeValue(this.context,
                    json.get("serv_window_start")), getTimeValue(this.context,
                    json.get("serv_window_end")), json.getString("serv_window_days"),
                    allow_work_on_holidays, notNull(this.requestParameters.get("site_id")));
     ...
     * @return where clause
     */
    private String appendResponseQueryParameter(final String field) {
        final String parameter = getRequestStringParameter(field);
        String sql = " AND ( ";
    ...
    /**
     * Lookup the two standard fields <code>em_std</code> and <code>eq_std</code> from the equipment
     * and requestor request parameters.
     * 
     */
    private void lookupStandardParameters() {
        ...
        this.requestParameters.put(
            "em_std",
            notNull(selectDbValue(this.context, "em", "em_std",
    ...
    public void saveHelperRules() {
        ...
        // add all of them
        for (final Map<String, Object> stepMap : this.helperRules) {
    ...
    public Map<String, Map<String, Object>> calculateEscalation(java.sql.Date dateRequested,
        ...
        final Map<String, Object> response =
                calculateEscalationResponse(dateRequested, timeRequested);
        final Map<String, Object> completion =
                calculateEscalationCompletion(dateRequested, timeRequested);
     ...
     */
    private Map<String, Object> calculateEscalationCompletion(final java.sql.Date dateRequested,
            final java.sql.Time timeRequested) {
     ...
     * @return map with escalation date and time
     */
    private Map<String, Object> calculateEscalationResponse(final java.sql.Date dateRequested,
            final java.sql.Time timeRequested) {
    ...
    public void parseHelperRules() throws DocumentException {
        ...
        final Document document =
                reader.read(new ByteArrayInputStream(this.xmlHelperRules.getBytes()));
        ...
        for (final Iterator<Element> j = root.elementIterator("state"); j.hasNext();) {
            final Element state = j.next();
            final String status = state.attributeValue("value");
            final String activity_id = state.attributeValue("activity");
            
            int step_order = 1;
            String step_status = status;
            for (final Iterator<Element> k = state.elementIterator(); k.hasNext();) {
                final Element element = k.next();
                ...
                if (element.attribute("step") != null) {
                    ...
                    step_status =
                            notNull(selectDbValue(this.context, Constants.STEP_TABLE,
                                "step_status_result",
                                "activity_id = " + literal(this.context, activity_id)
                                        + " AND status = " + literal(this.context, status)
                                        + " AND step=" + literal(this.context, step)));
                }
                if (element.attribute("step_rejected") != null) {
                    String rejectedStep = (String) element.attribute("step_rejected").getData();
                    rejectedStep = unescape(rejectedStep);
                    stepMap.put("step_rejected", rejectedStep);
                }
                if (element.attribute("em_id") != null) {
                    final String em_id = (String) element.attribute("em_id").getData();
                    stepMap.put("em_id", unescape(em_id));
                }
                
                if (element.attribute("vn_id") != null) {
                    final String vn_id = (String) element.attribute("vn_id").getData();
                    stepMap.put("vn_id", unescape(vn_id));
                }
                if (element.attribute("cf_id") != null) {
                    final String cf_id = (String) element.attribute("cf_id").getData();
                    stepMap.put("cf_id", unescape(cf_id));
                }
                
                if (element.attribute("role") != null) {
                    final String role = (String) element.attribute("role").getData();
                    stepMap.put("role", unescape(role));
                }
                if (element.attribute("condition") != null) {
                    final String condition = (String) element.attribute("condition").getData();
                ...
                if (element.attribute("multiple_required") != null) {
                    final String multiple_required =
                            (String) element.attribute("multiple_required").getData();
                ...
                if (element.attribute("notify_responsible") != null) {
                    final String notify_resp =
                            (String) element.attribute("notify_responsible").getData();
    ...
    public void checkAutomation() {
        ...
        for (final Map<String, Object> step : this.helperRules) {
    ...
    private void saveAutomation() {
        ...
        final int autocreate_wr = this.autocreate_wr ? 1 : 0;
        values.put("autocreate_wr", new Integer(autocreate_wr));
        
        final int autocreate_wo = this.autocreate_wo ? 1 : 0;
        values.put("autocreate_wo", new Integer(autocreate_wo));
        
        final int autoschedule = this.autoschedule ? 1 : 0;
        values.put("autoschedule", new Integer(autoschedule));
        
        final int autoissue = this.autoissue ? 1 : 0;
        values.put("autoissue", new Integer(autoissue));
        
        final int approve = this.autoapprove ? 1 : 0;
        values.put("autoapprove", new Integer(approve));
        
        final int dispatch = this.autodispatch ? 1 : 0;
        values.put("autodispatch", new Integer(dispatch));
        
        final int accept = this.autoaccept ? 1 : 0;
    ...
    public JSONObject getSlaResponseAsJson() {
        ...
        final JSONObject json = new JSONObject(this.responseParameters);
        ...
        if (this.serviceWindow != null) { // SLA found, return null, not exception
            ...
            final int allow_work_on_holidays = this.serviceWindow.isAllowWorkOnHolidays() ? 1 : 0;
            ...
            final int notify = this.notifyRequestor ? 1 : 0;
            json.put("notify_requestor", notify);
            
            final int accept = this.autoaccept ? 1 : 0;
            json.put("autoaccept", accept);
            
            final int notifyCf = this.notifyCraftsperson ? 1 : 0;
            json.put("notify_craftsperson", notifyCf);
            
            final int notifySp = this.notifyServiceProvider ? 1 : 0;
    ...
    private void lookupSlaPriorityLevels() {
        ...
        final List<Object[]> records =
                selectDbRecords(this.context, Constants.SLA_RESPONSE_TABLE, new String[] {
                        "priority", "priority_label" }, where);
        
        if (records == null || records.size() <= 0) {
            return; // return null, not throw exception
        }
        for (final Object[] rec : records) {
            final int priority = (getIntegerValue(this.context, rec[0])).intValue();
            if (priority >= Constants.MINIMAL_PRIORITY && priority <= Constants.PRIORITY_LEVELS) {
                this.priority_levels[priority - Constants.MINIMAL_PRIORITY] =
                        getStringValue(rec[1]); // the
                // array
                // starts
                // at 0
    ...
    private void lookupHelperRules() {
        final String SQL =
                "SELECT status, step_order, step_type, activity_id, step,step_rejected, step_status, em_id, vn_id, cf_id, role, condition, multiple_required, notify_responsible "
                        + "FROM helpdesk_sla_steps WHERE activity_type = "
                        + literal(this.context, this.activity_type)
                        + " AND ordering_seq = "
                        + this.ordering_seq + " AND priority= " + this.priority;
        final String[] flds =
                { "status", "step_order", "step_type", "activity_id", "step", "step_rejected",
                        "step_status", "em_id", "vn_id", "cf_id", "role", "condition",
                        "multiple_required", "notify_responsible" };
        
        final List<DataRecord> steps = SqlUtils.executeQuery("helpdesk_sla_steps", flds, SQL);
        
        this.helperRules = new ArrayList<Map<String, Object>>();
        for (final DataRecord step : steps) {
            final Map<String, Object> stepMap = new HashMap<String, Object>();
            for (final String element : flds) {
     ...
     * @param activity_type Activity type
     */
    public void setActivity_type(final String activity_type) {
        this.activity_type = activity_type;
    }
    ...
    public void setOrdering_seq(final int ordering_seq) {
        this.ordering_seq = ordering_seq;
    }
     ...
     */
    public void setPriorityLevel(final String label, final int level) {
     ...
     */
    public String getPriorityLevel(final int level) {
    ...
    public void setServiceWindow(final ServiceWindow serviceWindow) {
    ...
    public void setXmlHelperRules(final String xmlHelperRules) {
    ...
    public void setHelperRules(final List helperRules) {
     ...
     */
    protected String getRequestStringParameter(final String parameter) {
     ...
     * @return field value (String) or null
     */
    public String getResponseStringParameter(final String parameter) {
     ...
     * This checks if the parameter is given in the {@link #responseParameters response parameter
     * map} and if the value is of type Integer
     * 
     * @param parameter field to retrieve
     * @return field value (Integer) or null
     */
    public Integer getResponseIntegerParameter(final String parameter) {
        if (this.responseParameters.containsKey(parameter)
     ...
     */
    public String getAssignee(final String tableName) {
        if (this.responseParameters.containsKey(tableName + "_id")) {
            return getResponseStringParameter(tableName + "_id");
        } else {
            // @translatable
            final String errorMessage =
                    "Illegal parameter for table [{0}] for assignee, only em, cf and vn";
    ...
    public boolean equals(final Object object) {
        ...
        if (object instanceof ServiceLevelAgreement) {
            final ServiceLevelAgreement sla = (ServiceLevelAgreement) object;
    ...
    public Map hasStepOfType(final String status, final String type) {
        for (final Map step : this.getHelperRules()) {
