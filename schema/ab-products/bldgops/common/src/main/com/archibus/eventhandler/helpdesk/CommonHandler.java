...
public class CommonHandler extends HelpdeskEventHandlerBase {
    ...
    public void getSelectList(String tableName,String valueField,String textField,String where) {
        ...
        if (!where.equals("") && !where.equals("null")) {
            records = selectDbRecords(context, "SELECT DISTINCT " + valueField + " AS value, "
                    + textField + " as text from " + tableName + " where "
                    + where);

        } else {
            records = selectDbRecords(context, "SELECT DISTINCT " + valueField + " AS value, "
                    + textField + " as text from " + tableName);
        }
        ...
        if (!records.isEmpty()) {
            for (Iterator it = records.iterator(); it.hasNext();) {
                ...
                json.put("value", notNull(record[0]));
                json.put("text", notNull(record[1]));
                items.put(json);
            }
        }
