...
public class PmWorkOrderGenerator extends JobBase implements DataSource.RecordHandler {
    ...
    public PmWorkOrderGenerator(final String pmType, final Date dateFrom, final Date dateTo,
        ...
        this.pmsidRestriction = pmsidRestriction;
    ...
    public void run() {
        ...
        if (this.woUpdateDS.isSqlServer()) {
    ...
    public void createWorkOrders(final String restriction) {
        ...
        this.notifyCraftPerson();
        this.notifySupervisior();
    }
     ...
     * 
     */
    private void notifyCraftPerson() {
        final EventHandlerContext context = ContextStore.get().getEventHandlerContext();
        ...
        for (final String cf : craftPersonList) {
            ...
            if (email != null && !email.equals("")) {
                ...
                message.setBody(this.CF_NOTIFY_BODY);
     ...
     * 
     */
    private void notifySupervisior() {
        final EventHandlerContext context = ContextStore.get().getEventHandlerContext();
        ...
        for (final String supervisior : supervisiorList) {
            ...
            if (email != null && !email.equals("")) {
                ...
                message.setBody(this.SV_NOTIFY_BODY);
        ...
        for (final DataRecord pmpsRecord : pmpsRecords) {
            ...
            if (wrId > 0) {
                ...
                if (slaResponse != null && cfId != null && !"".equals(cfId)) {
                    if (this.wrcfSelectForPmsDateDS.setParameter("pmsId", pmsId)
                        .setParameter("dateAssigned", dateAssigned).getRecord() == null) {
                        // Only create just one craftsperson assignment for the entire procedure
                        // (set of work requests),
                        createWorkRequestCraftperson(wrRecord, slaResponse);
                    }
                }
                if (counter++ == 0) {
                    firstWr = wrRecord;
    ...
    private void createStepLogForWorkRequest(final DataRecord wrRecord) {
        ...
        final DataRecord stepLogRecord = this.stepLogDS.createNewRecord();
        
        stepLogRecord.setValue("helpdesk_step_log.activity_id", "AbBldgOpsOnDemandWork");
        stepLogRecord.setValue("helpdesk_step_log.table_name", "wr");
        stepLogRecord.setValue("helpdesk_step_log.field_name", "wr_id");
        stepLogRecord.setValue("helpdesk_step_log.pkey_value", wrRecord.getValue("wr.wr_id"));
        stepLogRecord.setValue("helpdesk_step_log.status", wrRecord.getValue("wr.status"));
        stepLogRecord.setValue("helpdesk_step_log.step", "Basic");
        stepLogRecord.setValue("helpdesk_step_log.step_code", Common.generateUUID());
        stepLogRecord.setValue("helpdesk_step_log.date_created",
            wrRecord.getValue("wr.date_requested"));
        stepLogRecord.setValue("helpdesk_step_log.time_created",
            wrRecord.getValue("wr.time_requested"));
        stepLogRecord.setValue("helpdesk_step_log.date_response",
            wrRecord.getValue("wr.date_requested"));
        stepLogRecord.setValue("helpdesk_step_log.time_response",
            wrRecord.getValue("wr.time_requested"));
        
        this.stepLogDS.saveRecord(stepLogRecord);
    ...
    private void createStepLogForServiceRequest(final DataRecord alRecord) {
        ...
        stepLogRecord.setValue("helpdesk_step_log.activity_id", "AbBldgOpsHelpDesk");
        stepLogRecord.setValue("helpdesk_step_log.table_name", "activity_log");
        stepLogRecord.setValue("helpdesk_step_log.field_name", "activity_log_id");
        stepLogRecord.setValue("helpdesk_step_log.pkey_value",
            alRecord.getValue("activity_log.activity_log_id"));
        stepLogRecord
            .setValue("helpdesk_step_log.status", alRecord.getValue("activity_log.status"));
        stepLogRecord.setValue("helpdesk_step_log.step", "Basic");
        stepLogRecord.setValue("helpdesk_step_log.step_code", Common.generateUUID());
        stepLogRecord.setValue("helpdesk_step_log.date_created",
            alRecord.getValue("activity_log.date_requested"));
        stepLogRecord.setValue("helpdesk_step_log.time_created",
            alRecord.getValue("activity_log.time_requested"));
        stepLogRecord.setValue("helpdesk_step_log.date_response",
            alRecord.getValue("activity_log.date_requested"));
        stepLogRecord.setValue("helpdesk_step_log.time_response",
            alRecord.getValue("activity_log.time_requested"));
    ...
    private List<DataRecord> getMatchSlaFromFieldValues(final JSONObject fieldValues,
        ...
        final String matchSql = getMatchSqlForFindingSlas(fieldValues, isEqNull);
     ...
     */
    private String getMatchSqlForFindingSlas(final JSONObject fieldValues, final boolean isEqNull) {
        String matchSql =
                " activity_type='" + PM_ACTIVITY_TYPE + "' AND ( prob_type='" + PM_PROB_TYPE + "' ";
     ...
     */
    private void createWorkRequestCraftperson(final DataRecord wrRecord,
        ...
        wrcfRec.setValue("wrcf.cf_id", slaResponse.getString("helpdesk_sla_response.cf_id"));
    ...
    private final Map<String, DataRecord> changedParts = new HashMap<String, DataRecord>();
    ...
    private void reserveParts() {
        ...
        final DataSource selectWrptDs =
                DataSourceFactory.createDataSourceForFields("wrpt", new String[] { "wr_id",
                        "part_id", "qty_estimated", "status" });
        ...
        for (long i = this.startWrId; i <= this.endWrId; i++) {
            ...
            for (final DataRecord wrpt : wrptList) {
                ...
                final String status =
                        this.reservePart(wrpt.getString("wrpt.part_id"),
        ...
        for (final DataRecord part : this.changedParts.values()) {
            
            this.partDS.updateRecord(part);
    ...
    private String reservePart(final String partId, final double quantityRequired) {
        ...
        final DataRecord part = this.cacheParts.get(partId);
        ...
        if (quantityRequired <= quantityOnHand) {
            ...
            this.changedParts.put(partId, part);
            reserveStatus = "R";
        }
        ...
        final String calculatePartCostEstimates =
                " UPDATE wrpt SET wrpt.cost_estimated = "
                        + EventHandlerBase
                            .formatSqlIsNull(
                                ContextStore.get().getEventHandlerContext(),
                                " ( SELECT wrpt.qty_estimated * pt.cost_unit_std "
    ...
    private void createWorkRequestParts() {
        ...
        final String insertWrptSql =
                " INSERT INTO wrpt ( wr_id, date_assigned, time_assigned,  part_id, qty_estimated, status ) "
                        ...
                        + ", case when pmpspt.qty_required<=pt.qty_on_hand then 'R' else 'NI' end "
    ...
    private void createWorkRequestTrades() {
        ...
        final String insertWrtrSql =
                " INSERT INTO wrtr ( wr_id, date_assigned, time_assigned, tr_id, hours_est ) "
                        ...
                        + "where pmpstr.tr_id is not null and wr.wr_id>=" + this.startWrId
                        + "      AND wr.wr_id<=" + this.endWrId;
