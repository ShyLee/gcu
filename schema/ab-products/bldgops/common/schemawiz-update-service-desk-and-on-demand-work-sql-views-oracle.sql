--partialJobStatus=Service Desk and On Demand Work Views
...
hwr.wr_id,hwr.site_id,hwr.bl_id,hwr.fl_id,hwr.rm_id,hwr.requestor,hwr.status,hwr.dv_id,hwr.dp_id,hwr.ac_id,hwr.repair_type,
    ...
    hwr.wo_id,hwr.work_team_id,hwr.escalated_response,hwr.escalated_completion,hwr.date_completed,hwr.time_completed,hwr.date_escalation_response,hwr.time_escalation_response,hwr.date_escalation_completion,hwr.time_escalation_completion,hwr.supervisor,
    TO_NUMBER( TO_CHAR( date_requested, 'YYYY' )) || '-' ||  TO_NUMBER( TO_CHAR( date_requested, 'MM' )) AS month, 
    TO_NUMBER( TO_CHAR( date_requested, 'YYYY' )) || '-' ||  TO_NUMBER( TO_CHAR( date_requested, 'WW' )) AS week,
...
select pkey_value as wr_id,em_id,vn_id,cf_id,user_name,date_created,time_created,date_response,time_response,status,comments,step,step_code,step_type,step_log_id,activity_id 
