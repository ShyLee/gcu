...
function SLA_changePriority(panelId){	 	
	...
	if(ordering_seq > 0){
		var priority_level = panel.getFieldElement("activity_log.priority").value;	
		var activity_type = panel.getFieldValue("activity_log.activity_type");
		...
		if(result.code == 'executed'){
			...
			if(priority_level == 0 ||priority_level == '0'){
				if($("priority_value0").innerHTML != ""){
					priority_level = getMessage("forPriority")+ " " + $("priority_value0").innerHTML;
				} else {
					priority_level = "";
				}
			} else {
