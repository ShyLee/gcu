...
//    return;
	...
	if(activityTypeValue=="SERVICE DESK - GROUP MOVE"){
		...
		}else if(valueExistsNotEmpty(date_start)&&compareLocalizedDates(date_end,date_start)){
			View.showMessage(getMessage('errorDateStartEnd'));
		}else if(compareLocalizedDates(date_end,getCurrentDate())){
		...
		}else if(compareLocalizedDates(move_end,getCurrentDate())){
    ...
    if(result){
    	...
    	tabs.date_start = getQuestionnaireFieldValue("date_start");
    	tabs.move_date = getQuestionnaireFieldValue("date_start");
    	tabs.date_end = getQuestionnaireFieldValue("date_end");
