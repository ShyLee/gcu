...
//    	return;
	...
	*/
	onSave: function(){
    	...
    	//kb 3035208 Don't need to prompt message:Have Employee without "To Location" in Select Employee panel 
    	//when user clicks "Save" button. This message only needed when user clicks "Issue Request" button.
...
function setSelectability(date_start,date_end,restriction,objBlandFl){
    ...
    for (var i = 0; i < allRmRecords.length; i++) {
    	var record = allRmRecords[i];
...
//        	drawingPanel.setSelectability.defer(1000, this, [opts_selectable, true]);
    ...
    reHightLightIfExistsFuture(drawingPanel,objBlandFl.blId,objBlandFl.flId,rooms,dateFormat(date_end));
...
function reSetHighLight(records,flag){
    ...
    if(flag==0){
        ...
        if(callFutrueFalg){
        	//kb 3037548 add logic with future trans when caculate highlight occupancy caculation.
        	reHightLightIfExistsFuture(drawingPanel,tempObjforHightLight.blId,tempObjforHightLight.flId,deletedRooms,dateFormat(tempObjforHightLight.date_end));
        }
    }else{
    	for (var j = 0; j < rooms.length; j++) {
    		var row = rooms[j];
