         ...
         for (var j = 0; j < grid.rows.length; j++) {
             ...
             if(!isInitial&&pct_id==''){
	             if(originalAssignments.length>0){
	            	 for(var m=0;m<originalAssignments.length;m++)  {
	            		 var blIdOld=originalAssignments[i]['bl_id'];
	            		 var flIdOld=originalAssignments[i]['fl_id'];
	            		 var rmIdOld=originalAssignments[i]['rm_id'];
	            		 var pctIdOld=originalAssignments[i]['pct_id'];
...
function onNext(){
	...
	try {
		var result = Workflow.callMethod("AbSpaceRoomInventoryBAR-SpaceTransactionProcess-submitDepartmentSpace",
