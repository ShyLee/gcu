var abHpdWrReqIssueSlktControllert = View.createController("abHpdWrReqIssueSlktControllert",{
	...
	requestConsole_afterRefresh: function(){
		...
		var ifRefresh = top.window.location.parameters["refresh_from_ab_helpdesk_workrequest_issue_update"]; 
		if( valueExists(ifRefresh)&& ifRefresh == true){
			top.window.location.parameters["refresh_from_ab_helpdesk_workrequest_issue_update"] = false;
	...
	requestConsole_onClear: function(){
 	...
 	getRestriction: function(){
 		var dateRequestedFrom = this.requestConsole.getFieldElement("wo.date_assigned.from").value;
		var dateRequestedTo = this.requestConsole.getFieldElement("wo.date_assigned.to").value;
