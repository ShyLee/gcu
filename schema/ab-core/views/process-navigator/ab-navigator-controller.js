...
function initNavigator(navigatorType, viewName) {
    ...
    if (dashboardTabsControl) {
		...
		mainToolbar.setNavigatorDashboardMessage();
	...
	if (navigatorType === 'navigator') {
		...
		if (valueExistsNotEmpty(viewName)) {
			...
			viewPanel.loadView(viewName, View.restriction);
			navigatorController.highlightTask(viewName);        
		}
	}
	else if (navigatorType === 'dashboard') {
		for (var i=0, tabItem; tabItem = dashboardTabsControl.activityTabItems[i]; i++) {
		    // find the active tab, load the view    
			if (tabItem.activeTab != 0) {
				var handler = dashboardTabsControl.getEventListener('clickitem');
				handler(tabItem.activeTab.record);
				break;
			}
		}
	}
