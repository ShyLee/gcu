...
Ab.navigator.Toolbar = Ab.view.Component.extend({
    ...
    doLayout: function() {
		...
		var titleLink = 'http://www.gcu.edu.cn/';

		//var target = jQuery('div#mainToolbar');
		jQuery('div').find('#mainToolbar').append(
			'<div class="navigatorToolbar"><a id="mainToolbar_title" href="' + titleLink + '" target="blank"></a></div>');
	...
	onSearchBoxKeyPress: function(event) {
		...
		if (charCode == 13) {
			...
			if (searchText.length > 0) {
				...
				if (userLang != ENGLISH) {
					taskTitleColumn = 'afm_ptasks.task_ch';
				}
