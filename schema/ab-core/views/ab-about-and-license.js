...
View.createController('aboutAndLicense', {
    ...
    displayLicenses: function(licenseDTO) {
        ...
        // determine whether this is an Application Control Point-style license
    	var isOldStyleLicense = false;
        for (var i = 0; i < licenseDTO.licenses.length; i++) {
        	var license = licenseDTO.licenses[i];
        	if (license.id == 'AbCoreLevel4Activity') {
        		isOldStyleLicense = (license.concurrentUsers + license.namedUsers > 0);
        		break;
        	}
        }
        ...
        for (var i = 0; i < licenseDTO.licenses.length; i++) {
        	...
        	if (id === 'AbCorePDA' || id === 'AbEnterpriseAccessLicenses' || id === 'AbRPLMEsriExtensions') {
            	...
            	if (id == 'AbCoreLevel4Activity') {
            		...
            		// display AbCoreLevel4Activity concurrent and named users only for activity-style licensing
            		...
            		// do not display AbCoreLevel1, AbCoreLevel2, AbCoreLevel3, AbCoreLevel4, AbCorelevelLevel4System licenses if 
            		// (a) this is an Application Control Point license and 
            		// (b) their concurrent + named counts are zero
            		if (isOldStyleLicense && id.indexOf('AbCoreLevel') == 0 && (license.concurrentUsers + license.namedUsers == 0)) {
            			continue;
            		}
        ...
        if (isOldStyleLicense) {
            ...
            this.displayActivities(activities, true);
        }
    },
    ...
    displayOtherOptions: function(records) {
        ...
        for (var i = 0; i < records.length; i++) {
        	...
        	if (records[i].id === 'AbRPLMEsriExtensions') {
        		...
        		concurrentUsers = getMessage('yes');
        		namedUsers = getMessage('NA');
        	}
