View.createController('exSelectFields', {
	...
	columnsPanel_onHide: function(action){
		...
		var values = this.getValues(this.shownEl);
		...
		this.shownField.removeOptions(values);
	},
	...
	columnsPanel_onShow: function(action){
		// get selected options to hide
		var values = this.getValues(this.hiddenEl);
		...
		for(var i=0; i<this.treePanels.length; i++){
			...
			for(var j=0; j<columns.length; j++){
				...
				if(column.hidden == 'false'){
					var visibleField = {name: column.id, isPk: column.primaryKey, controlType: column.controlType, type: column.type, value: column.value, title: column.title};
