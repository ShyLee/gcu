...
Ab.timeline.TimelineController = Base.extend({
	...
	constructor: function(id, isEditable, policy) {
        ...
        EVENT.on(window, "resize", this.refreshTimelineUI, this, true);
	},
	...
	cacheTimeslotPositions: function() {
		...
		for (var c = 0; c < this.getTimemarks().length; c++) {
	        ...
	        var left = DOM.getX(timemarkCell) - this.timelineX + offsetX;
	...
	createRowBlocks: function(row) {
	    ...
	    this.addRowBlock(row, Ab.timeline.Timeslot.STATUS_NOT_AVAILABLE, 0, rowResource.columnAvailableFrom - 1);
	    this.addRowBlock(row, Ab.timeline.Timeslot.STATUS_FUTURE_BLOCK,  rowResource.columnAvailableFrom, rowResource.columnAvailableFrom + rowResource.preBlockTimeslots - 1);
	    this.addRowBlock(row, Ab.timeline.Timeslot.STATUS_FUTURE_BLOCK,  rowResource.columnAvailableTo - rowResource.postBlockTimeslots, rowResource.columnAvailableTo - 1);
	    this.addRowBlock(row, Ab.timeline.Timeslot.STATUS_NOT_AVAILABLE, rowResource.columnAvailableTo, this.getTimemarks().length - 1);
	...
	addRowBlock: function(row, status, startColumn, endColumn) {
	    if (endColumn >= startColumn) {
	        var block = new Ab.timeline.Block(this, row, startColumn, endColumn, status, true);
