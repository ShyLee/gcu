...
/**
 * Called only when index is selected.  Performs regular filter actions and then checks
...
/**
 * Called only when index is selected.  Performs regular clear filter actions and then checks
 * the checkboxes of any previously selected standard fields
 *
 * @param	indexString 	Obj passed from original indexListener
 * @return	None
 *
 */
function checkFieldsClearFilter(indexString){
	var grid = Ab.view.View.getControl('', 'fields_grid');
...
/**
 * Called only when filter is click.  Performs regular filter actions and then checks
 * the checkboxes of any previously selected standard fields
 *
 * @param	indexString 	Obj passed from original indexListener
 * @return	None
 *
 */
function checkStandardsFilter(indexString) {
	var grid = Ab.view.View.getControl('', 'field_grid');
...
function checkStandards(){
    ...
    if (fields == undefined) {
        view.tableGroups[index].fields = new Array();
        tabsFrame.newView = view;
    }
    ...
    for (var j = 0; j < numRows; j++) {
        ...
        // get the row, checkbox cell, field name cell, and table name cell
        var rowElement = dataRows[j];
        var checkBoxCell = rowElement.childNodes[0];
        var fieldNameCell = rowElement.childNodes[2];
        var tableNameCell = rowElement.childNodes[1];
        
        // loop through fields in the tablegroup of view object
        for (k in fields) {
            var table_name = fields[k].table_name;
            var field_name = fields[k].field_name;
                       
            // determine if the field was selected as a standard.  if so, check the checkbox
            if ((field_name == fieldNameCell.innerHTML) && (table_name == tableNameCell.innerHTML)) {
...
function checkBoxesForSelectedFields(){
    ...
    if ((grid != null) && (fields != undefined) && (fields.length > 0)) {
        ...
        for (var j = 0; j < numRows; j++) {
            var row = rows[j];
            var fieldName = row['afm_flds.field_name'];
            var tableName = row['afm_flds.table_name'];
            
            // loop through array of selected fields
            for (k = 0; k < fields.length; k++) {
            
                // if table name and field name matches, check the checkbox
                if ((fields[k].field_name == fieldName) && (fields[k].table_name == tableName)) {
                    var checkBoxCell = getCellElement(grid, j, 0);
                    checkBoxCell.firstChild.checked = true;
                }
            }
        }
...
function saveStandards(row){
	...
	if (valueExists(row)) {
		...
		// get the row index to get the selected row to get the checkbox
		var grid = Ab.view.View.getControl('', 'field_grid');
		var rowIndex = row.index;
		var dataRow = grid.getDataRows()[rowIndex];
		
		if (valueExists(dataRow)) {
			var selectedCheckbox = dataRow.firstChild.firstChild;
