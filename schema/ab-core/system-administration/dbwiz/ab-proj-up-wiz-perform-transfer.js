var performTransferController = View.createController('performTransferController', {
    ...
    actionProgressPanel_onStart:function(){
		...
		if(transferPanel.getSelectedGridRows().length == 0){
	...
	callTransferJob:function(){
		...
		if (!this.progressPanelWrapper.isJobStarted()) {
			...
			try{
				...
				if (this.wizardController.transferOut){
					var isDeleteFiles = this.wizardController.deleteBeforeTransferOut;
					this.startTransferOutJob(isDeleteFiles);
				}else if(this.wizardController.transferIn){
					var isDeleteFiles = this.wizardController.deleteAfterTransferIn;
					var ddSelected = this.wizardController.isMergeDataDict;
					this.startTransferInJob(isDeleteFiles, ddSelected);
				}else if(this.wizardController.compare){
   ...
   afterJobFinished: function(status) {
		...
		// the job has finished	and we can clear the interval
	   	...
	   	if (this.wizardController.isMergeDataDict){
			var mergeTab = this.wizardController.updProjWizTabs.tabs[3];
			var panelGrid = mergeTab.getContentFrame().View.controllers.items[0].afmFldsTrans_grid;
		   	panelGrid.refresh();
	   	...
	   	if (this.wizardController.compare){
			var compareTab = this.wizardController.updProjWizTabs.tabs[4];
	...
	startTransferOutJob:function(isDeleteFiles){
		try {
			ProjectUpdateWizardService.startTransferOutJob(isDeleteFiles,
					{
	...
	startTransferInJob:function(isDeleteFiles, ddSelected){
		try {
			ProjectUpdateWizardService.startTransferInJob(isDeleteFiles, ddSelected,
					{
					callback: function(job_id) {
						performTransferController.afterCallJob(job_id);
	    			},
	...
	afmTransferSetCompare_grid_afterRefresh: function(){
		...
		for(var i=0;i< grid.rows.length;i++){
			...
			var tableName = row['afm_transfer_set.table_name'];
			var URLToCsv = View.getBaseUrl() + "/projects/users/" + View.user.name.toLowerCase() + "/" + tableName + ".csv";
