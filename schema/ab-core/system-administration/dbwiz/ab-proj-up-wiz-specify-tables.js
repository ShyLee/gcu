var specifyTablesController = View.createController('specifyTables_ctrl', {
	...
	afterInitialDataFetch:function(){
		...
		checkTableLike();
		this.afmFileLocation = '/projects/users/' + View.user.name.toLowerCase();
	},
	...
	updProjSpecifyTables_onNext:function(){
		...
		if(this.wizardController.compare){
			...
			archibusFiles[0] = this.afmFileLocation + '/afm_tbls.csv';
			archibusFiles[1] = this.afmFileLocation + '/afm_flds.csv';
	...
	getFileNotExistsWarningMessage: function(){
		var endOfMessage = getMessage('fileIsMissing');
		var path = View.contextPath + this.afmFileLocation;
		var	tblsFileMessage = path + '/afm_tbls.csv ';
		var	fldsFileMessage = path + '/afm_flds.csv ';
		return tblsFileMessage + getMessage('andOr') + ' ' + fldsFileMessage + endOfMessage;
	},
