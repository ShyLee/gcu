	...
	*/
			...
			if(button == 'yes'){
				...
				for(var i=0;i<gridPanel.rows.length;i++){
					if(i<displayOrders.length){
						gridPanel.gridRows.get(i).setFieldValue('afm_ptasks.display_order', displayOrders[i]);
					}else{
						break;
					}
				}
	...
	viewPanel_onSave: function(){
		...
		if(result.data.message == 'message_saved'){
			...
			var messageElement = Ext.DomHelper.append(this.saveMessagePanel.getMessageCell(), '<p>' + getMessage(result.data.message)+' \n('+ fullFileName+')' + '</p>', true);
	...
	viewPanel_onComplete: function(){
		...
		// clear save message
		var messageCell = this.saveMessagePanel.getMessageCell();
	...
	updateOrder: function(){
		for(var i=0;i<this.viewsGrid.rows.length;i++){
			if(i<this.displayOrders.length){
				this.viewsGrid.gridRows.get(i).setFieldValue('afm_ptasks.display_order', this.displayOrders[i]);
			}else{
				break;
			}
		}
