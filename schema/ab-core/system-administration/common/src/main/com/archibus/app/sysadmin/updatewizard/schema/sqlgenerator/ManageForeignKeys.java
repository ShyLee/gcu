...
public class ManageForeignKeys {
    ...
    public void dropForeignKey(final DatabaseSchemaForeignKeyDef foreignKey) {
        String alterStmt = SchemaUpdateWizardConstants.ALTER_TABLE;
        
        if (SchemaUpdateWizardUtilities.useAfmSecurePrefixForTable(foreignKey.getTableName())) {
            alterStmt +=
                    SchemaUpdateWizardConstants.getSecureUser() + SchemaUpdateWizardConstants.DOT;
        }
        
        alterStmt += foreignKey.getTableName();
        
        if (SqlUtils.isOracle() || SqlUtils.isSqlServer()) {
            alterStmt += SchemaUpdateWizardConstants.DROP_CONSTRAINT;
        } else {
            alterStmt += SchemaUpdateWizardConstants.DELETE_FKEY;
        }
        
        alterStmt += foreignKey.getRole();
        if (!this.contraintsToDrop.contains(foreignKey.getRole())) {
            this.contraintsToDrop.add(foreignKey.getRole());
            this.output.runCommand(alterStmt, DataSource.DB_ROLE_SCHEMA);
        }
    }
    ...
    private void runDropForeignKeyStmt(final String tableNameFrom, final String tableNameTo,
        ...
        for (final DatabaseSchemaForeignKeyDef fkey : foreignKeysFrom) {
            ...
            if (!this.contraintsToDrop.contains(constraintName)) {
                ...
                if (SchemaUpdateWizardUtilities.useAfmSecurePrefixForTable(tableNameFrom)) {
                    dropConstraintStmt += "afm_secure.";
                }
                dropConstraintStmt += tableNameFrom;
                if (SqlUtils.isOracle() || SqlUtils.isSqlServer()) {
                ...
                this.output.runCommand(dropConstraintStmt, DataSource.DB_ROLE_SCHEMA);
            }
        }
    }
