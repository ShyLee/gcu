...
public class CompareFieldProperties extends LoadLangFieldData {
        ...
        if (SqlTypes.isDataTypeDifferent(afmDataType, csvDataType)) {
            csvDiff =
                    String.format(CompareFieldUtilities.OLD_AND_NEW_VAL_MESS,
                        SqlTypes.dataTypeToLiteral(Integer.valueOf(afmDataType)),
                        SqlTypes.dataTypeToLiteral(Integer.valueOf(csvDataType)));
        }
    ...
    private void sizeMatter(final String csvDataType) {
        if (!this.isDocStgAfmType()) {
            if (new SqlTypes(Integer.valueOf(csvDataType)).isDecimalsMatters()) {
                compareDecimals();
            }
            if (new SqlTypes(Integer.valueOf(csvDataType)).isSizeMatters()) {
                compareSize();
            }
        }
    }
    ...
    private void compareSize() {
        ...
        final int afmSizeInt = Integer.valueOf(afmSize);
        ...
        if (!csvSize.equalsIgnoreCase(afmSize)) {
            ...
            final int csvSizeInt = Integer.valueOf(csvSize);
    ...
    private void compareDecimals() {
        ...
        final int afmDecimalsInt = Integer.valueOf(afmDecimals);
        ...
        if (!csvDecimals.equalsIgnoreCase(afmDecimals)) {
            csvDiff =
                    String.format(CompareFieldUtilities.OLD_AND_NEW_VAL_MESS, afmDecimals,
                        csvDecimals);
            final int csvSizeI = Integer.valueOf(csvDecimals);
    ...
    private void comparePrimaryKey() {
        ...
        String csvDiff = "";
        if ((Integer.valueOf(csvPkey) > 0 || Integer.valueOf(afmPkey) > 0)
