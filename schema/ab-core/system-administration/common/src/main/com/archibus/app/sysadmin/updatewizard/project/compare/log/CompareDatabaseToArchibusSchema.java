...
public class CompareDatabaseToArchibusSchema implements LogWriter {
    ...
    private List<String> getLogMessageForMissingTblsFromSql() {
        ...
        for (final DataRecord difference : this.diffRecords) {
            final String changeType =
                    difference.getValue(UpdateArchibusSchema.CHANGE_TYPE_FIELD).toString();
            if (changeType.equals(DifferenceMessage.TBL_IS_NEW.name())) {
                hasDiffs = true;
                messages.add(difference.getValue(ProjectUpdateWizardUtilities.A_F_T_TABLE_NAME)
                    .toString());
            }
        }
        if (!hasDiffs) {
    ...
    private List<String> getLogMessageForMissingTblsFromArch() {
        ...
        for (final DataRecord difference : this.diffRecords) {
            final String changeType =
                    difference.getValue(UpdateArchibusSchema.CHANGE_TYPE_FIELD).toString();
            if ("Table is in SQL only".equals(changeType)) {
                hasDiffs = true;
                messages.add(difference.getValue(ProjectUpdateWizardUtilities.A_F_T_TABLE_NAME)
                    .toString());
            }
    ...
    private List<String> getLogMessageForMissingFldsFromSql() {
        ...
        for (final DataRecord difference : this.diffRecords) {
            final String changeType =
                    difference.getValue(UpdateArchibusSchema.CHANGE_TYPE_FIELD).toString();
            if (changeType.equals(DifferenceMessage.NEW.name())) {
                hasDiffs = true;
                ...
                final String fieldName =
                        difference.getValue(ProjectUpdateWizardUtilities.A_F_T_FIELD_NAME)
                            .toString();
                messages.add(tableName + SEPARATOR + fieldName);
            }
        }
        if (!hasDiffs) {
    ...
    private List<String> getLogMessageForMissingFldsFromArch() {
        ...
        for (final DataRecord difference : this.diffRecords) {
            final String changeType =
                    difference.getValue(UpdateArchibusSchema.CHANGE_TYPE_FIELD).toString();
            if ("Field is in SQL only".equals(changeType)) {
                hasDiffs = true;
                final String tableName =
                        difference.getValue(ProjectUpdateWizardUtilities.A_F_T_TABLE_NAME)
                            .toString();
