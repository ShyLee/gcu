...
public class AddDropFields extends CreateAlterTable {
     ...
     */
    private void drop(final List<String> fieldsNames) {
        for (final String fieldName : fieldsNames) {
            final DatabaseSchemaTableDef sqlTblDef = this.getSqlTableDef().loadTableFieldsDefn();
            if (sqlTblDef.getFieldDef(fieldName).isForeignKey()) {
                final List<DatabaseSchemaForeignKeyDef> sqlFkeys = sqlTblDef.getFKeysDefn();
                dropForeignKeys(sqlFkeys);
            }
            if (sqlTblDef.getFieldDef(fieldName).isPrimaryKey()) {
                dropPrimaryKeys();
            }
            if (SqlUtils.isSqlServer()) {
                final String dropDfltConstr =
                        SqlServerActions.dropDefaultValueConstraintIfExists(
                            sqlTblDef.getTableName(), fieldName);
                this.output.runCommand(dropDfltConstr, DataSource.DB_ROLE_SCHEMA);
            }
    ...
    protected void dropColumn(final String oldFieldName) {
        String deleteColumn = " DELETE ";
        if (SqlUtils.isOracle() || SqlUtils.isSqlServer()) {
            deleteColumn = SchemaUpdateWizardConstants.DROP_COLUMN;
        }
        final String dropColumnStmt =
                SchemaUpdateWizardConstants.ALTER_TABLE + this.getTableName() + deleteColumn
                        + oldFieldName;
    ...
    private void add(final List<String> fieldsNames) {
        ...
        final int noOfRecords = ProjectUpdateWizardUtilities.getNoOfRecordsFromDB(this.getSqlTableDef().getTableName());
    ...
    private void addNotNullFields(final String fieldName) {
        ...
        final AlterField fieldToAlter =
                new AlterField(fieldToCompare, this.isNlsToChar(), false, this.output);
        final String alterFieldStmt =
                fieldToAlter.getAlterFieldStatementPrefix() + fieldName
                        + fieldToAlter.getAllowNullStatement();
        this.output.runCommand(alterFieldStmt, DataSource.DB_ROLE_SCHEMA);
    }
