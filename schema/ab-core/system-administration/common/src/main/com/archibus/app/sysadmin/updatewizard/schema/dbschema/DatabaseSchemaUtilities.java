...
public final class DatabaseSchemaUtilities {
    ...
    public static boolean existsConstraint(final String roleName) {
        boolean exists = true;
        String query = String.format(SystemSql.CONSTRAINT_FINDER_SYBASE, roleName);
        if (SqlUtils.isOracle()) {
            query = String.format(SystemSql.CONSTRAINT_FINDER_ORACLE, roleName);
        } else if (SqlUtils.isSqlServer()) {
            query = String.format(SystemSql.CONSTRAINT_FINDER_MSSQL, roleName);
        }
        
        final DataSource constraintDS =
                DataSourceFactory.createDataSource()
                    .addTable(ProjectUpdateWizardConstants.AFM_TBLS).addQuery(query);
        final List<DataRecord> records = constraintDS.getRecords();
        if (records.isEmpty()) {
            exists = false;
        }
        return exists;
    ...
    public static boolean isFieldInSql(final String tableName, final String fieldName) {
        ...
        final String sql = String.format(SystemSql.EXISTS_FIELD_ORACLE_SQL, tableName, fieldName);
