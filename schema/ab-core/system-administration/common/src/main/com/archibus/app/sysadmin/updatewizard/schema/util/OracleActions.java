...
public final class OracleActions {
    ...
    public static boolean existsSequence(final String sequenceName) {
        ...
        final int result = Integer.valueOf(record.getValue("afm_tbls.exists_seq").toString());
    ...
    public static boolean existsTrigger(final String triggerName) {
        ...
        final int result = Integer.valueOf(record.getValue("afm_tbls.exists_trigger").toString());
        return result > 0 ? true : false;
    }
        ...
        if (DatabaseSchemaUtilities.isFieldInSql(tableName, fieldName)) {
            final String query =
                    String.format(SystemSql.MAX_AUTONUM_VALUE_ORACLE_SQL, fieldName, fieldName,
                        tableName);
            final DataSource getMaxDS = DataSourceFactory.createDataSource();
            ...
            startValue = Long.valueOf(record.getValue(tableName + DOT + fieldName).toString()) + 1;
        }
        return startValue;
    }
